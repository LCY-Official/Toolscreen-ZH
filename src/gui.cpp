...
bool Spinner(const char* id_label, int* v, int step = 1, int min_val = INT_MIN, int max_val = INT_MAX, float inputWidth = 80.0f, float margin = 0.0f);
...
static std::map<std::string, std::future<ImagePickerResult>>
                                          g_imagePickerFutures;
...
static bool HttpGetToString(const std::wstring& url, std::string& outBody, std::string& outError) {
    ...
    HINTERNET hSession =
        WinHttpOpen(L"Toolscreen/1.0", WINHTTP_ACCESS_TYPE_AUTOMATIC_PROXY, WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS, 0);
    if (!hSession) {
    ...
    for (const auto& roleJson : parsed) {
        ...
        if (roleJson.contains("members") && roleJson["members"].is_array()) {
            for (const auto& member : roleJson["members"]) {
                if (member.is_string()) {
                    role.members.push_back(member.get<std::string>());
                }
            }
        }

        if (!role.name.empty()) {
            outRoles.push_back(std::move(role));
        }
    }
...
void StartSupportersFetch() {
    bool expected = false;
    if (!g_supportersFetchStarted.compare_exchange_strong(
    ...
    std::thread([]() {
        static const std::wstring kMembershipUrl =
            L"https://raw.githubusercontent.com/jojoe77777/toolscreen-meta/refs/"
            L"heads/main/membership.json";
        ...
        while (true) {
            ...
            if (fetchError.empty()) {
                fetchError = "Unknown fetch failure";
            }
            g_supportersFetchEverFailed.store(true, std::memory_order_release);
            Log("Supporters metadata fetch failed; retrying in " +
                std::to_string(retryDelaySeconds) + "s. Reason: " + fetchError);

            std::this_thread::sleep_for(std::chrono::seconds(retryDelaySeconds));
            retryDelaySeconds =
                (std::min)(retryDelaySeconds * 2, kMaxRetryDelaySeconds);
        }
    }).detach();
}
...
void RegisterBindingInputEvent(UINT uMsg, WPARAM wParam, LPARAM lParam) {
    static constexpr ULONG_PTR kToolscreenInjectedExtraInfo =
        (ULONG_PTR)0x5453434E;
    if ((uMsg == WM_KEYDOWN || uMsg == WM_SYSKEYDOWN) &&
        GetMessageExtraInfo() == kToolscreenInjectedExtraInfo) {
        return;
    }
    ...
    auto resolveVkFromKeyboardMessage = [](WPARAM keyWParam,
                                           LPARAM keyLParam) -> DWORD {
        ...
        UINT mappedVk = 0;
        if ((scanCodeWithFlags & 0xFF) != 0) {
            mappedVk = MapVirtualKey(scanCodeWithFlags, MAPVK_VSC_TO_VK_EX);
        }

        DWORD resolvedVk = static_cast<DWORD>(keyWParam);
        if (mappedVk != 0) {
            resolvedVk = static_cast<DWORD>(mappedVk);
        }

        // Windows typically reports VK_CONTROL/VK_MENU/VK_SHIFT in wParam for both
        // sides.
        const bool isExtended = (keyLParam & (1LL << 24)) != 0;
        const UINT scanOnly   = static_cast<UINT>((keyLParam >> 16) & 0xFF);
        if (resolvedVk == VK_SHIFT) {
            DWORD lr =
                static_cast<DWORD>(::MapVirtualKeyW(scanOnly, MAPVK_VSC_TO_VK_EX));
            if (lr != 0) {
                resolvedVk = lr;
            }
...
uint64_t GetLatestBindingInputSequence() {
    return g_bindingInputEventSequence.load(std::memory_order_acquire);
}
...
bool ConsumeBindingInputEventSince(uint64_t& lastSeenSequence, DWORD& outVk, LPARAM& outLParam, bool& outIsMouseButton) {
    uint64_t currentSequence =
        g_bindingInputEventSequence.load(std::memory_order_acquire);
...
static std::wstring GetImagePickerInitialDirectory(
    const std::wstring& fallbackInitialDir
) {
    PWSTR downloadsPath = nullptr;
    if (SUCCEEDED(SHGetKnownFolderPath(FOLDERID_Downloads, KF_FLAG_DEFAULT, NULL, &downloadsPath)) &&
        downloadsPath != nullptr) {
        std::wstring downloads(downloadsPath);
        CoTaskMemFree(downloadsPath);
        if (!downloads.empty() && std::filesystem::exists(downloads)) {
            return downloads;
        }
    }

    if (!fallbackInitialDir.empty() &&
        std::filesystem::exists(fallbackInitialDir)) {
        return fallbackInitialDir;
    }
...
static std::string ValidateImageFile(const std::string& path, const std::wstring& toolscreenPath) {
    ...
    if (!std::filesystem::exists(final_path)) {
        return tr("error.file_not_exists");
    }
    ...
    if (stbi_info(path_utf8.c_str(), &w, &h, &c) == 0) {
        ...
        return ftr("error.invalid_image", (reason ? reason : trc("error.unknown_format")));
    }

    if (w <= 0 || h <= 0) {
        return ftr("error.invalid_image", "Invalid image dimensions");
    }

    if (w > 16384 || h > 16384) {
        return tr("error.image_too_large");
    }

    return "";
}

static ImagePickerResult OpenImagePickerAndValidate(
    HWND ownerHwnd, const std::wstring& initialDir,
    const std::wstring& toolscreenPath
) {
    ImagePickerResult result;

    HWND safeOwner = NULL;
    if (ownerHwnd != NULL && IsWindow(ownerHwnd)) {
        // Check if the window is in the foreground or is the current thread's
        // window
        ...
        if (foreground == ownerHwnd || windowThreadId == currentThreadId) {
            safeOwner = ownerHwnd;
        }
    }
    ...
    ofn.lpstrFilter =
        L"Image Files "
        L"(*.png;*.jpg;*.jpeg;*.bmp;*.gif)\0*.png;*.jpg;*.jpeg;*.bmp;*.gif\0PNG "
        L"Files (*.png)\0*.png\0All Files (*.*)\0*.*\0";
    ofn.nFilterIndex              = 1;
    ofn.lpstrFileTitle            = NULL;
    ofn.nMaxFileTitle             = 0;
    std::wstring pickerInitialDir = GetImagePickerInitialDirectory(initialDir);
    ofn.lpstrInitialDir =
        pickerInitialDir.empty() ? NULL : pickerInitialDir.c_str();
...
static void ClearExpiredImageErrors() {
    ...
    for (const auto& [key, time] : g_imageErrorTimes) {
        auto elapsed =
            std::chrono::duration_cast<std::chrono::seconds>(now - time).count();
        if (elapsed >= 5) {
...
static void SliderCtrlClickTip() {
    ImGui::TextDisabled("%s", trc("tip.double_click"));
    ImGui::Spacing();
}

static bool ShouldForceSliderTextInputOnDoubleClick() {
    ImGuiIO& io = ImGui::GetIO();
    if (io.MouseClickedCount[ImGuiMouseButton_Left] != 2) {
        return false;
    }
    ...
    return mouse.x >= cursor.x && mouse.x <= (cursor.x + width) &&
           mouse.y >= cursor.y && mouse.y <= (cursor.y + height);
}

static bool SliderFloatDoubleClickInput(const char* label, float* v, float v_min, float v_max, const char* format = "%.3f", ImGuiSliderFlags flags = 0) {
    ImGuiIO&   io             = ImGui::GetIO();
    ...
    if (ImGui::IsItemHovered()) {
        ImGui::SetTooltip("%s", trc("tooltip.double_click"));
    }
    return changed;
}

static bool SliderIntDoubleClickInput(const char* label, int* v, int v_min, int v_max, const char* format = "%d", ImGuiSliderFlags flags = 0) {
    ImGuiIO&   io             = ImGui::GetIO();
    const bool forceTextInput = ShouldForceSliderTextInputOnDoubleClick();
    const bool prevCtrl       = io.KeyCtrl;
    if (forceTextInput) {
...
namespace ImGui {
bool SliderFloatDoubleClickInput(const char* label, float* v, float v_min, float v_max, const char* format = "%.3f", ImGuiSliderFlags flags = 0) {
...
static void RenderTransitionSettingsHorizontalNoBackground(
    ModeConfig& mode, const std::string& idSuffix
) {
    ImGui::PushStyleVar(ImGuiStyleVar_CellPadding, ImVec2(10, 5));

    if (ImGui::BeginTable(
            ("TransitionTableNoBg" + idSuffix).c_str(), 1,
            ImGuiTableFlags_Borders | ImGuiTableFlags_SizingStretchProp
        )) {
        ImGui::TableSetupColumn(trc("transition.game"), ImGuiTableColumnFlags_WidthStretch, 1.0f);
        ...
        ImGui::Text("%s", trc("transition.viewport_animation"));
        ImGui::PopStyleColor();
        ImGui::Separator();

        ImGui::Text("%s", trc("transition.type"));
        ...
        if (ImGui::Combo(("##GameTrans" + idSuffix).c_str(), &gameType, gameOptions, IM_ARRAYSIZE(gameOptions))) {
            mode.gameTransition = (gameType == 0) ? GameTransitionType::Cut
        ...
        if (mode.gameTransition == GameTransitionType::Bounce) {
            ImGui::Spacing();
            ImGui::Text("%s", trc("transition.duration"));
            ImGui::SetNextItemWidth(-FLT_MIN);
            if (Spinner(("##GameDur" + idSuffix).c_str(), &mode.transitionDurationMs, 10, 50, 5000)) {
                g_configIsDirty = true;
            }
            ImGui::SameLine();
            ImGui::TextDisabled("%s", trc("transition.ms"));

            ImGui::Spacing();
            ImGui::Text("%s", trc("transition.ease_in"));
            ImGui::SetNextItemWidth(-FLT_MIN);
            if (ImGui::SliderFloat(("##EaseIn" + idSuffix).c_str(), &mode.easeInPower, 1.0f, 6.0f, "%.1f")) {
                g_configIsDirty = true;
            }

            ImGui::Text("%s", trc("transition.ease_out"));
            ImGui::SetNextItemWidth(-FLT_MIN);
            if (ImGui::SliderFloat(("##EaseOut" + idSuffix).c_str(), &mode.easeOutPower, 1.0f, 6.0f, "%.1f")) {
                g_configIsDirty = true;
            }

            ImGui::Spacing();
            ImGui::Text("%s", trc("transition.bounces"));
            ImGui::SetNextItemWidth(-FLT_MIN);
            if (Spinner(("##BounceCount" + idSuffix).c_str(), &mode.bounceCount, 1, 0, 10)) {
                g_configIsDirty = true;
            }

            if (mode.bounceCount > 0) {
                ImGui::Text("%s", trc("transition.intensity"));
                ImGui::SetNextItemWidth(-FLT_MIN);
                ...
                ImGui::Text("%s", trc("transition.bounce_ms"));
                ImGui::SetNextItemWidth(-FLT_MIN);
                if (Spinner(("##BounceDur" + idSuffix).c_str(), &mode.bounceDurationMs, 10, 20, 500)) {
                    g_configIsDirty = true;
                }
            }
            ...
            ImGui::Spacing();
            ImGui::Separator();
            if (ImGui::Checkbox((tr("transition.relative_streching") + "##" + idSuffix).c_str(), &mode.relativeStretching)) {
                g_configIsDirty = true;
            }
            if (ImGui::IsItemHovered()) {
    ...
    ImGui::TextDisabled("%s", trc("transition.note.fullscreen_background"));
}
...
static void RenderTransitionSettingsHorizontal(ModeConfig& mode, const std::string& idSuffix) {
    ...
    if (ImGui::BeginTable(
            ("TransitionTable" + idSuffix).c_str(), 1,
            ImGuiTableFlags_Borders | ImGuiTableFlags_SizingStretchProp
        )) {
        ImGui::TableSetupColumn(trc("transition.game"), ImGuiTableColumnFlags_WidthStretch, 1.0f);
        ImGui::TableHeadersRow();
...
if (ImGui::Checkbox((std::string(trc("transition.relative_stretching")) + "##" + idSuffix).c_str(), &mode.relativeStretching)) {                g_configIsDirty = true;
            ...
            if (ImGui::Checkbox((tr("transition.skip_x_animation") + "##" + idSuffix).c_str(), &mode.skipAnimateX)) {
                g_configIsDirty = true;
            }
            if (ImGui::IsItemHovered()) {
                ImGui::SetTooltip(
                    "%s", trc("transition.tooltip.skip_x_animation")
                );
            }

            if (ImGui::Checkbox((tr("transition.skip_y_animation") + "##" + idSuffix).c_str(), &mode.skipAnimateY)) {
                g_configIsDirty = true;
            }
            if (ImGui::IsItemHovered()) {
                ImGui::SetTooltip(
                    "%s", trc("transition.tooltip.skip_y_animation")
                );
            }
        }
    ...
    if (ImGui::Button((tr("transition.preview_transition") + "##" + idSuffix).c_str())) {
        ...
        Log("[GUI] Queued transition preview: " + g_config.defaultMode + " -> " +
            mode.id);
    }
    ImGui::SameLine();
    HelpMarker(ftrc("transition.tooltip.preview_transition", g_config.defaultMode));
}
...
BackgroundTransitionType StringToBackgroundTransitionType(
...
std::string VkToString(DWORD vk) {
    ...
    static std::map<DWORD, std::string> specialKeys = {{VK_LBUTTON, "MOUSE1"}, {VK_RBUTTON, "MOUSE2"}, {VK_MBUTTON, "MOUSE3"}, {VK_XBUTTON1, "MOUSE4"}, {VK_XBUTTON2, "MOUSE5"}, {VK_SHIFT, "SHIFT"}, {VK_LSHIFT, "LSHIFT"}, {VK_RSHIFT, "RSHIFT"}, {VK_CONTROL, "CTRL"}, {VK_LCONTROL, "LCTRL"}, {VK_RCONTROL, "RCTRL"}, {VK_MENU, "ALT"}, {VK_LMENU, "LALT"}, {VK_RMENU, "RALT"}, {VK_LWIN, "LWIN"}, {VK_RWIN, "RWIN"}, {VK_F1, "F1"}, {VK_F2, "F2"}, {VK_F3, "F3"}, {VK_F4, "F4"}, {VK_F5, "F5"}, {VK_F6, "F6"}, {VK_F7, "F7"}, {VK_F8, "F8"}, {VK_F9, "F9"}, {VK_F10, "F10"}, {VK_F11, "F11"}, {VK_F12, "F12"}, {VK_F13, "F13"}, {VK_F14, "F14"}, {VK_F15, "F15"}, {VK_F16, "F16"}, {VK_F17, "F17"}, {VK_F18, "F18"}, {VK_F19, "F19"}, {VK_F20, "F20"}, {VK_F21, "F21"}, {VK_F22, "F22"}, {VK_F23, "F23"}, {VK_F24, "F24"}, {VK_BACK, "BACKSPACE"}, {VK_TAB, "TAB"}, {VK_RETURN, "ENTER"}, {VK_CAPITAL, "CAPS LOCK"}, {VK_ESCAPE, "ESC"}, {VK_SPACE, "SPACE"}, {VK_PRIOR, "PAGE UP"}, {VK_NEXT, "PAGE DOWN"}, {VK_END, "END"}, {VK_HOME, "HOME"}, {VK_LEFT, "LEFT"}, {VK_UP, "UP"}, {VK_RIGHT, "RIGHT"}, {VK_DOWN, "DOWN"}, {VK_INSERT, "INSERT"}, {VK_DELETE, "DELETE"}, {VK_NUMPAD0, "NUM 0"}, {VK_NUMPAD1, "NUM 1"}, {VK_NUMPAD2, "NUM 2"}, {VK_NUMPAD3, "NUM 3"}, {VK_NUMPAD4, "NUM 4"}, {VK_NUMPAD5, "NUM 5"}, {VK_NUMPAD6, "NUM 6"}, {VK_NUMPAD7, "NUM 7"}, {VK_NUMPAD8, "NUM 8"}, {VK_NUMPAD9, "NUM 9"}, {VK_MULTIPLY, "NUM *"}, {VK_ADD, "NUM +"}, {VK_SEPARATOR, "NUM SEP"}, {VK_SUBTRACT, "NUM -"}, {VK_DECIMAL, "NUM ."}, {VK_DIVIDE, "NUM /"}, {VK_OEM_1, ";"}, {VK_OEM_PLUS, "="}, {VK_OEM_COMMA, ","}, {VK_OEM_MINUS, "-"}, {VK_OEM_PERIOD, "."}, {VK_OEM_2, "/"}, {VK_OEM_3, "`"}, {VK_OEM_4, "["}, {VK_OEM_5, "\\"}, {VK_OEM_6, "]"}, {VK_OEM_7, "'"}};
    if (specialKeys.count(vk)) {
        return specialKeys.at(vk);
    }
...
DWORD StringToVk(const std::string& keyStr) {
    ...
    static std::map<std::string, DWORD> keyMap = {{"MOUSE1", VK_LBUTTON}, {"LBUTTON", VK_LBUTTON}, {"LEFTMOUSE", VK_LBUTTON}, {"MOUSE2", VK_RBUTTON}, {"RBUTTON", VK_RBUTTON}, {"RIGHTMOUSE", VK_RBUTTON}, {"MOUSE3", VK_MBUTTON}, {"MBUTTON", VK_MBUTTON}, {"MIDDLEMOUSE", VK_MBUTTON}, {"MOUSE4", VK_XBUTTON1}, {"XBUTTON1", VK_XBUTTON1}, {"MOUSE BUTTON 4", VK_XBUTTON1}, {"MOUSEBUTTON4", VK_XBUTTON1}, {"MOUSE5", VK_XBUTTON2}, {"XBUTTON2", VK_XBUTTON2}, {"MOUSE BUTTON 5", VK_XBUTTON2}, {"MOUSEBUTTON5", VK_XBUTTON2},
                                                  ...
                                                  {"NUM /", VK_DIVIDE},
                                                  {"NUM/", VK_DIVIDE},
                                                  {"NUMPAD SEP", VK_SEPARATOR},
                                                  {"NUMPADSEP", VK_SEPARATOR},
                                                  {"NUM SEP", VK_SEPARATOR},
                                                  {"NUMSEP", VK_SEPARATOR},
...
std::vector<DWORD> ParseHotkeyString(const std::string& hotkeyStr) {
    ...
    if (usedSeparator != '\0') {
        ...
        std::string       keyPart;
        while (std::getline(ss, keyPart, usedSeparator)) {
...
void CopyToClipboard(HWND hwnd, const std::string& text) {
    if (!OpenClipboard(hwnd)) {
        Log("ERROR: Could not open clipboard. Error code: " +
    ...
    if (!EmptyClipboard()) {
        Log("ERROR: Could not empty clipboard. Error code: " +
            std::to_string(GetLastError()));
        return;
    }
    ...
    if (!hg) {
        Log("ERROR: GlobalAlloc failed. Error code: " +
            std::to_string(GetLastError()));
        return;
    }
    ...
    if (!pGlobal) {
        Log("ERROR: GlobalLock failed. Error code: " +
            std::to_string(GetLastError()));
        GlobalFree(hg);
        return;
    }
    ...
    if (!SetClipboardData(CF_UNICODETEXT, hg)) {
        Log("ERROR: SetClipboardData failed. Error code: " +
            std::to_string(GetLastError()));
        GlobalFree(hg);
    }
}

// This function MUST be defined before the JSON serialization functions that
// call it
EyeZoomConfig GetDefaultEyeZoomConfig() {
    return GetDefaultEyeZoomConfigFromEmbedded();
}
...
void SaveConfig() {
    ...
    auto timeSinceLastSave =
        std::chrono::duration_cast<std::chrono::milliseconds>(currentTime - s_lastSaveTime)
            .count();
    ...
    try {
        // Snapshot config and convert to TOML (thread-safe for detached write
        // thread)
        toml::table tbl;
    ...
    if (s_isConfigSaving.load()) {
        ...
        while (s_isConfigSaving.load()) {
            if (std::chrono::duration_cast<std::chrono::seconds>(
                    ...
                    .count() > 3) {
                Log("SaveConfigImmediate: Timed out waiting for background save. "
                    "Proceeding anyway.");
                break;
            }
...
static void ApplyPresetThemeColors(const std::string& themeName) {
    ...
    if (themeName == "Dracula") {
        ...
        style.Colors[ImGuiCol_TitleBgCollapsed] =
            ImVec4(0.16f, 0.16f, 0.21f, 0.51f);
        ...
        style.Colors[ImGuiCol_SliderGrabActive] =
            ImVec4(0.98f, 0.47f, 0.60f, 1.00f);
        ...
        style.Colors[ImGuiCol_TitleBgCollapsed] =
            ImVec4(0.18f, 0.20f, 0.25f, 0.51f);
        ...
        style.Colors[ImGuiCol_SliderGrabActive] =
            ImVec4(0.69f, 0.76f, 0.82f, 1.00f);
        ...
        style.Colors[ImGuiCol_TitleBgCollapsed] =
            ImVec4(0.00f, 0.17f, 0.21f, 0.51f);
        ...
        style.Colors[ImGuiCol_SliderGrabActive] =
            ImVec4(0.52f, 0.60f, 0.00f, 1.00f);
        ...
        style.Colors[ImGuiCol_TitleBgCollapsed] =
            ImVec4(0.15f, 0.15f, 0.13f, 0.51f);
        ...
        style.Colors[ImGuiCol_SliderGrabActive] =
            ImVec4(0.98f, 0.89f, 0.36f, 1.00f);
        ...
        style.Colors[ImGuiCol_TitleBgCollapsed] =
            ImVec4(0.12f, 0.12f, 0.18f, 0.51f);
        ...
        style.Colors[ImGuiCol_SliderGrabActive] =
            ImVec4(0.95f, 0.55f, 0.66f, 1.00f);
        ...
        style.Colors[ImGuiCol_TitleBgCollapsed] =
            ImVec4(0.16f, 0.18f, 0.21f, 0.51f);
        ...
        style.Colors[ImGuiCol_SliderGrabActive] =
            ImVec4(0.20f, 0.80f, 0.62f, 1.00f);
        ...
        style.Colors[ImGuiCol_TitleBgCollapsed] =
            ImVec4(0.16f, 0.15f, 0.13f, 0.51f);
        ...
        style.Colors[ImGuiCol_SliderGrabActive] =
            ImVec4(0.72f, 0.73f, 0.15f, 1.00f);
        ...
        style.Colors[ImGuiCol_TitleBgCollapsed] =
            ImVec4(0.10f, 0.11f, 0.17f, 0.51f);
        ...
        style.Colors[ImGuiCol_SliderGrabActive] =
            ImVec4(0.98f, 0.55f, 0.67f, 1.00f);
...
void ApplyAppearanceConfig() {
    ...
    } else if (theme == "Dracula" || theme == "Nord" || theme == "Solarized" ||
               theme == "Monokai" || theme == "Catppuccin" ||
               theme == "One Dark" || theme == "Gruvbox" ||
               theme == "Tokyo Night" || theme == "Purple" || theme == "Pink" ||
               theme == "Blue" || theme == "Teal" || theme == "Red" ||
               theme == "Green") {
    ...
    if (theme == "Custom" && !g_config.appearance.customColors.empty()) {
        ...
        static const std::map<std::string, ImGuiCol_> colorNameToIdx = {
        ...
        for (const auto& [name, color] : g_config.appearance.customColors) {
            ...
            if (it != colorNameToIdx.end()) {
                style.Colors[it->second] = ImVec4(color.r, color.g, color.b, color.a);
            }
        }
    }
...
void SaveTheme() {
    ...
    try {
        ...
        Log("ERROR: Failed to save theme: " + std::string(e.what()));
    }
}
...
void LoadTheme() {
    ...
    try {
        ...
        if (!result) {
            ...
            Log("ERROR: Failed to parse theme.toml: " +
        ...
        Log("ERROR: Failed to load theme: " + std::string(e.what()));
    }
}

std::vector<ModeConfig> GetDefaultModes() {
    return GetDefaultModesFromEmbedded();
}

std::vector<MirrorConfig> GetDefaultMirrors() {
    return GetDefaultMirrorsFromEmbedded();
}

std::vector<MirrorGroupConfig> GetDefaultMirrorGroups() {
    return GetDefaultMirrorGroupsFromEmbedded();
}

std::vector<ImageConfig> GetDefaultImages() {
    return GetDefaultImagesFromEmbedded();
}
...
std::vector<HotkeyConfig> GetDefaultHotkeys() {
    return GetDefaultHotkeysFromEmbedded();
}
...
void WriteDefaultConfig(const std::wstring& path) {
    ...
    if (LoadEmbeddedDefaultConfig(defaultConfig)) {
        ...
        try {
            ...
            o << tbl;
            o.close();
            Log("Wrote default config.toml from embedded defaults, customized for "
                "your monitor (" +
        ...
        Log("WARNING: Could not load embedded default config, creating minimal "
            "fallback config");
        ...
        try {
            ...
            o << tbl;
            o.close();
            Log("Wrote fallback config.toml for your monitor (" +
                std::to_string(screenWidth) + "x" + std::to_string(screenHeight) +
                ").");
        } catch (const std::exception& e) {
...
void LoadConfig() {
    ...
    if (!std::filesystem::exists(configPath)) {
        ...
        if (!std::filesystem::exists(configPath)) {
            std::string errorMessage =
                "FATAL: Could not create or read default config. Aborting load.";
            Log(errorMessage);
    ...
    try {
...

        {
            ...
            if (!modeExists("Preemptive")) {
                ...
                if (preemptiveModePtr) {
                    bool changed = false;
                    if (!preemptiveModePtr->widthExpr.empty() ||
                        !preemptiveModePtr->heightExpr.empty()) {
                        preemptiveModePtr->widthExpr.clear();
                        preemptiveModePtr->heightExpr.clear();
                        changed = true;
                    }
                    if (preemptiveModePtr->relativeWidth >= 0.0f ||
                        preemptiveModePtr->relativeHeight >= 0.0f ||
        ...
        for (auto& mode : g_config.modes) {
            bool widthIsRelative = mode.widthExpr.empty() &&
                                   mode.relativeWidth >= 0.0f &&
                                   mode.relativeWidth <= 1.0f;
            bool heightIsRelative = mode.heightExpr.empty() &&
                                    mode.relativeHeight >= 0.0f &&
                                    mode.relativeHeight <= 1.0f;

            if (widthIsRelative && hasClientMetrics) {
                mode.width = static_cast<int>(std::lround(mode.relativeWidth * clientWidth));
                if (mode.width < 1) mode.width = 1;
            }
            if (heightIsRelative && hasClientMetrics) {
                mode.height = static_cast<int>(std::lround(mode.relativeHeight * clientHeight));
            ...
            if (EqualsIgnoreCase(mode.id, "Fullscreen")) {
                ...
                if (!mode.useRelativeSize || mode.relativeWidth != 1.0f ||
                    mode.relativeHeight != 1.0f) {
                ...
                if (!mode.stretch.enabled || mode.stretch.x != 0 ||
                    mode.stretch.y != 0 || mode.stretch.width != targetW ||
        ...
        for (auto& hotkey : g_config.hotkeys) {
            if (hotkey.mainMode.empty()) {
                hotkey.mainMode = g_config.defaultMode;
            }
        }
...

        {
            ...
            if (g_currentModeId.empty()) {
                ...
                int nextIndex =
                    1 - g_currentModeIdIndex.load(std::memory_order_relaxed);
                g_modeIdBuffers[nextIndex] = g_config.defaultMode;
                g_currentModeIdIndex.store(nextIndex, std::memory_order_release);
            }
        }

        Log("Config loaded: " + std::to_string(g_config.modes.size()) + " modes, " +
            std::to_string(g_config.mirrors.size()) + " mirrors, " +
            std::to_string(g_config.images.size()) + " images, " +
            std::to_string(g_config.windowOverlays.size()) + " window overlays, " +
            std::to_string(g_config.hotkeys.size()) + " hotkeys.");
        ...
        if (loadedConfigVersion < currentConfigVersion) {
            Log("Config version upgrade detected: v" +
                std::to_string(loadedConfigVersion) + " -> v" +
            ...
            Log("WARNING: Config version is newer than tool version (config: v" +
                std::to_string(loadedConfigVersion) + ", tool: v" +
                std::to_string(currentConfigVersion) + ")");
        } else {
            Log("Config version: v" + std::to_string(loadedConfigVersion) +
                " (current)");
        }
        ...
        // This is safe because we use the internal version that doesn't try to
        // reacquire the lock
        {
...

        {
            ...
            const bool hasValidStartupClient =
                GetWindowClientRectInScreen(startupHwnd, startupClientRect);
            if (hasValidStartupClient) {
                RecalculateExpressionDimensions();
            } else {
                Log("Deferring mode dimension recalculation until game client size is "
                    "valid.");
            }
        }
        ...
        std::string errorMessage =
            "Error parsing config.toml: " + std::string(e.what()) +
            "\n\nPlease fix the error in the config file or delete it to generate "
            "a new one.";
        Log(errorMessage);
        g_configLoadFailed = true;
        {
...
bool HasDuplicateModeName(const std::string& name, size_t currentIndex) {
    for (size_t i = 0; i < g_config.modes.size(); i++) {
        if (i != currentIndex && g_config.modes[i].id == name) {
            return true;
        }
    }
    return false;
...
bool HasDuplicateMirrorName(const std::string& name, size_t currentIndex) {
    for (size_t i = 0; i < g_config.mirrors.size(); i++) {
        if (i != currentIndex && g_config.mirrors[i].name == name) {
            return true;
        }
    }
    return false;
...
bool HasDuplicateMirrorGroupName(const std::string& name, size_t currentIndex) {
    for (size_t i = 0; i < g_config.mirrorGroups.size(); i++) {
        if (i != currentIndex && g_config.mirrorGroups[i].name == name) {
            return true;
        }
    }
    return false;
...
bool HasDuplicateImageName(const std::string& name, size_t currentIndex) {
    for (size_t i = 0; i < g_config.images.size(); i++) {
        if (i != currentIndex && g_config.images[i].name == name) {
            return true;
        }
    }
    return false;
...
bool HasDuplicateWindowOverlayName(const std::string& name, size_t currentIndex) {
    for (size_t i = 0; i < g_config.windowOverlays.size(); i++) {
        if (i != currentIndex && g_config.windowOverlays[i].name == name) {
            return true;
        }
    }
    return false;
...
bool Spinner(const char* id_label, int* v, int step, int min_val, int max_val, float inputWidth, float margin) {
    ...
    if (ImGui::IsItemActive()) {
        ...
        if (hold_time > spinnerHoldDelay) {
            int repeat_count =
                (int)((hold_time - spinnerHoldDelay) / spinnerHoldInterval);
...
bool SpinnerFloat(const char* id_label, float* v, float step = 0.1f, float min_val = 0.0f, float max_val = FLT_MAX, const char* format = "%.1f") {
    ImGui::PushID(id_label);
    bool  value_changed = false;
    float button_size   = ImGui::GetFrameHeight();
    ...
    if (ImGui::InputFloat("##value", v, 0.0f, 0.0f, format)) {
        value_changed = true;
    }
    ImGui::SameLine();
...
void RenderConfigErrorGUI() {
    ...
    if (ImGui::Begin(trc("error.configuration_error"), NULL, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoMove)) {
        ...
        ImGui::TextWrapped("%s", trc("error.configuration_error.detail.before_reason"));
        ImGui::Separator();
        ImGui::TextWrapped("%s", errorMsg.c_str());
        ImGui::Separator();
        ImGui::TextWrapped("%s", trc("error.configuration_error.detail.after_reason"));
        ImGui::Separator();

        bool show_feedback = std::chrono::duration_cast<std::chrono::seconds>(
                                 std::chrono::steady_clock::now() - s_lastCopyTime
                             )
                                 .count() < 3;

        float button_width_copy = ImGui::CalcTextSize(trc("button.copy_debug_info")).x +
                                  ImGui::GetStyle().FramePadding.x * 2.0f;
        float button_width_quit =
            ImGui::CalcTextSize(trc("button.quit")).x + ImGui::GetStyle().FramePadding.x * 2.0f;
        float total_button_width =
            button_width_copy + button_width_quit + ImGui::GetStyle().ItemSpacing.x;
        ...
        ImGui::SameLine();
        if (ImGui::Button(trc("button.quit"))) {
            exit(0);
        }
...
// Binding-active flags are read from the window thread (WndProc) to decide
// whether Escape should close the GUI. The binding UI state variables above are
// mutated on the render thread; reading them cross-thread would be a data race.
// Instead, we expose thread-safe "binding active" signals as timestamps
// refreshed by the render thread while the binding UI is present. The window
// thread treats binding as active for a short grace window after the last
// refresh.
...
static inline uint64_t NowMs_TickCount64() {
    return static_cast<uint64_t>(::GetTickCount64());
}

static bool IsHotkeyBindingActive_UiState() {
    return s_mainHotkeyToBind != -1 || s_sensHotkeyToBind != -1 ||
           s_exclusionToBind.hotkey_idx != -1 ||
           s_altHotkeyToBind.hotkey_idx != -1;
}

bool IsHotkeyBindingActive() {
    const uint64_t last =
        s_lastHotkeyBindingMarkMs.load(std::memory_order_acquire);
...
bool IsRebindBindingActive() {
    const uint64_t last =
        s_lastRebindBindingMarkMs.load(std::memory_order_acquire);
    if (last == 0) return false;
    return (NowMs_TickCount64() - last) <= kBindingActiveGraceMs;
}
...
void RenderSettingsGUI() {
    ...
    static const std::vector<const char*> validGameStates = {
        "wall", "inworld,cursor_free", "inworld,cursor_grabbed", "title",
        "waiting", "generating"
    };

    static const std::vector<const char*> guiGameStates = {
        "wall", "inworld,cursor_free", "inworld,cursor_grabbed", "title",
        "generating"
    };
    ...
    if (ImGui::BeginPopupModal(
            trc("hotkeys.bind_hotkey"), NULL,
            ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoTitleBar
        )) {
        ImGui::Text("%s", trc("hotkeys.bind_hotkey.tooltip.prompt"));
        ImGui::Text("%s", trc("hotkeys.bind_hotkey.tooltip.confirm"));
        ImGui::Text("%s", trc("hotkeys.bind_hotkey.tooltip.clear"));
        ImGui::Text("%s", trc("hotkeys.bind_hotkey.tooltip.cancel"));
        ImGui::Separator();

        static uint64_t s_lastBindingInputSeqHotkeyBind = 0;
        if (ImGui::IsWindowAppearing()) {
            s_lastBindingInputSeqHotkeyBind = GetLatestBindingInputSequence();
        }
        ...
        auto finalize_bind = [&](const std::vector<DWORD>& keys) {
            if (s_mainHotkeyToBind != -1) {
                ...
                g_config.hotkeys[s_altHotkeyToBind.hotkey_idx]
                    .altSecondaryModes[s_altHotkeyToBind.alt_idx]
                    .keys         = keys;
                s_altHotkeyToBind = {-1, -1};
            } else if (s_exclusionToBind.hotkey_idx != -1) {
                if (!keys.empty()) {
                    g_config.hotkeys[s_exclusionToBind.hotkey_idx]
                        .conditions.exclusions[s_exclusionToBind.exclusion_idx] =
                        keys.back();
                }
        ...
        const bool ctrlPreHeld = s_preHeldKeys.count(VK_LCONTROL) ||
                                 s_preHeldKeys.count(VK_RCONTROL) ||
                                 s_preHeldKeys.count(VK_CONTROL);
        const bool shiftPreHeld = s_preHeldKeys.count(VK_LSHIFT) ||
                                  s_preHeldKeys.count(VK_RSHIFT) ||
                                  s_preHeldKeys.count(VK_SHIFT);
        const bool altPreHeld = s_preHeldKeys.count(VK_LMENU) ||
                                s_preHeldKeys.count(VK_RMENU) ||
                                s_preHeldKeys.count(VK_MENU);

        if ((lctrlDown || rctrlDown) && !ctrlPreHeld)
            currentlyPressed.push_back(VK_CONTROL);
        if ((lshiftDown || rshiftDown) && !shiftPreHeld)
            currentlyPressed.push_back(VK_SHIFT);
        if ((laltDown || raltDown) && !altPreHeld)
            currentlyPressed.push_back(VK_MENU);

        for (int vk = 1; vk < 0xFF; ++vk) {
            // Skip escape (used for cancel), generic modifiers, and Windows keys
            if (vk == VK_ESCAPE || vk == VK_CONTROL || vk == VK_SHIFT ||
            ...
            if (GetAsyncKeyState(vk) & 0x8000) {
                currentlyPressed.push_back(vk);
            }
        }

        for (DWORD key : currentlyPressed) {
            if (std::find(s_bindingKeys.begin(), s_bindingKeys.end(), key) ==
                s_bindingKeys.end()) {
                bool isModifier =
                    (key == VK_CONTROL || key == VK_SHIFT || key == VK_MENU ||
                     key == VK_LCONTROL || key == VK_RCONTROL || key == VK_LSHIFT ||
                     key == VK_RSHIFT || key == VK_LMENU || key == VK_RMENU);
                if (isModifier) {
                    auto insertPos = s_bindingKeys.begin();
                    for (auto it = s_bindingKeys.begin(); it != s_bindingKeys.end();
                         ++it) {
                        bool itIsModifier =
                            (*it == VK_CONTROL || *it == VK_SHIFT || *it == VK_MENU ||
                             *it == VK_LCONTROL || *it == VK_RCONTROL || *it == VK_LSHIFT ||
                             *it == VK_RSHIFT || *it == VK_LMENU || *it == VK_RMENU);
                        if (!itIsModifier) {
                            insertPos = it;
                            break;
                        }
        ...
        std::string combo = tr("hotkeys.none");
        if (!s_bindingKeys.empty()) {
            combo = GetKeyComboString(s_bindingKeys);
        }
        ImGui::Text("%s", ftrc("hotkeys.bind_hotkey.current", combo));
    ...
    if (windowHeight > 1080) {
        scaleFactor = static_cast<float>(windowHeight) / 1080.0f;
    }
    ...
    std::string windowTitle = ftr("window.title", GetToolscreenVersionString());
    ...
    if (ImGui::Begin(windowTitle.c_str(), &windowOpen, ImGuiWindowFlags_NoCollapse)) {
...

        {
            ...
            auto                                         now        = std::chrono::steady_clock::now();
            bool                                         showCopied = std::chrono::duration_cast<std::chrono::seconds>(
                                  now - s_lastScreenshotTime
                              )
                                  .count() < 3;

            const char* buttonLabel = showCopied ? trc("button.screenshot.copied") : trc("button.screenshot");
            float       buttonWidth = ImGui::CalcTextSize(buttonLabel).x +
                                ImGui::GetStyle().FramePadding.x * 2.0f;
...
void InitializeImGuiContext(HWND hwnd) {
    if (ImGui::GetCurrentContext() == nullptr) {
        ...
        std::string usePath =
            fontPath.empty() ? ConfigDefaults::CONFIG_FONT_PATH : fontPath;
        if (!isStable(usePath, baseFontSize)) {
            usePath = ConfigDefaults::CONFIG_FONT_PATH;
        }

        ImFont* baseFont =
            io.Fonts->AddFontFromFileTTF(usePath.c_str(), baseFontSize);
        if (!baseFont && usePath != ConfigDefaults::CONFIG_FONT_PATH) {
            baseFont = io.Fonts->AddFontFromFileTTF(
                ConfigDefaults::CONFIG_FONT_PATH.c_str(), baseFontSize
            );
        }
        if (!baseFont) {
        ...
        auto addKeyboardFontWithFallback = [&](float preferredSize) -> ImFont* {
            ...
            for (float s : kSteps) {
                ...
                ImFont*     f =
                    io.Fonts->AddFontFromFileTTF(usePath.c_str(), sz, &keyFontCfg);
                if (!f && usePath != ConfigDefaults::CONFIG_FONT_PATH) {
                    f = io.Fonts->AddFontFromFileTTF(
                        ConfigDefaults::CONFIG_FONT_PATH.c_str(), sz, &keyFontCfg
                    );
                }
                if (f) return f;
            }
        ...
        g_keyboardLayoutPrimaryFont =
            addKeyboardFontWithFallback(keyboardPrimarySize);
        g_keyboardLayoutSecondaryFont =
            addKeyboardFontWithFallback(keyboardSecondarySize);

        if (!g_keyboardLayoutPrimaryFont) g_keyboardLayoutPrimaryFont = baseFont;
        if (!g_keyboardLayoutSecondaryFont)
            g_keyboardLayoutSecondaryFont = baseFont;
...
bool IsGuiHotkeyPressed(WPARAM wParam) {
    return CheckHotkeyMatch(g_config.guiHotkey, wParam);
}
...
void RenderWelcomeToast(bool isFullscreen) {
    ...
    if (isFullscreen) {
        if (s_toast2FinishedThisFullscreen) {
            return;
        }
...
void main() {
    ...
    auto ensureToastTexture = [&](const wchar_t* resourceName, GLuint& outTexture, int& outW,
                                  int& outH) {
        ...
        HRSRC hResource = FindResourceW(hModule, resourceName, RT_RCDATA);
        if (!hResource) {
            return;
        }
        ...
        const auto* rawData  = (const unsigned char*)LockResource(hData);
        ...
        unsigned char* pixels =
            stbi_load_from_memory(rawData, (int)dataSize, &w, &h, &channels, 4);
        if (!pixels || w <= 0 || h <= 0) {
            return;
        }
    ...
    ensureToastTexture(L"bitmap_toast1", s_toast1Texture, s_toast1Width, s_toast1Height);
    ensureToastTexture(L"bitmap_toast2", s_toast2Texture, s_toast2Width, s_toast2Height);
    ...
    if (s_program == 0 || s_vao == 0 || s_vbo == 0 || texture == 0 || imgW <= 0 ||
        imgH <= 0) {
        return;
    }
    ...
    GLint savedProgram = 0, savedVAO = 0, savedVBO = 0, savedFBO = 0,
          savedTex = 0, savedActiveTex = 0;
    GLboolean savedBlend = GL_FALSE, savedDepthTest = GL_FALSE,
              savedScissor = GL_FALSE, savedStencil = GL_FALSE;
    GLint savedBlendSrcRGB = 0, savedBlendDstRGB = 0, savedBlendSrcA = 0,
          savedBlendDstA = 0;
    GLint     savedViewport[4];
    GLboolean savedColorMask[4];
    GLint     savedUnpackRowLength = 0, savedUnpackSkipPixels = 0,
          savedUnpackSkipRows = 0, savedUnpackAlignment = 0;
    ...
    // The render thread draws overlays into an offscreen FBO and then blits it;
    // binding 0 would
...
void RenderPerformanceOverlay(bool showPerformanceOverlay) {
    ...
    auto timeSinceLastUpdate =
        std::chrono::duration_cast<std::chrono::milliseconds>(currentTime - lastOverlayUpdate);
    ...
    ImGui::Begin("DebugOverlay", nullptr, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoNav | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoInputs | ImGuiWindowFlags_AlwaysAutoResize);
...
void RenderProfilerOverlay(bool showProfiler, bool showPerformanceOverlay) {
    ...
    ImGui::SetNextWindowBgAlpha(0.35f);
    ImGui::Begin("ProfilerOverlay", nullptr, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoNav | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoInputs | ImGuiWindowFlags_AlwaysAutoResize);
    ...
    auto renderTreeSection =
        [](const char* sectionTitle,
           const std::vector<std::pair<std::string, Profiler::ProfileEntry>>&
                  entries,
           ImVec4 headerColor) {
...
void HandleConfigLoadFailed(HDC hDc, BOOL (*oWglSwapBuffers)(HDC)) {
...

    {
        ...
        GLint     last_blend_src_rgb, last_blend_dst_rgb, last_blend_src_alpha,
