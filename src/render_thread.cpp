...
// Using the game's HDC on a different thread is undefined on some drivers and
// can cause intermittent SEH/AVs and black mirrors. Prefer a dedicated dummy
// window/DC.
static HWND g_renderFallbackDummyHwnd = NULL;
static HDC  g_renderFallbackDummyDC   = NULL;
static HWND g_renderOwnedDCHwnd =
    NULL;  // Non-null when we called GetDC(hwnd) for g_renderThreadDC

static bool RT_CreateFallbackDummyWindowWithMatchingPixelFormat(
    HDC gameHdc, const wchar_t* windowNameTag, HWND& outHwnd, HDC& outDc
) {
    if (outHwnd && outDc) {
        return true;
    }
    ...
    outHwnd = CreateWindowExW(0, L"ToolscreenRenderThreadDummy", wndName.c_str(), WS_OVERLAPPED, 0, 0, 1, 1, NULL, NULL, GetModuleHandleW(NULL), NULL);
    if (!outHwnd) {
        return false;
    }
...
struct RenderFBO {
    ...
    GLsync            gpuFence =
        nullptr;  // Fence to ensure rendering is complete before reading
};

static RenderFBO        g_renderFBOs[RENDER_THREAD_FBO_COUNT];
static std::atomic<int> g_writeFBOIndex{
    0
};  // FBO currently being written by render thread
static std::atomic<int> g_readFBOIndex{
    -1
};  // FBO ready for reading by main thread (-1 = none ready)

// Consumer fences (main thread -> render thread):
// One fence per FBO index, created by the main thread after it finishes
// sampling that FBO's texture. The render thread waits on (and deletes) the
// fence before reusing the FBO as a render target.
...
static void RT_WaitForConsumerFence(bool isObsRequest, int writeIdx) {
    ...
    std::atomic<GLsync>* fenceArray =
        isObsRequest ? g_obsFBOConsumerFences : g_renderFBOConsumerFences;
    GLsync consumer =
        fenceArray[writeIdx].exchange(nullptr, std::memory_order_acq_rel);
    if (consumer) {
        if (glIsSync(consumer)) {
            glWaitSync(consumer, 0, GL_TIMEOUT_IGNORED);
        }
        if (glIsSync(consumer)) {
...
// Last known good texture - updated only after GPU fence confirms rendering
// complete
static std::atomic<GLuint> g_lastGoodTexture{0};
static std::atomic<GLuint> g_lastGoodObsTexture{0};

// Fence for the last good texture - main thread can wait on this for
// synchronization This is more efficient than glFinish() as it only waits for
// the render thread's commands
...
// Double-buffered request queue: main thread writes to one slot, render thread
// reads from other This allows lock-free submission - main thread never blocks
// waiting for render thread
static FrameRenderRequest g_requestSlots[2];
static std::atomic<int>   g_requestWriteSlot{
    0
};  // Slot main thread writes to next
// the producer can lap and overwrite the slot the render thread is currently
// copying from (e.g. at very high FPS), which is a C++ data race and can
// manifest as a 1-frame "missing overlay".
static std::atomic<int> g_requestReadSlot{
    -1
};  // Slot currently being copied by render thread (-1 = none)
// Producer stores with release AFTER fully writing the struct. Consumer
// exchanges with acq_rel
...
static std::atomic<int>   g_obsReadSlot{
    -1
};  // Slot currently being copied by render thread (-1 = none)
...
static bool RT_IsFontStable(const std::string& fontPath, float sizePixels) {
    ...
    // can crash in release builds. Therefore, RT_IsFontStable must NOT call
    // AddFontFromFileTTF.
...
static ImFont* RT_SafeAddFontFromFileTTF(ImFontAtlas* atlas, const char* path, float sizePixels, const ImFontConfig* fontCfg = nullptr, const ImWchar* glyphRanges = nullptr) {
...
static ImFont* RT_AddFontWithArialFallback(ImFontAtlas* atlas, const std::string& requestedPath, float sizePixels, const char* what, std::string* outUsedPath = nullptr) {
    ...
    if (RT_IsFontStable(arial, sizePixels)) {
        ...
        if (ImFont* f =
                RT_SafeAddFontFromFileTTF(atlas, arial.c_str(), sizePixels)) {
            setUsed(arial);
            return f;
        }
    }

    Log(std::string("Render Thread: Failed to load ") + what +
...
static bool RT_TryInitializeImGui(HWND hwnd, const Config& cfg) {
    ...
    if (screenHeight > 1080) {
        scaleFactor = static_cast<float>(screenHeight) / 1080.0f;
    }
    ...
    {
        std::string eyeZoomFontPath = cfg.eyezoom.textFontPath.empty()
                                          ? cfg.fontPath
                                          : cfg.eyezoom.textFontPath;
        g_eyeZoomTextFont           = RT_AddFontWithArialFallback(
            io.Fonts, eyeZoomFontPath, 80.0f * scaleFactor, "EyeZoom font",
            &g_eyeZoomFontPathCached
        );
        if (g_eyeZoomFontPathCached.empty()) {
            g_eyeZoomFontPathCached = ConfigDefaults::CONFIG_FONT_PATH;
        }
    }
    ...
    if (!io.Fonts->Build()) {
        Log("Render Thread: Initial font atlas build failed; falling back to ImGui "
            "default font");
        io.Fonts->Clear();
...
// These shaders are created on the render thread context (not shared with main
// thread)
...
// NOTE: Border rendering shaders (brute force and JFA) have been removed from
// render_thread. All border rendering is now done by mirror_thread.cpp which
// has its own local shader programs. Render thread just blits the pre-rendered
// finalTexture using the passthrough/background shader.
...
static bool RT_InitializeShaders() {
    ...
    // NOTE: Border rendering shaders have been removed - all border rendering is
    // done by mirror_thread Render thread only needs: background (for mirror
    // blitting), solid color (for game borders), image render, static border, and
    // gradient
    rt_backgroundProgram = RT_CreateShaderProgram(rt_passthrough_vert_shader, rt_background_frag_shader);
    rt_solidColorProgram =
        RT_CreateShaderProgram(rt_solid_vert_shader, rt_solid_color_frag_shader);
    ...
    if (!rt_backgroundProgram || !rt_solidColorProgram ||
        !rt_imageRenderProgram || !rt_staticBorderProgram ||
        !rt_gradientProgram) {
    ...
    if (GLEW_ARB_compute_shader && GLEW_ARB_shader_image_load_store) {
        ...
        if (g_vcComputeProgram) {
            g_vcUseCompute = true;
            g_vcLocRgbaTexture =
                glGetUniformLocation(g_vcComputeProgram, "u_rgbaTexture");
            ...
            LogCategory("init",
                        "RenderThread: NV12 compute shader compiled successfully "
                        "(Rec. 709, image2D path)");
        } else {
            Log("RenderThread: NV12 compute shader failed, falling back to CPU "
                "conversion");
            g_vcUseCompute = false;
        }
    } else {
        Log("RenderThread: Compute shaders not supported, using CPU NV12 "
            "conversion");
        g_vcUseCompute = false;
    }

    rt_backgroundShaderLocs.backgroundTexture =
        glGetUniformLocation(rt_backgroundProgram, "backgroundTexture");
    rt_backgroundShaderLocs.opacity =
        glGetUniformLocation(rt_backgroundProgram, "u_opacity");

    rt_solidColorShaderLocs.color =
        glGetUniformLocation(rt_solidColorProgram, "u_color");

    rt_staticBorderShaderLocs.shape =
        glGetUniformLocation(rt_staticBorderProgram, "u_shape");
    rt_staticBorderShaderLocs.borderColor =
        glGetUniformLocation(rt_staticBorderProgram, "u_borderColor");
    rt_staticBorderShaderLocs.thickness =
        glGetUniformLocation(rt_staticBorderProgram, "u_thickness");
    rt_staticBorderShaderLocs.radius =
        glGetUniformLocation(rt_staticBorderProgram, "u_radius");
    rt_staticBorderShaderLocs.size =
        glGetUniformLocation(rt_staticBorderProgram, "u_size");
    rt_staticBorderShaderLocs.quadSize =
        glGetUniformLocation(rt_staticBorderProgram, "u_quadSize");

    rt_imageRenderShaderLocs.imageTexture =
        glGetUniformLocation(rt_imageRenderProgram, "imageTexture");
    rt_imageRenderShaderLocs.enableColorKey =
        glGetUniformLocation(rt_imageRenderProgram, "u_enableColorKey");
    rt_imageRenderShaderLocs.colorKey =
        glGetUniformLocation(rt_imageRenderProgram, "u_colorKey");
    rt_imageRenderShaderLocs.sensitivity =
        glGetUniformLocation(rt_imageRenderProgram, "u_sensitivity");
    rt_imageRenderShaderLocs.opacity =
        glGetUniformLocation(rt_imageRenderProgram, "u_opacity");

    rt_gradientShaderLocs.numStops =
        glGetUniformLocation(rt_gradientProgram, "u_numStops");
    rt_gradientShaderLocs.stopColors =
        glGetUniformLocation(rt_gradientProgram, "u_stopColors");
    rt_gradientShaderLocs.stopPositions =
        glGetUniformLocation(rt_gradientProgram, "u_stopPositions");
    rt_gradientShaderLocs.angle =
        glGetUniformLocation(rt_gradientProgram, "u_angle");
    rt_gradientShaderLocs.time =
        glGetUniformLocation(rt_gradientProgram, "u_time");
    rt_gradientShaderLocs.animationType =
        glGetUniformLocation(rt_gradientProgram, "u_animationType");
    rt_gradientShaderLocs.animationSpeed =
        glGetUniformLocation(rt_gradientProgram, "u_animationSpeed");
    rt_gradientShaderLocs.colorFade =
        glGetUniformLocation(rt_gradientProgram, "u_colorFade");
...
static void RT_RenderCursorForObs(int fullW, int fullH, int viewportX, int viewportY, int viewportW, int viewportH, int windowW, int windowH, GLuint vao, GLuint vbo) {
    if (!IsCursorVisible()) {
        return;
    }
    ...
    const CursorTextures::CursorData* cursorData =
        CursorTextures::LoadOrFindCursorFromHandle(cursorInfo.hCursor);
    if (!cursorData || cursorData->texture == 0) {
        return;
    }
    ...
    if (windowW > 0 && windowH > 0) {
        if (cursorPos.x < 0 || cursorPos.x >= windowW || cursorPos.y < 0 ||
            cursorPos.y >= windowH) {
            return;
        }
    }

    float scaleX = (viewportW > 0 && windowW > 0)
                       ? static_cast<float>(viewportW) / windowW
                       : 1.0f;
    float scaleY = (viewportH > 0 && windowH > 0)
                       ? static_cast<float>(viewportH) / windowH
                       : 1.0f;

    int renderX = viewportX +
                  static_cast<int>((cursorPos.x - cursorData->hotspotX) * scaleX);
    int renderY = viewportY +
                  static_cast<int>((cursorPos.y - cursorData->hotspotY) * scaleY);
    ...
    if (renderX + renderW < 0 || renderX >= fullW || renderY + renderH < 0 ||
        renderY >= fullH) {
        return;
    }
    ...
    float cursorQuad[] = {left, bottom, 0.0f, 1.0f, right, bottom, 1.0f, 1.0f, right, top, 1.0f, 0.0f, left, bottom, 0.0f, 1.0f, right, top, 1.0f, 0.0f, left, top, 0.0f, 0.0f};
...
// This mirrors RenderGameBorder() from render.cpp but uses render thread
// resources
static void RT_RenderGameBorder(int x, int y, int w, int h, int borderWidth, int radius, const Color& color, int fullW, int fullH, GLuint vao, GLuint vbo) {
    ...
    auto toNdcX = [fullW](int px) {
        return (static_cast<float>(px) / fullW) * 2.0f - 1.0f;
    };
    auto toNdcY = [fullH](int py) {
        return (static_cast<float>(py) / fullH) * 2.0f - 1.0f;
    };

    float topBorder[] = {toNdcX(outerLeft), toNdcY(y_gl + h), 0, 0, toNdcX(outerRight), toNdcY(y_gl + h), 0, 0, toNdcX(outerRight), toNdcY(outerTop), 0, 0, toNdcX(outerLeft), toNdcY(y_gl + h), 0, 0, toNdcX(outerRight), toNdcY(outerTop), 0, 0, toNdcX(outerLeft), toNdcY(outerTop), 0, 0};
    glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(topBorder), topBorder);
    glDrawArrays(GL_TRIANGLES, 0, 6);

    float bottomBorder[] = {toNdcX(outerLeft), toNdcY(outerBottom), 0, 0, toNdcX(outerRight), toNdcY(outerBottom), 0, 0, toNdcX(outerRight), toNdcY(y_gl), 0, 0, toNdcX(outerLeft), toNdcY(outerBottom), 0, 0, toNdcX(outerRight), toNdcY(y_gl), 0, 0, toNdcX(outerLeft), toNdcY(y_gl), 0, 0};
    glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(bottomBorder), bottomBorder);
    glDrawArrays(GL_TRIANGLES, 0, 6);

    float leftBorder[] = {toNdcX(outerLeft), toNdcY(y_gl), 0, 0, toNdcX(x), toNdcY(y_gl), 0, 0, toNdcX(x), toNdcY(y_gl + h), 0, 0, toNdcX(outerLeft), toNdcY(y_gl), 0, 0, toNdcX(x), toNdcY(y_gl + h), 0, 0, toNdcX(outerLeft), toNdcY(y_gl + h), 0, 0};
    glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(leftBorder), leftBorder);
    glDrawArrays(GL_TRIANGLES, 0, 6);

    float rightBorder[] = {toNdcX(x + w), toNdcY(y_gl), 0, 0, toNdcX(outerRight), toNdcY(y_gl), 0, 0, toNdcX(outerRight), toNdcY(y_gl + h), 0, 0, toNdcX(x + w), toNdcY(y_gl), 0, 0, toNdcX(outerRight), toNdcY(y_gl + h), 0, 0, toNdcX(x + w), toNdcY(y_gl + h), 0, 0};
    glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(rightBorder), rightBorder);
    glDrawArrays(GL_TRIANGLES, 0, 6);
}

static void RT_RenderBackground(bool isImage, GLuint bgTexture, float bgR, float bgG, float bgB, float opacity, int viewportX, int viewportY, int viewportW, int viewportH, int letterboxExtendX, int letterboxExtendY, int fullW, int fullH, GLuint vao, GLuint vbo) {
    if (viewportX == 0 && viewportY == 0 && viewportW == fullW &&
        viewportH == fullH)
        return;
    ...
    float vpNx1 =
        (static_cast<float>(viewportX + letterboxExtendX) / fullW) * 2.0f - 1.0f;
    float vpNx2 =
        (static_cast<float>(viewportX + viewportW - letterboxExtendX) / fullW) *
            2.0f -
        1.0f;
    float vpNy1 =
        (static_cast<float>(viewportY_gl + letterboxExtendY) / fullH) * 2.0f -
        1.0f;
    float vpNy2 =
        (static_cast<float>(viewportY_gl + viewportH - letterboxExtendY) /
         fullH) *
            2.0f -
        1.0f;

    float stencilQuad[] = {vpNx1, vpNy1, 0, 0, vpNx2, vpNy1, 0, 0, vpNx2, vpNy2, 0, 0, vpNx1, vpNy1, 0, 0, vpNx2, vpNy2, 0, 0, vpNx1, vpNy2, 0, 0};
    ...
    float fullscreenQuad[] = {-1.0f, -1.0f, 0.0f, 0.0f, 1.0f, -1.0f, 1.0f, 0.0f, 1.0f, 1.0f, 1.0f, 1.0f, -1.0f, -1.0f, 0.0f, 0.0f, 1.0f, 1.0f, 1.0f, 1.0f, -1.0f, 1.0f, 0.0f, 1.0f};
...
static void InitRenderFBOs(int width, int height) {
    ...
    for (int i = 0; i < RENDER_THREAD_FBO_COUNT; i++) {
        ...
        if (fbo.stencilRbo == 0) {
            glGenRenderbuffers(1, &fbo.stencilRbo);
        }
        ...
        if (fbo.width != width || fbo.height != height) {
            ...
            // Setting these here avoids per-frame glTexParameteri churn (driver
            // overhead).
            ...
            if (status != GL_FRAMEBUFFER_COMPLETE) {
                Log("RenderThread: FBO " + std::to_string(i) +
                    " incomplete: " + std::to_string(status));
            }

            fbo.width   = width;
            fbo.height  = height;
            mainResized = true;
            LogCategory("init", "RenderThread: Initialized FBO " + std::to_string(i) + " at " + std::to_string(width) + "x" + std::to_string(height));
        }

        glBindFramebuffer(GL_FRAMEBUFFER, 0);
    }

    // (which uses the same GL name) now points to undefined content. The main
    // thread reading this would display garbage/frozen frames. Clear it so the
    // main thread
    if (mainResized) {
        ...
        // Don't delete the old fence - it's managed by the deferred deletion ring
        // buffer.
    ...
    for (int i = 0; i < RENDER_THREAD_FBO_COUNT; i++) {
        ...
        if (fbo.width != width || fbo.height != height) {
            ...
            GLenum status = glCheckFramebufferStatus(GL_FRAMEBUFFER);
            if (status != GL_FRAMEBUFFER_COMPLETE) {
                Log("RenderThread: OBS FBO " + std::to_string(i) +
                    " incomplete: " + std::to_string(status));
            }

            fbo.width  = width;
            fbo.height = height;
            LogCategory("init", "RenderThread: Initialized OBS FBO " + std::to_string(i) + " at " + std::to_string(width) + "x" + std::to_string(height));
        }
...
static void StartVirtualCameraComputeReadback(GLuint srcTexture, int texW, int texH, int outW, int outH) {
    if (g_vcComputePending && g_vcFence) {
        // Non-blocking check: if GPU isn't done yet, skip this frame's virtual
        // camera update
    ...
    // Step 6: Dispatch compute shader with image2D bindings (no atomics, no SSBO
    // clear)
...
static void StartVirtualCameraPBOReadback(GLuint obsTexture, int width, int height) {
    ...
    if (g_virtualCamPBOWidth != width || g_virtualCamPBOHeight != height ||
        g_virtualCamPBO == 0) {
        if (g_virtualCamPBO != 0) {
            glDeleteBuffers(1, &g_virtualCamPBO);
        }
        ...
        if (g_virtualCamCopyFBO == 0) {
            glGenFramebuffers(1, &g_virtualCamCopyFBO);
        }
    }
...
static void RT_RenderGameTexture(GLuint gameTexture, int x, int y, int w, int h, int fullW, int fullH, int srcGameW, int srcGameH, int texW, int texH, GLuint vao, GLuint vbo) {
    ...
    float verts[] = {nx1, ny1, 0.0f, 0.0f, nx2, ny1, u_max, 0.0f, nx2, ny2, u_max, v_max, nx1, ny1, 0.0f, 0.0f, nx2, ny2, u_max, v_max, nx1, ny2, 0.0f, v_max};
...
static void RT_RenderEyeZoom(
    GLuint gameTexture, int requestViewportX, int fullW, int fullH,
    int gameTexW, int gameTexH, GLuint vao, GLuint vbo,
    bool isTransitioningFromEyeZoom = false, GLuint snapshotTexture = 0,
    int snapshotWidth = 0, int snapshotHeight = 0,
    const EyeZoomConfig* externalZoomConfig = nullptr
) {
    if (gameTexture == UINT_MAX) return;

    // Using the caller's snapshot avoids a TOCTOU race where the config changes
    // between
    ...
    bool isTransitioningToEyeZoom =
        (viewportX < targetViewportX && !isTransitioningFromEyeZoom);

    int stableZoomOutputWidth =
        targetViewportX - (2 * zoomConfig.horizontalMargin);
    if (stableZoomOutputWidth <= 1) return;

    int zoomOutputWidth = zoomConfig.slideZoomIn
                              ? stableZoomOutputWidth
                              : (viewportX - (2 * zoomConfig.horizontalMargin));

    if (zoomOutputWidth <= 1) {
        return;
    }

    int finalZoomX = zoomConfig.useCustomPosition ? zoomConfig.positionX
                                                  : zoomConfig.horizontalMargin;
    int zoomX      = finalZoomX;

    if (zoomConfig.slideZoomIn) {
        int offScreenX = -zoomOutputWidth;

        if ((isTransitioningToEyeZoom || isTransitioningFromEyeZoom) &&
            targetViewportX > 0) {
    ...
    int zoomY = zoomConfig.useCustomPosition ? zoomConfig.positionY
                                             : zoomConfig.verticalMargin;

    if (zoomConfig.useCustomPosition) {
        int maxZoomX = (std::max)(0, fullW - zoomOutputWidth);
        int maxZoomY = (std::max)(0, fullH - zoomOutputHeight);

        bool isSlidingNow = zoomConfig.slideZoomIn && (isTransitioningToEyeZoom ||
                                                       isTransitioningFromEyeZoom);
        if (!isSlidingNow) {
    ...
    auto EnsureRtEyeZoomSnapshotAllocated = [&]() {
        if (rt_eyeZoomSnapshotTexture == 0 ||
            rt_eyeZoomSnapshotWidth != zoomOutputWidth ||
            rt_eyeZoomSnapshotHeight != zoomOutputHeight) {
            if (rt_eyeZoomSnapshotTexture != 0) {
                glDeleteTextures(1, &rt_eyeZoomSnapshotTexture);
            }
            if (rt_eyeZoomSnapshotFBO != 0) {
                glDeleteFramebuffers(1, &rt_eyeZoomSnapshotFBO);
            }
    ...
    auto BlitRtEyeZoomSnapshotToDest = [&]() {
        ...
        if (snapshotReadFBO == 0) {
            glGenFramebuffers(1, &snapshotReadFBO);
        }
        ...
        glBlitFramebuffer(0, 0, rt_eyeZoomSnapshotWidth, rt_eyeZoomSnapshotHeight, zoomX, zoomY_gl, zoomX + zoomOutputWidth, zoomY_gl + zoomOutputHeight, GL_COLOR_BUFFER_BIT, GL_NEAREST);
    };
    ...
    if (isTransitioningFromEyeZoom && rt_eyeZoomSnapshotValid &&
        rt_eyeZoomSnapshotTexture != 0) {
        ...
        if (gameReadFBO == 0) {
            glGenFramebuffers(1, &gameReadFBO);
        }
    ...
    if (overlayLabelsPerSide > labelsPerSide)
        overlayLabelsPerSide = labelsPerSide;
    float centerY = zoomY_gl + zoomOutputHeight / 2.0f;

    float boxHeight = zoomConfig.linkRectToFont ? (zoomConfig.textFontSize * 1.2f)
                                                : (float)zoomConfig.rectHeight;

    for (int xOffset = -overlayLabelsPerSide; xOffset <= overlayLabelsPerSide;
         xOffset++) {
        ...
        Color boxColor =
            (boxIndex % 2 == 0) ? zoomConfig.gridColor1 : zoomConfig.gridColor2;
        float boxOpacity = (boxIndex % 2 == 0) ? zoomConfig.gridColor1Opacity
                                               : zoomConfig.gridColor2Opacity;
        ...
        // For now, we'll skip text labels for OBS (they require cross-thread
        // coordination)
    }
    ...
    glUniform4f(rt_solidColorShaderLocs.color, zoomConfig.centerLineColor.r, zoomConfig.centerLineColor.g, zoomConfig.centerLineColor.b, zoomConfig.centerLineColorOpacity);
...
static void RT_RenderMirrors(
    const std::vector<MirrorConfig>& activeMirrors,
    const GameViewportGeometry& geo, int fullW, int fullH, float modeOpacity,
    bool excludeOnlyOnMyScreen, bool relativeStretching,
    float transitionProgress, float mirrorSlideProgress, int fromX, int fromY,
    int fromW, int fromH, int toX, int toY, int toW, int toH,
    bool isEyeZoomMode, bool isTransitioningFromEyeZoom,
    int eyeZoomAnimatedViewportX, bool skipAnimation,
    const std::string& fromModeId, bool fromSlideMirrorsIn,
    bool toSlideMirrorsIn, bool isSlideOutPass, GLuint vao, GLuint vbo
) {
    ...
    if (!fromModeId.empty() && (fromSlideMirrorsIn || toSlideMirrorsIn ||
                                slideCfg.eyezoom.slideMirrorsIn)) {
        for (const auto& mode : slideCfg.modes) {
            if (EqualsIgnoreCase(mode.id, fromModeId)) {
                for (const auto& mirrorName : mode.mirrorIds) {
                    sourceMirrorNames.insert(mirrorName);
                }
                    ...
                    for (const auto& group : slideCfg.mirrorGroups) {
                        if (group.name == groupName) {
                            for (const auto& item : group.mirrors) {
                                sourceMirrorNames.insert(item.mirrorId);
                            }
                            break;
                        }
                    }
    ...
    // PHASE 2: Wait on GPU fences OUTSIDE the lock (avoids blocking mirror
    // thread)
...

    {
        ...
        for (const auto& conf : activeMirrors) {
            ...
            // If the mirror is fully transparent, skip EVERYTHING (including fence
            // waits).
            ...
            float scaleX =
                conf.output.separateScale ? conf.output.scaleX : conf.output.scale;
            float scaleY =
                conf.output.separateScale ? conf.output.scaleY : conf.output.scale;

            // ALWAYS prefer finalTexture when available - it has borders already
            // applied by mirror_thread
            if (inst.finalTexture != 0 && inst.final_w > 0 && inst.final_h > 0) {
                ...
                // This shouldn't happen in normal operation - mirror_thread always
                // produces finalTexture
            ...
            // The fence is not deleted here; it lives until SwapMirrorBuffers
            // replaces it
            ...
            bool        cacheMatchesCurrentGeo =
                cache.isValid && !isAnimating && cache.finalX == geo.finalX &&
                cache.finalY == geo.finalY && cache.finalW == geo.finalW &&
                cache.finalH == geo.finalH && cache.screenW == fullW &&
                cache.screenH == fullH && cache.outputX == conf.output.x &&
                cache.outputY == conf.output.y &&
                cache.outputScale == conf.output.scale &&
                cache.outputSeparateScale == conf.output.separateScale &&
                cache.outputScaleX == conf.output.scaleX &&
                cache.outputScaleY == conf.output.scaleY &&
                cache.outputRelativeTo == conf.output.relativeTo;
    ...
    // This prevents priority inversion where the mirror thread can't acquire the
    // lock because we're holding it while blocking on a GPU fence.
    ...
    // Render thread just blits the pre-rendered finalTexture using passthrough
    // shader
    glUseProgram(rt_backgroundProgram);
    ...
    for (auto& renderData : mirrorsToRender) {
        ...
        if (renderData.cacheValid) {
            ...
            if (isScreenRelative) {
                ...
                float toScaleX   = (toW > 0 && geo.gameW > 0)
                                       ? static_cast<float>(toW) / geo.gameW
                                       : 1.0f;
                float toScaleY   = (toH > 0 && geo.gameH > 0)
                                       ? static_cast<float>(toH) / geo.gameH
                                       : 1.0f;
                float fromScaleX = (fromW > 0 && geo.gameW > 0)
                                       ? static_cast<float>(fromW) / geo.gameW
                                       : toScaleX;
                float fromScaleY = (fromH > 0 && geo.gameH > 0)
                                       ? static_cast<float>(fromH) / geo.gameH
                                       : toScaleY;

                int toSizeW   = relativeStretching
                                    ? static_cast<int>(renderData.outW * toScaleX)
                                    : renderData.outW;
                int toSizeH   = relativeStretching
                                    ? static_cast<int>(renderData.outH * toScaleY)
                                    : renderData.outH;
                int fromSizeW = relativeStretching
                                    ? static_cast<int>(renderData.outW * fromScaleX)
                                    : renderData.outW;
                int fromSizeH = relativeStretching
                                    ? static_cast<int>(renderData.outH * fromScaleY)
                                    : renderData.outH;
                ...
                int effectiveFromSizeH =
                    isTransitioningFromEyeZoom ? toSizeH : fromSizeH;
                GetRelativeCoords(anchor, conf.output.x, conf.output.y, fromSizeW, effectiveFromSizeH, fromW, effectiveFromH, fromOutX, fromOutY);
                ...
                if (relativeStretching) {
                    finalW_screen =
                        static_cast<int>(fromSizeW + (toSizeW - fromSizeW) * t);
                    finalH_screen =
                        static_cast<int>(fromSizeH + (toSizeH - fromSizeH) * t);
                } else {
                    finalW_screen = renderData.outW;
                    finalH_screen = renderData.outH;
                }
            ...
            // 1. EyeZoom-specific: Uses eyeZoomAnimatedViewportX for precise
            // viewport-synchronized slide
            ...
            bool hasEyeZoomAnimatedPosition =
                eyeZoomAnimatedViewportX >= 0 && targetViewportX > 0;
            bool isEyeZoomTransitioning = hasEyeZoomAnimatedPosition &&
                                          eyeZoomAnimatedViewportX < targetViewportX;

            bool isTransitioningToEyeZoom = isEyeZoomMode && isEyeZoomTransitioning &&
                                            !isTransitioningFromEyeZoom;
            bool isEyeZoomSlideOut =
                isEyeZoomMode && isTransitioningFromEyeZoom && isEyeZoomTransitioning;

            if (zoomConfig.slideMirrorsIn &&
                (isTransitioningToEyeZoom || isEyeZoomSlideOut) &&
                hasEyeZoomAnimatedPosition) {
                shouldApplySlide = true;
                slideProgress    = static_cast<float>(eyeZoomAnimatedViewportX) /
            ...
            if (shouldApplySlide && sourceMirrorNames.count(conf.name) > 0) {
                shouldApplySlide = false;
            }

            if (shouldApplySlide) {
                slideProgress = (slideProgress < 0.0f)
                                    ? 0.0f
                                    : (slideProgress > 1.0f ? 1.0f : slideProgress);
                ...
                if (isOnLeftSide) {
                    int slideX =
                        offScreenLeft +
                        static_cast<int>((finalX_screen - offScreenLeft) * slideProgress);
                    finalX_screen = slideX;
                } else {
                    int slideX = offScreenRight -
                                 static_cast<int>((offScreenRight - finalX_screen) * slideProgress);
                    finalX_screen = slideX;
                }
            ...
            float nx2 =
                (static_cast<float>(finalX_screen + finalW_screen) / fullW) * 2.0f -
                1.0f;
            float ny2 =
                (static_cast<float>(finalY_gl + finalH_screen) / fullH) * 2.0f - 1.0f;
    ...
    for (const auto& renderData : mirrorsToRender) {
        ...
        int baseW =
            (border.staticWidth > 0) ? border.staticWidth : renderData.screenW;
        int baseH =
            (border.staticHeight > 0) ? border.staticHeight : renderData.screenH;
        ...
        int quadX = renderData.screenX - centerOffsetX + border.staticOffsetX -
                    borderExtension;
        int quadY = renderData.screenY - centerOffsetY + border.staticOffsetY -
                    borderExtension;

        glUniform1i(rt_staticBorderShaderLocs.shape, static_cast<int>(border.staticShape));
        glUniform4f(rt_staticBorderShaderLocs.borderColor, border.staticColor.r, border.staticColor.g, border.staticColor.b, border.staticColor.a * conf.opacity * modeOpacity);
...
static void RT_RenderImages(const std::vector<ImageConfig>& activeImages, int fullW, int fullH, int gameX, int gameY, int gameW, int gameH, int gameResW, int gameResH, bool relativeStretching, float transitionProgress, int fromX, int fromY, int fromW, int fromH, float modeOpacity, bool excludeOnlyOnMyScreen, GLuint vao, GLuint vbo) {
    ...
    drawInputs.reserve(activeImages.size());
    {
        ...
        for (const auto& conf : activeImages) {
            ...
            if (it_inst == g_userImages.end() || it_inst->second.textureId == 0)
                continue;
    ...
    for (const auto& in : drawInputs) {
        ...
        const bool  hasBg            = conf.background.enabled &&
                           conf.background.opacity > 0.0f && !isFullyTransparent;
        const bool hasBorder =
            conf.border.enabled && conf.border.width > 0 && !isFullyTransparent;
        if (effectiveOpacity <= 0.0f && !hasBg && !hasBorder) {
            continue;
        }
        ...
        bool configChanged =
            !cache.isValid || cache.crop_left != conf.crop_left ||
            cache.crop_right != conf.crop_right ||
            cache.crop_top != conf.crop_top ||
            cache.crop_bottom != conf.crop_bottom || cache.scale != conf.scale ||
            cache.x != conf.x || cache.y != conf.y ||
            cache.relativeTo != conf.relativeTo || cache.screenWidth != fullW ||
        ...
        if (!configChanged) {
            ...
            bool isViewportRelative =
                conf.relativeTo.length() > 8 &&
                conf.relativeTo.substr(conf.relativeTo.length() - 8) == "Viewport";
            ...
            if (isViewportRelative) {
                float toScaleX   = (gameW > 0 && gameResW > 0)
                                       ? static_cast<float>(gameW) / gameResW
                                       : 1.0f;
                float toScaleY   = (gameH > 0 && gameResH > 0)
                                       ? static_cast<float>(gameH) / gameResH
                                       : 1.0f;
                float fromScaleX = (fromW > 0 && gameResW > 0)
                                       ? static_cast<float>(fromW) / gameResW
                                       : toScaleX;
                float fromScaleY = (fromH > 0 && gameResH > 0)
                                       ? static_cast<float>(fromH) / gameResH
                                       : toScaleY;

                int toDisplayW   = relativeStretching
                                       ? static_cast<int>(displayW * toScaleX)
                                       : displayW;
                int toDisplayH   = relativeStretching
                                       ? static_cast<int>(displayH * toScaleY)
                                       : displayH;
                int fromDisplayW = relativeStretching
                                       ? static_cast<int>(displayW * fromScaleX)
                                       : displayW;
                int fromDisplayH = relativeStretching
                                       ? static_cast<int>(displayH * fromScaleY)
                                       : displayH;

                int toPosX, toPosY;
                GetRelativeCoordsForImageWithViewport(
                    conf.relativeTo, conf.x, conf.y, toDisplayW, toDisplayH, gameX,
                    gameY, gameW, gameH, fullW, fullH, toPosX, toPosY
                );

                int fromPosX, fromPosY;
                GetRelativeCoordsForImageWithViewport(
                    conf.relativeTo, conf.x, conf.y, fromDisplayW, fromDisplayH, fromX,
                    fromY, fromW, fromH, fullW, fullH, fromPosX, fromPosY
                );
                ...
                if (relativeStretching) {
                    finalDisplayW =
                        static_cast<int>(fromDisplayW + (toDisplayW - fromDisplayW) * t);
                    finalDisplayH =
                        static_cast<int>(fromDisplayH + (toDisplayH - fromDisplayH) * t);
                }
            } else {
                GetRelativeCoordsForImageWithViewport(
                    conf.relativeTo, conf.x, conf.y, finalDisplayW, finalDisplayH,
                    gameX, gameY, gameW, gameH, fullW, fullH, finalScreenX_win,
                    finalScreenY_win
                );
            }
            ...
            nx2                 = (static_cast<float>(finalScreenX_win + finalDisplayW) / fullW) *
                      2.0f -
                  1.0f;
            ny2 =
                (static_cast<float>(finalScreenY_gl + finalDisplayH) / fullH) * 2.0f -
                1.0f;
        ...
        if (hasBg) {
            glUseProgram(rt_solidColorProgram);
            glUniform4f(rt_solidColorShaderLocs.color, conf.background.color.r, conf.background.color.g, conf.background.color.b, conf.background.opacity * modeOpacity);
        ...
        float verts[] = {nx1, ny1, tu1, tv1, nx2, ny1, tu2, tv1, nx2, ny2, tu2, tv2, nx1, ny1, tu1, tv1, nx2, ny2, tu2, tv2, nx1, ny2, tu1, tv2};
        glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(verts), verts);
        glDrawArrays(GL_TRIANGLES, 0, 6);
        ...
        if (hasBorder) {
            ...
            RT_RenderGameBorder(finalScreenX_win, finalScreenY_win, displayW, displayH, conf.border.width, conf.border.radius, conf.border.color, fullW, fullH, vao, vbo);
...
static void RT_RenderWindowOverlays(
    const std::vector<const WindowOverlayConfig*>& overlays, int fullW,
    int fullH, int gameX, int gameY, int gameW, int gameH, int gameResW,
    int gameResH, bool relativeStretching, float transitionProgress, int fromX,
    int fromY, int fromW, int fromH, float modeOpacity,
    bool excludeOnlyOnMyScreen, GLuint vao, GLuint vbo
) {
    ...
    for (const WindowOverlayConfig* conf : overlays) {
        ...
        const float effectiveOpacity = conf->opacity * modeOpacity;
        const bool  hasBg =
            conf->background.enabled && conf->background.opacity > 0.0f;
        ...
        if (entry.hasNewFrame.load(std::memory_order_acquire)) {
            // Swap readyBuffer with backBuffer under lock - this gives us exclusive
        ...
        if (renderData && renderData->pixelData && renderData->width > 0 &&
            ...
            if (renderData != entry.lastUploadedRenderData) {
                ...
                if (entry.glTextureWidth != renderData->width ||
                    entry.glTextureHeight != renderData->height) {
                    entry.glTextureWidth  = renderData->width;
                    entry.glTextureHeight = renderData->height;
                    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, renderData->width, renderData->height, 0, GL_RGBA, GL_UNSIGNED_BYTE, renderData->pixelData);
                } else {
                    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, renderData->width, renderData->height, GL_RGBA, GL_UNSIGNED_BYTE, renderData->pixelData);
                }
        ...
        if (isViewportRelative) {
            ...
            int toDisplayW =
                relativeStretching ? static_cast<int>(displayW * toScaleX) : displayW;
            int toDisplayH =
                relativeStretching ? static_cast<int>(displayH * toScaleY) : displayH;
            int fromDisplayW = relativeStretching
                                   ? static_cast<int>(displayW * fromScaleX)
                                   : displayW;
            int fromDisplayH = relativeStretching
            ...
            GetRelativeCoordsForImageWithViewport(
                conf->relativeTo, conf->x, conf->y, displayW, displayH, gameX, gameY,
                gameW, gameH, fullW, fullH, screenX, screenY
            );
        }
        ...
        float ny2 =
            (static_cast<float>(screenY_gl + displayH) / fullH) * 2.0f - 1.0f;
        ...
        float tu2 = static_cast<float>(entry.glTextureWidth - conf->crop_right) /
                    entry.glTextureWidth;
        float tv2 = static_cast<float>(entry.glTextureHeight - conf->crop_bottom) /
                    entry.glTextureHeight;

        float verts[] = {nx1, ny1, tu1, tv2, nx2, ny1, tu2, tv2, nx2, ny2, tu2, tv1, nx1, ny1, tu1, tv2, nx2, ny2, tu2, tv1, nx1, ny2, tu1, tv1};
        glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(verts), verts);
        glDrawArrays(GL_TRIANGLES, 0, 6);

        if (hasBorder) {
            RT_RenderGameBorder(screenX, screenY, displayW, displayH, conf->border.width, conf->border.radius, conf->border.color, fullW, fullH, vao, vbo);
        ...
        if (!focusedName.empty() && focusedName == overlayId) {
            ...
            RT_RenderGameBorder(screenX, screenY, displayW, displayH, focusedBorderWidth, focusedBorderRadius, focusedBorderColor, fullW, fullH, vao, vbo);
...
static void RT_CollectActiveElements(
    const Config& config, const std::string& modeId, bool onlyOnMyScreenPass,
    ...
    // These caches are safe because `config` is immutable for the lifetime of the
    // snapshot.
    ...
    static std::unordered_map<std::string, const MirrorGroupConfig*>
                                                               s_groupByName;
    static std::unordered_map<std::string, const ImageConfig*> s_imageByName;
    static std::unordered_map<std::string, const WindowOverlayConfig*>
        s_windowOverlayByName;
    ...
    if (s_cachedConfigPtr != &config) {
        ...
        for (const auto& m : config.mirrors) {
            s_mirrorByName[m.name] = &m;
        }

        s_groupByName.reserve(config.mirrorGroups.size());
        for (const auto& g : config.mirrorGroups) {
            s_groupByName[g.name] = &g;
        }

        s_imageByName.reserve(config.images.size());
        for (const auto& img : config.images) {
            s_imageByName[img.name] = &img;
        }

        s_windowOverlayByName.reserve(config.windowOverlays.size());
        for (const auto& o : config.windowOverlays) {
            s_windowOverlayByName[o.name] = &o;
        }
    }
        ...
        for (const auto& item : group.mirrors) {
            ...
            if (mit != s_mirrorByName.end() && mit->second) {
                ...
                if (!onlyOnMyScreenPass || mirror.onlyOnMyScreen) {
                    ...
                    groupedMirror.output.useRelativePosition =
                        group.output.useRelativePosition;
                    ...
                    if (item.widthPercent != 1.0f || item.heightPercent != 1.0f) {
                        ...
                        float baseScaleX                   = mirror.output.separateScale
                                                                 ? mirror.output.scaleX
                                                                 : mirror.output.scale;
                        float baseScaleY                   = mirror.output.separateScale
    ...
    if (g_imageOverlaysVisible.load(std::memory_order_acquire)) {
        for (const auto& imageName : mode->imageIds) {
            ...
            if (!onlyOnMyScreenPass || image.onlyOnMyScreen) {
    ...
    if (g_windowOverlaysVisible.load(std::memory_order_acquire)) {
        for (const auto& overlayId : mode->windowOverlayIds) {
            ...
            if (!onlyOnMyScreenPass || overlay.onlyOnMyScreen) {
                outWindowOverlays.push_back(it->second);
            }
        }
    }
}
...
static void RenderThreadFunc(void* gameGLContext) {
    ...
    try {
        ...
        if (!wglMakeCurrent(g_renderThreadDC, g_renderThreadContext)) {
            Log("Render Thread: Failed to make context current (error " +
        ...
        if (initCfg && initCfg->debug.virtualCameraEnabled) {
            ...
            if (StartVirtualCamera(vcW, vcH, initCfg->debug.virtualCameraFps)) {
                LogCategory(
                    "init",
                    "Render Thread: Virtual Camera initialized at " +
                        std::to_string(vcW) + "x" + std::to_string(vcH) + " @ " +
        ...
        // Initialize ImGui on render thread
        {
            ...
            if (hwnd) {
                ...
                if (!fontCfg) {
                    Log("Render Thread: Config snapshot not available for font loading, "
                        "using defaults");
                ...
                std::string eyeZoomFontPath = fontCfgRef.eyezoom.textFontPath.empty()
                                                  ? fontCfgRef.fontPath
                                                  : fontCfgRef.eyezoom.textFontPath;
                g_eyeZoomTextFont           = RT_AddFontWithArialFallback(
                    io.Fonts, eyeZoomFontPath, 80.0f * scaleFactor, "EyeZoom font",
                    &g_eyeZoomFontPathCached
                );
        ...
        while (!g_renderThreadShouldStop.load()) {
            // Wait for frame request (lock only held during wait, not during
            // processing)
...

            {
                ...
                g_requestCV.wait(lock, [] {
                    return g_requestReadySlot.load(std::memory_order_acquire) != -1 ||
                           g_obsReadySlot.load(std::memory_order_acquire) != -1 ||
            ...
            if (hasObsRequest) {
                ...
                // Build the full request on the render thread (deferred from main
                // thread)
        ...
        process_request:
            ...
            // === Image Processing (moved from main thread) ===
            {
                ...
                std::vector<DecodedImageData> imagesToProcess;
                {
                    ...
                    if (!g_decodedImagesQueue.empty()) {
                        imagesToProcess.swap(g_decodedImagesQueue);
                    }
                }
            ...
            // Ensure the main thread has finished sampling this FBO's texture before
            // we overwrite it.
            ...
            if (isObsRequest) {
                ...
                if (!request.isRawWindowedMode) {
                    ...
                    if (request.isTransitioningFromEyeZoom) {
                        bgModeId = "EyeZoom";
                    } else if (EqualsIgnoreCase(request.modeId, "Fullscreen") &&
                               !request.fromModeId.empty()) {
                    ...
                    if (mode && mode->background.selectedMode == "gradient" &&
                        mode->background.gradientStops.size() >= 2) {
                        glUseProgram(rt_gradientProgram);
                        glBindVertexArray(renderVAO);
                        glBindBuffer(GL_ARRAY_BUFFER, renderVBO);

                        int numStops = (std::min)(
                            static_cast<int>(mode->background.gradientStops.size()), 8
                        );
                        ...
                        float bgVerts[] = {-1.0f, -1.0f, 0.0f, 0.0f, 1.0f, -1.0f, 1.0f, 0.0f, 1.0f, 1.0f, 1.0f, 1.0f, -1.0f, -1.0f, 0.0f, 0.0f, 1.0f, 1.0f, 1.0f, 1.0f, -1.0f, 1.0f, 0.0f, 1.0f};
                                ...
                                if (bgInst.isAnimated && !bgInst.frameTextures.empty()) {
                                    ...
                                    auto elapsed =
                                        std::chrono::duration_cast<std::chrono::milliseconds>(
                                            now - bgInst.lastFrameTime
                                        )
                                            .count();
                                    int delay = bgInst.frameDelays.empty()
                                                    ? 100
                                                    : bgInst.frameDelays[bgInst.currentFrame];
                                    if (delay < 10) delay = 100;
                                    while (elapsed >= delay) {
                                        elapsed -= delay;
                                        bgInst.currentFrame =
                                            (bgInst.currentFrame + 1) % bgInst.frameTextures.size();
                                        delay = bgInst.frameDelays.empty()
                                                    ? 100
                                                    : bgInst.frameDelays[bgInst.currentFrame];
                                        if (delay < 10) delay = 100;
                                    }
                                    bgInst.textureId = bgInst.frameTextures[bgInst.currentFrame];
                                    bgInst.lastFrameTime =
                                        now - std::chrono::milliseconds(elapsed);
                                }
                ...
                // GetSafeReadTexture returns the texture NOT being written to (always
                // valid, no fence needed)
                ...
                if (readyTex != 0 && srcW > 0 && srcH > 0) {
                    int uvSrcW = srcW;
                    int uvSrcH = srcH;
                    if (request.isPre113Windowed && request.windowW > 0 &&
                        request.windowH > 0) {
                    ...
                    RT_RenderGameTexture(readyTex, request.animatedX, request.animatedY, request.animatedW, request.animatedH, request.fullW, request.fullH, uvSrcW, uvSrcH, srcW, srcH, renderVAO, renderVBO);

                    if (!request.isRawWindowedMode && request.transitioningToFullscreen &&
                        request.fromBorderEnabled && request.fromBorderWidth > 0) {
                        Color fromBorderColor = {request.fromBorderR, request.fromBorderG, request.fromBorderB, 1.0f};
                        RT_RenderGameBorder(
                            request.animatedX, request.animatedY, request.animatedW,
                            request.animatedH, request.fromBorderWidth,
                            request.fromBorderRadius, fromBorderColor, request.fullW,
                            request.fullH, renderVAO, renderVBO
                        );
                    } else if (!request.isRawWindowedMode && request.borderEnabled &&
                               request.borderWidth > 0) {
                        Color borderColor = {request.borderR, request.borderG, request.borderB, 1.0f};
                        RT_RenderGameBorder(request.animatedX, request.animatedY, request.animatedW, request.animatedH, request.borderWidth, request.borderRadius, borderColor, request.fullW, request.fullH, renderVAO, renderVBO);
                    }

                    if (!request.isRawWindowedMode && request.showEyeZoom) {
                        RT_RenderEyeZoom(
                            readyTex, request.eyeZoomAnimatedViewportX, request.fullW,
                            request.fullH, srcW, srcH, renderVAO, renderVBO,
                            request.isTransitioningFromEyeZoom,
                            request.eyeZoomSnapshotTexture, request.eyeZoomSnapshotWidth,
                            request.eyeZoomSnapshotHeight, &cfg.eyezoom
                        );
                    }
                }
                ...
                if (request.gameTextureFence && glIsSync(request.gameTextureFence)) {
                    glDeleteSync(request.gameTextureFence);
                }
            } else {
                // Background/border rendering is done on main thread (render.cpp), we
                // only render overlays here
                glDisable(GL_SCISSOR_TEST);
                glColorMask(GL_TRUE, GL_TRUE, GL_TRUE, GL_TRUE);
            ...
            // Collect active elements from g_config (this work moved from main
            // thread)
            ...
            static std::vector<const WindowOverlayConfig*>
            ...
            // the render thread doing a full clear + fence every frame. Treat those
            ...
            for (const auto& img : activeImages) {
                ...
                const bool couldHaveVisibleBg =
                    img.background.enabled && img.background.opacity > 0.0f;
                const bool couldHaveVisibleBorder =
                    img.border.enabled && img.border.width > 0;
                if ((request.overlayOpacity * img.opacity) > 0.0f ||
                    couldHaveVisibleBg || couldHaveVisibleBorder) {
            ...
            bool hasVisibleWindowOverlays = false;
            {
                if (!activeWindowOverlays.empty()) {
                    for (const WindowOverlayConfig* oconf : activeWindowOverlays) {
                        ...
                        const bool couldHaveVisibleBg =
                            oconf->background.enabled && oconf->background.opacity > 0.0f;
                        const bool couldHaveVisibleBorder =
                            oconf->border.enabled && oconf->border.width > 0;
                        if ((request.overlayOpacity * oconf->opacity) > 0.0f ||
                            couldHaveVisibleBg || couldHaveVisibleBorder) {
            ...
            const bool hasAnyVisibleOverlay =
                hasVisibleMirrors || hasVisibleImages || hasVisibleWindowOverlays;

            bool shouldRenderAnyImGui = request.shouldRenderGui ||
                                        request.showPerformanceOverlay ||
                                        request.showProfiler || request.showEyeZoom ||
                                        request.showTextureGrid;

            // Some systems can start the render thread before a valid HWND is
            // published, which previously meant the GUI never initialized (Ctrl+I
            // would do nothing, then ESC could crash).
            if (!g_renderThreadImGuiInitialized && shouldRenderAnyImGui) {
                HWND hwnd = g_minecraftHwnd.load();
                if (hwnd) {
                    RT_TryInitializeImGui(hwnd, cfg);
                }
            }

            if (!hasAnyVisibleOverlay && !shouldRenderAnyImGui &&
                ...
                if (isObsRequest) {
                    // Exchange fences - delete the OLDEST pending fence, not the one just
                    // swapped out
                    GLsync oldFence =
                        g_lastGoodObsFence.exchange(fence, std::memory_order_acq_rel);
                    // Deferred deletion: delete the fence from 2 cycles ago, store
                    // current old fence
                    ...
                    g_pendingDeleteObsIndex =
                        (g_pendingDeleteObsIndex + 1) % FENCE_DELETION_DELAY;
                    g_lastGoodObsTexture.store(writeFBO.texture, std::memory_order_release);
                } else {
                    // Exchange fences - delete the OLDEST pending fence, not the one just
                    // swapped out
                    GLsync oldFence =
                        g_lastGoodFence.exchange(fence, std::memory_order_acq_rel);
                    // Deferred deletion: delete the fence from 2 cycles ago, store
                    // current old fence
                    if (g_pendingDeleteFences[g_pendingDeleteIndex]) {
                        if (glIsSync(g_pendingDeleteFences[g_pendingDeleteIndex])) {
                            glDeleteSync(g_pendingDeleteFences[g_pendingDeleteIndex]);
                        }
                    }
                    g_pendingDeleteFences[g_pendingDeleteIndex] = oldFence;
                    g_pendingDeleteIndex =
                        (g_pendingDeleteIndex + 1) % FENCE_DELETION_DELAY;
            ...
            // Use ready frame texture from mirror thread for synchronized,
            ...
            if (!isObsRequest && request.showEyeZoom) {
                ...
                if (readyTex != 0 && srcW > 0 && srcH > 0) {
                    ...
                    RT_RenderEyeZoom(readyTex, request.eyeZoomAnimatedViewportX, request.fullW, request.fullH, srcW, srcH, renderVAO, renderVBO, request.isTransitioningFromEyeZoom, request.eyeZoomSnapshotTexture, request.eyeZoomSnapshotWidth, request.eyeZoomSnapshotHeight, &cfg.eyezoom);
                }
            }

            if (!request.isRawWindowedMode && !activeMirrors.empty()) {
                PROFILE_SCOPE_CAT("RT Mirror Render", "Render Thread");
                // Swap ready buffers from capture thread (done on render thread to
                // avoid main thread locks) This must happen before reading mirror
                // textures
                SwapMirrorBuffers();

                bool isEyeZoomMode = (request.modeId == "EyeZoom");

                RT_RenderMirrors(activeMirrors, geo, request.fullW, request.fullH, request.overlayOpacity, excludeOoms, request.relativeStretching, request.transitionProgress, request.mirrorSlideProgress, request.fromX, request.fromY, request.fromW, request.fromH, request.toX, request.toY, request.toW, request.toH, isEyeZoomMode, request.isTransitioningFromEyeZoom, request.eyeZoomAnimatedViewportX, request.skipAnimation, request.fromModeId, request.fromSlideMirrorsIn, request.toSlideMirrorsIn, false /* isSlideOutPass */, renderVAO, renderVBO);
            }

            if (!request.isRawWindowedMode && request.isTransitioningFromEyeZoom &&
                cfg.eyezoom.slideMirrorsIn && !request.skipAnimation) {
                ...
                for (const auto& ezMirror : eyeZoomMirrors) {
                    ...
                    if (!existsInTarget) {
                        mirrorsToSlideOut.push_back(ezMirror);
                    }
                }

                if (!mirrorsToSlideOut.empty()) {
                    RT_RenderMirrors(
                        mirrorsToSlideOut, geo, request.fullW, request.fullH,
                        request.overlayOpacity, excludeOoms, request.relativeStretching,
                        request.transitionProgress, request.mirrorSlideProgress,
                        request.fromX, request.fromY, request.fromW, request.fromH,
                        request.toX, request.toY, request.toW, request.toH, true,
                        request.isTransitioningFromEyeZoom,
                        request.eyeZoomAnimatedViewportX, request.skipAnimation,
                        request.modeId, cfg.eyezoom.slideMirrorsIn,
                        request.toSlideMirrorsIn, true /* isSlideOutPass */, renderVAO,
                        renderVBO
                    );
                }
            }

            if (!request.isTransitioningFromEyeZoom && request.fromSlideMirrorsIn &&
                !request.fromModeId.empty() && request.mirrorSlideProgress < 1.0f &&
                !request.skipAnimation) {
                ...
                for (const auto& fromMirror : fromModeMirrors) {
                    ...
                    if (!existsInTarget) {
                        mirrorsToSlideOut.push_back(fromMirror);
                    }
                }
                ...
                if (!mirrorsToSlideOut.empty()) {
                    RT_RenderMirrors(
                        ...
                        request.transitionProgress, request.mirrorSlideProgress,
                        request.fromX, request.fromY, request.fromW, request.fromH,
                        request.toX, request.toY, request.toW, request.toH, false, false,
                        -1, request.skipAnimation, request.modeId,
                        request.fromSlideMirrorsIn, request.toSlideMirrorsIn,
            ...
            if (!request.isRawWindowedMode && !activeImages.empty()) {
                ...
                RT_RenderImages(
                    activeImages, request.fullW, request.fullH, request.toX,
                    request.toY, request.toW, request.toH, request.gameW, request.gameH,
                    request.relativeStretching, request.transitionProgress,
                    request.fromX, request.fromY, request.fromW, request.fromH,
                    request.overlayOpacity, excludeOoms, renderVAO, renderVBO
                );
            }

            if (!activeWindowOverlays.empty()) {
                PROFILE_SCOPE_CAT("RT Window Overlay Render", "Render Thread");
                RT_RenderWindowOverlays(
                    activeWindowOverlays, request.fullW, request.fullH, request.toX,
                    request.toY, request.toW, request.toH, request.gameW, request.gameH,
                    request.relativeStretching, request.transitionProgress,
                    request.fromX, request.fromY, request.fromW, request.fromH,
                    request.overlayOpacity, excludeOoms, renderVAO, renderVBO
            ...
            if (g_renderThreadImGuiInitialized && shouldRenderAnyImGui) {
                ...
                if (g_eyeZoomFontNeedsReload.exchange(false)) {
                    std::string newFontPath = cfg.eyezoom.textFontPath.empty()
                                                  ? cfg.fontPath
                                                  : cfg.eyezoom.textFontPath;
                    ...
                    if (newFontPath != g_eyeZoomFontPathCached) {
                        ...
                        g_eyeZoomTextFont = RT_AddFontWithArialFallback(
                            io.Fonts, newFontPath, 80.0f * g_eyeZoomScaleFactor,
                            "EyeZoom font", &g_eyeZoomFontPathCached
                        );
                        if (g_eyeZoomFontPathCached.empty()) {
                            g_eyeZoomFontPathCached = ConfigDefaults::CONFIG_FONT_PATH;
                        }
                        ...
                        if (!io.Fonts->Build()) {
                            Log("Render Thread: Font atlas build failed after reload; "
                                "forcing Arial fallback");
                            io.Fonts->Clear();
                            (void)RT_AddFontWithArialFallback(
                                io.Fonts, ConfigDefaults::CONFIG_FONT_PATH,
                                16.0f * g_eyeZoomScaleFactor, "base font (forced Arial)"
                            );
                            g_eyeZoomTextFont = RT_AddFontWithArialFallback(
                                io.Fonts, ConfigDefaults::CONFIG_FONT_PATH,
                            ...
                            if (!io.Fonts->Build()) {
                                Log("ERROR: Render Thread: Font atlas still failing after "
                                    "Arial fallback; using ImGui default font only");
                                io.Fonts->Clear();
                                io.Fonts->AddFontDefault();
                                (void)io.Fonts->Build();
                ...
                if (g_hwndChanged.exchange(false)) {
                    ...
                    if (newHwnd != NULL) {
                        Log("Render Thread: HWND changed, reinitializing ImGui Win32 "
                            "backend");
                ...
                if (request.showEyeZoom && request.eyeZoomFadeOpacity > 0.0f &&
                    ...
                    int viewportX = (request.eyeZoomAnimatedViewportX >= 0)
                                        ? request.eyeZoomAnimatedViewportX
                                        : targetViewportX;

                    // Calculate dimensions and position - must match RT_RenderEyeZoom
                    // logic
                    int zoomOutputWidth;
                    // Use request value, NOT global atomic - ensures text and boxes use
                    ...
                    if (stableZoomOutputWidth <= 1) {
                        ...
                        zoomOutputWidth =
                            zoomConfig.slideZoomIn
                                ? stableZoomOutputWidth
                                : (viewportX - (2 * zoomConfig.horizontalMargin));
                    }

                    if (viewportX > 0 && zoomOutputWidth > 20) {
                        int finalZoomX = zoomConfig.useCustomPosition
                                             ? zoomConfig.positionX
                                             : zoomConfig.horizontalMargin;
                        int zoomX      = finalZoomX;
                        ...
                        if (zoomConfig.slideZoomIn) {
                            ...
                            if ((isTransitioningToEyeZoom || isTransitioningFromEyeZoom) &&
                                ...
                                float progress = (float)viewportX / (float)targetViewportX;
                                zoomX =
                                    offScreenX + (int)((finalZoomX - offScreenX) * progress);
                            }
                        }

                        int zoomOutputHeight =
                            request.fullH - (2 * zoomConfig.verticalMargin);
                        int minHeight = (int)(0.2f * request.fullH);
                        if (zoomOutputHeight < minHeight) zoomOutputHeight = minHeight;

                        int zoomY = zoomConfig.useCustomPosition
                                        ? zoomConfig.positionY
                                        : zoomConfig.verticalMargin;
                        ...
                        if (zoomConfig.useCustomPosition) {
                            ...
                            bool isSlidingNow =
                                zoomConfig.slideZoomIn &&
                                (isTransitioningToEyeZoom || isTransitioningFromEyeZoom);
                            if (!isSlidingNow) {
                                zoomX = (std::max)(0, (std::min)(zoomX, maxZoomX));
                            }
                            zoomY = (std::max)(0, (std::min)(zoomY, maxZoomY));
                        ...
                        float pixelWidthOnScreen =
                            zoomOutputWidth / (float)zoomConfig.cloneWidth;
                        int labelsPerSide        = zoomConfig.cloneWidth / 2;
                        int overlayLabelsPerSide = zoomConfig.overlayWidth;
                        if (overlayLabelsPerSide < 0) overlayLabelsPerSide = labelsPerSide;
                        if (overlayLabelsPerSide > labelsPerSide)
                        ...
                        ImDrawList* drawList          = request.shouldRenderGui
                        ...
                        if (zoomConfig.autoFontSize) {
                            float boxHeight       = zoomConfig.linkRectToFont
                                                        ? (requestedFontSize * 1.2f)
                                                        : (float)zoomConfig.rectHeight;
                            float maxFontByWidth  = pixelWidthOnScreen * 0.90f;
                            float maxFontByHeight = boxHeight * 0.85f;
                            if (maxFontByWidth > 0.0f)
                                fontSize = (std::min)(fontSize, maxFontByWidth);
                            if (maxFontByHeight > 0.0f)
                                fontSize = (std::min)(fontSize, maxFontByHeight);
                            if (fontSize < 6.0f) fontSize = 6.0f;
                        }
                        float finalTextAlpha =
                            zoomConfig.textColorOpacity * request.eyeZoomFadeOpacity;
                        ImU32 textColor =
                            IM_COL32(static_cast<int>(zoomConfig.textColor.r * 255), static_cast<int>(zoomConfig.textColor.g * 255), static_cast<int>(zoomConfig.textColor.b * 255), static_cast<int>(finalTextAlpha * 255));

                        ImFont* font =
                            g_eyeZoomTextFont ? g_eyeZoomTextFont : ImGui::GetFont();

                        for (int xOffset = -overlayLabelsPerSide;
                             xOffset <= overlayLabelsPerSide; xOffset++) {
                            if (xOffset == 0) continue;
                            ...
                            if (zoomConfig.autoFontSize) {
                                ...
                                if (maxTextWidth > 0.0f && textSize.x > maxTextWidth &&
                                    textSize.x > 0.0f) {
                ...
                // Publish capture flags for the window thread (ESC handling, overlay
                // keyboard forwarding, etc.)
            ...
            if (isObsRequest) {
                ...
                GLsync oldFence =
                    g_lastGoodObsFence.exchange(fence, std::memory_order_acq_rel);
                // Deferred deletion: delete the fence from 2 cycles ago, store current
                // old fence
                if (g_pendingDeleteObsFences[g_pendingDeleteObsIndex]) {
                    if (glIsSync(g_pendingDeleteObsFences[g_pendingDeleteObsIndex])) {
                        glDeleteSync(g_pendingDeleteObsFences[g_pendingDeleteObsIndex]);
                ...
                if (IsVirtualCameraActive()) {
                    ...
                    if (g_vcCursorFBO == 0 || g_vcCursorWidth != vcW ||
                        g_vcCursorHeight != vcH) {
                        if (g_vcCursorTexture != 0) {
                            glDeleteTextures(1, &g_vcCursorTexture);
                        }
                        if (g_vcCursorFBO == 0) {
                            glGenFramebuffers(1, &g_vcCursorFBO);
                        }
...
void StartRenderThread(void* gameGLContext) {
    ...
    if (g_renderThread.joinable()) {
        if (g_renderThreadRunning.load()) {
            ...
            // If the previous thread exited early (exception), it may not have
            // cleaned up.
    ...
    if (sharedContext && sharedDC) {
        ...
        Log("Render Thread: Using pre-shared context (GPU texture sharing "
            "enabled)");
    } else {
        ...
        // Prefer a dedicated dummy DC for the worker context to avoid cross-thread
        // HDC issues.
        if (RT_CreateFallbackDummyWindowWithMatchingPixelFormat(
                gameHdc, L"render", g_renderFallbackDummyHwnd,
                g_renderFallbackDummyDC
            ) &&
            g_renderFallbackDummyDC) {
            g_renderThreadDC = g_renderFallbackDummyDC;
            // If we had to call GetDC(hwnd) only to query the pixel format, release
            // it now.
            if (gameHwndForDC) {
            ...
            g_renderOwnedDCHwnd =
                gameHwndForDC;  // Release on StopRenderThread if non-null
        }

        // Create the render context on main thread
        g_renderThreadContext = wglCreateContext(g_renderThreadDC);
        if (!g_renderThreadContext) {
            Log("Render Thread: Failed to create GL context (error " +
                std::to_string(GetLastError()) + ")");
            if (g_renderOwnedDCHwnd && g_renderThreadDC) {
                ReleaseDC(g_renderOwnedDCHwnd, g_renderThreadDC);
                g_renderOwnedDCHwnd = NULL;
        ...
        if (!wglShareLists((HGLRC)gameGLContext, g_renderThreadContext)) {
            ...
            if (!wglShareLists(g_renderThreadContext, (HGLRC)gameGLContext)) {
                ...
                Log("Render Thread: wglShareLists failed (errors " +
                    std::to_string(err1) + ", " + std::to_string(err2) + ")");
                wglDeleteContext(g_renderThreadContext);
                g_renderThreadContext = NULL;
                if (prevRC && prevDC) {
        ...
        Log("Render Thread: Context created and shared on main thread (fallback "
            "mode)");
    }
    ...
    // Clear consumer fences (should already be null, but be safe across hot
    // reloads)
    for (int i = 0; i < RENDER_THREAD_FBO_COUNT; ++i) {
...
void StopRenderThread() {
    if (!g_renderThreadRunning.load() && !g_renderThread.joinable()) {
        return;
    }
    ...
    // If the render thread crashed, it may not have reached its normal cleanup
    // path.
    if (!g_renderContextIsShared && g_renderThreadContext) {
        wglDeleteContext(g_renderThreadContext);
        g_renderThreadContext = NULL;
    ...
    // If we created a fallback dummy window/DC, destroy it on the main thread
    // after join.
    if (!g_renderContextIsShared) {
        if (g_renderOwnedDCHwnd && g_renderThreadDC) {
            ReleaseDC(g_renderOwnedDCHwnd, g_renderThreadDC);
...
void SubmitFrameForRendering(const FrameRenderRequest& request) {
    ...
    if (g_requestReadySlot.load(std::memory_order_relaxed) != -1) {
    ...
    if (writeSlot == readSlotInUse) {
        // Extremely unlikely (would require invalid state), but never risk a data
        // race.
        g_framesDropped.fetch_add(1, std::memory_order_relaxed);
        return;
    }
    ...
    // This also tells render thread which slot to read from (the one we just
    // wrote)
    ...
    // Signal the condition variable (brief lock only for CV, not for data
    // protection)
    {
...
int WaitForRenderComplete(int timeoutMs) {
    ...
    bool completed = g_completionCV.wait_for(
...
GLsync GetCompletedRenderFence() {
    ...
    // This is more efficient than glFinish() as it only waits for the render
    // thread's commands
...
void SubmitRenderFBOConsumerFence(int fboIndex, GLsync consumerFence) {
    ...
    if (fboIndex < 0 || fboIndex >= RENDER_THREAD_FBO_COUNT) {
        if (glIsSync(consumerFence)) {
            glDeleteSync(consumerFence);
        }
        return;
    }

    GLsync old = g_renderFBOConsumerFences[fboIndex].exchange(
        consumerFence, std::memory_order_acq_rel
    );
    if (old && glIsSync(old)) {
        glDeleteSync(old);
    }
...
void SubmitObsFrameContext(const ObsFrameSubmission& submission) {
    ...
    // NOTE: We do NOT delete fences here even if overwriting a pending
    // submission. The render thread owns the gameTextureFence and is responsible
    // for deleting it after processing. Deleting here causes a race condition
    // where the render thread may have already copied the fence pointer and will
    // try to delete it again. Occasional fence leaks from dropped frames are
    // acceptable and rare.

    if (g_obsReadySlot.load(std::memory_order_relaxed) != -1) {
        g_framesDropped.fetch_add(1, std::memory_order_relaxed);
    }
...
FrameRenderRequest BuildObsFrameRequest(const ObsFrameContext& ctx, bool isDualRenderingPath) {
    ...
    if (!transitionState.fromModeId.empty()) {
        const ModeConfig* fromMode =
            GetModeFromSnapshot(obsCfg, transitionState.fromModeId);
        if (fromMode) {
            req.fromSlideMirrorsIn = fromMode->slideMirrorsIn;
        }
    }
    const ModeConfig* toMode = GetModeFromSnapshot(obsCfg, ctx.modeId);
    if (toMode) {
        req.toSlideMirrorsIn = toMode->slideMirrorsIn;
    }
    ...
    bool transitionEffectivelyComplete =
        !transitionState.active || transitionState.progress >= 1.0f;
    ...
    bool transitioningToFullscreen = EqualsIgnoreCase(ctx.modeId, "Fullscreen") &&
                                     !transitionState.fromModeId.empty();
    ...
    req.transitioningToFullscreen =
        transitioningToFullscreen && !transitionEffectivelyComplete;
    ...
    req.eyeZoomAnimatedViewportX =
        isDualRenderingPath ? transitionState.x : ctx.eyeZoomAnimatedViewportX;
