    ...
    if (auto arr = GetArray(tbl, "targetColors")) {
        for (const auto& elem : *arr) {
            if (auto colorArr = elem.as_array()) {
                cfg.targetColors.push_back(ColorFromTomlArray(colorArr, {0.0f, 1.0f, 0.0f}));
            }
        }
    }

    if (cfg.targetColors.empty()) {
        cfg.targetColors.push_back(ColorFromTomlArray(GetArray(tbl, "target"), {0.0f, 1.0f, 0.0f}));
    }
...
void MirrorConfigFromToml(const toml::table& tbl, MirrorConfig& cfg) {
    ...
    if (auto t = GetTable(tbl, "output")) {
        MirrorRenderConfigFromToml(*t, cfg.output);
    }

    if (auto t = GetTable(tbl, "colors")) {
        MirrorColorsFromToml(*t, cfg.colors);
    }
...
void StretchConfigToToml(const StretchConfig& cfg, toml::table& out) {
    ...
    if (!cfg.widthExpr.empty()) {
        out.insert("widthExpr", cfg.widthExpr);
    }
    if (!cfg.heightExpr.empty()) {
        out.insert("heightExpr", cfg.heightExpr);
    }
    if (!cfg.xExpr.empty()) {
        out.insert("xExpr", cfg.xExpr);
    }
    if (!cfg.yExpr.empty()) {
        out.insert("yExpr", cfg.yExpr);
    }
}
...
void ImageConfigFromToml(const toml::table& tbl, ImageConfig& cfg) {
    ...
    if (auto t = GetTable(tbl, "background")) {
        ImageBackgroundConfigFromToml(*t, cfg.background);
    }
    ...
    if (auto t = GetTable(tbl, "border")) {
        BorderConfigFromToml(*t, cfg.border);
    }
}
...
void ModeConfigToToml(const ModeConfig& cfg, toml::table& out) {
    ...
    for (const auto& id : cfg.mirrorIds) {
        mirrorIds.push_back(id);
    }
    out.insert("mirrorIds", mirrorIds);

    toml::array mirrorGroupIds;
    for (const auto& id : cfg.mirrorGroupIds) {
        mirrorGroupIds.push_back(id);
    }
...
void ModeConfigFromToml(const toml::table& tbl, ModeConfig& cfg) {
    ...
    if (auto t = GetTable(tbl, "background")) {
        BackgroundConfigFromToml(*t, cfg.background);
    }
    ...
    if (auto arr = GetArray(tbl, "mirrorGroupIds")) {
        for (const auto& elem : *arr) {
            if (auto val = elem.value<std::string>()) {
                cfg.mirrorGroupIds.push_back(*val);
            }
        }
    }
    ...
    if (auto t = GetTable(tbl, "stretch")) {
        StretchConfigFromToml(*t, cfg.stretch);
    }
...
void HotkeyConditionsToToml(const HotkeyConditions& cfg, toml::table& out) {
    ...
    for (const auto& excl : cfg.exclusions) {
        exclusionsArr.push_back(static_cast<int64_t>(excl));
    }
    ...
    if (auto arr = GetArray(tbl, "exclusions")) {
        for (const auto& elem : *arr) {
            if (auto val = elem.value<int64_t>()) {
                cfg.exclusions.push_back(static_cast<DWORD>(*val));
            }
        }
    }
}
...
void AltSecondaryModeFromToml(const toml::table& tbl, AltSecondaryMode& cfg) {
    ...
    if (auto arr = GetArray(tbl, "keys")) {
        for (const auto& elem : *arr) {
            if (auto val = elem.value<int64_t>()) {
                cfg.keys.push_back(static_cast<DWORD>(*val));
            }
        }
    }
...
void HotkeyConfigFromToml(const toml::table& tbl, HotkeyConfig& cfg) {
    ...
    if (auto t = GetTable(tbl, "conditions")) {
        HotkeyConditionsFromToml(*t, cfg.conditions);
    }
...
void CursorsConfigFromToml(const toml::table& tbl, CursorsConfig& cfg) {
    ...
    if (auto t = GetTable(tbl, "title")) {
        CursorConfigFromToml(*t, cfg.title);
    }
    if (auto t = GetTable(tbl, "wall")) {
        CursorConfigFromToml(*t, cfg.wall);
    }
    if (auto t = GetTable(tbl, "ingame")) {
        CursorConfigFromToml(*t, cfg.ingame);
    }
}
    ...
    if (auto colorsTbl = GetTable(tbl, "customColors")) {
        for (const auto& [key, value] : *colorsTbl) {
            if (auto arr = value.as_array()) {
                cfg.customColors[std::string(key.str())] = ColorFromTomlArray(arr);
            }
        }
    }
}
...
void ConfigToToml(const Config& config, toml::table& out) {
    ...
    for (const auto& key : config.borderlessHotkey) {
        borderlessHotkeyArr.push_back(static_cast<int64_t>(key));
    }
    ...
    for (const auto& key : config.imageOverlaysHotkey) {
        imageOverlaysHotkeyArr.push_back(static_cast<int64_t>(key));
    }
    out.insert("imageOverlaysHotkey", imageOverlaysHotkeyArr);

    toml::array windowOverlaysHotkeyArr;
    for (const auto& key : config.windowOverlaysHotkey) {
        windowOverlaysHotkeyArr.push_back(static_cast<int64_t>(key));
    }
...
void ConfigFromToml(const toml::table& tbl, Config& config) {
    ...
    if (auto arr = GetArray(tbl, "guiHotkey")) {
        for (const auto& elem : *arr) {
            if (auto val = elem.value<int64_t>()) {
                config.guiHotkey.push_back(static_cast<DWORD>(*val));
            }
        }
    }
    if (config.guiHotkey.empty()) {
        config.guiHotkey = ConfigDefaults::GetDefaultGuiHotkey();
    }
    ...
    if (auto arr = GetArray(tbl, "borderlessHotkey")) {
        for (const auto& elem : *arr) {
            if (auto val = elem.value<int64_t>()) {
                config.borderlessHotkey.push_back(static_cast<DWORD>(*val));
            }
        }
    }
    if (!hasBorderlessHotkey) {
        config.borderlessHotkey = ConfigDefaults::GetDefaultBorderlessHotkey();
    }
    ...
    if (auto arr = GetArray(tbl, "imageOverlaysHotkey")) {
        for (const auto& elem : *arr) {
            if (auto val = elem.value<int64_t>()) {
                config.imageOverlaysHotkey.push_back(static_cast<DWORD>(*val));
            }
        }
    }
    if (!hasImageOverlaysHotkey) {
        config.imageOverlaysHotkey = ConfigDefaults::GetDefaultImageOverlaysHotkey();
    }
    ...
    if (auto arr = GetArray(tbl, "windowOverlaysHotkey")) {
        for (const auto& elem : *arr) {
            if (auto val = elem.value<int64_t>()) {
                config.windowOverlaysHotkey.push_back(static_cast<DWORD>(*val));
            }
        }
    }
    if (!hasWindowOverlaysHotkey) {
        config.windowOverlaysHotkey = ConfigDefaults::GetDefaultWindowOverlaysHotkey();
    }

    if (auto t = GetTable(tbl, "debug")) {
        DebugGlobalConfigFromToml(*t, config.debug);
    }

    if (auto t = GetTable(tbl, "eyezoom")) {
        EyeZoomConfigFromToml(*t, config.eyezoom);
    }

    if (auto t = GetTable(tbl, "cursors")) {
        CursorsConfigFromToml(*t, config.cursors);
    }

    if (auto t = GetTable(tbl, "keyRebinds")) {
        KeyRebindsConfigFromToml(*t, config.keyRebinds);
    }

    if (auto t = GetTable(tbl, "appearance")) {
        AppearanceConfigFromToml(*t, config.appearance);
    }
...
bool SaveConfigToTomlFile(const Config& config, const std::wstring& path) {
    try {
        ...
        std::vector<std::string> orderedKeys = {"configVersion", "disableHookChaining", "hookChainingNextTarget", "defaultMode", "fontPath", "fpsLimit", "fpsLimitSleepThreshold", "allowCursorEscape", "mouseSensitivity", "windowsMouseSpeed", "hideAnimationsInGame", "keyRepeatStartDelay", "keyRepeatDelay", "basicModeEnabled", "disableFullscreenPrompt", "disableConfigurePrompt", "guiHotkey", "borderlessHotkey", "autoBorderless", "imageOverlaysHotkey", "windowOverlaysHotkey", "debug", "eyezoom", "cursors", "keyRebinds", "appearance", "mode", "mirror", "mirrorGroup", "image", "windowOverlay", "hotkey", "sensitivityHotkey"};

        std::vector<std::string> modeKeys          = {"id", "width", "height", "background", "mirrorIds", "mirrorGroupIds", "imageIds", "windowOverlayIds", "stretch", "gameTransition", "overlayTransition", "backgroundTransition", "transitionDurationMs", "easeInPower", "easeOutPower", "bounceCount", "bounceIntensity", "bounceDurationMs", "relativeStretching", "skipAnimateX", "skipAnimateY", "border", "sensitivityOverrideEnabled", "modeSensitivity", "separateXYSensitivity", "modeSensitivityX", "modeSensitivityY"};
        std::vector<std::string> mirrorKeys        = {"name", "captureWidth", "captureHeight", "input", "output", "colors", "colorSensitivity", "border", "fps", "rawOutput", "colorPassthrough", "onlyOnMyScreen", "debug"};
        std::vector<std::string> mirrorGroupKeys   = {"name", "output", "mirrorIds"};
        std::vector<std::string> imageKeys         = {"name", "path", "x", "y", "scale", "relativeTo", "crop_top", "crop_bottom", "crop_left", "crop_right", "enableColorKey", "colorKeys", "opacity", "background", "pixelatedScaling", "onlyOnMyScreen", "border"};
        std::vector<std::string> windowOverlayKeys = {"name", "windowTitle", "windowClass", "executableName", "windowMatchPriority", "x", "y", "scale", "relativeTo", "crop_top", "crop_bottom", "crop_left", "crop_right", "enableColorKey", "colorKeys", "opacity", "background", "pixelatedScaling", "onlyOnMyScreen", "fps", "captureMethod", "enableInteraction", "border"};
...
std::string GetEmbeddedDefaultConfigString() {
    if (s_embeddedConfigLoaded) {
        return s_embeddedConfigCache;
    }

    HMODULE hModule = nullptr;
    if (!GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT, reinterpret_cast<LPCWSTR>(&GetEmbeddedDefaultConfigString), &hModule)) {
        Log("ERROR: Failed to get module handle for embedded config");
        return "";
    }

    HRSRC hResource = FindResourceW(hModule, L"config_default", RT_RCDATA);
    if (!hResource) {
...
std::vector<ModeConfig> GetDefaultModesFromEmbedded() {
    ...
    try {
        ...
        if (const auto* arr = GetArray(tbl, "mode")) {
            for (const auto& elem : *arr) {
                if (const auto* t = elem.as_table()) {
                    ModeConfig mode;
                    ModeConfigFromToml(*t, mode);
        ...
        Log("ERROR: Failed to parse embedded modes: " + std::string(e.what()));
    }
...
std::vector<MirrorConfig> GetDefaultMirrorsFromEmbedded() {
    ...
    try {
        ...
        Log("ERROR: Failed to parse embedded mirrors: " + std::string(e.what()));
    }
...
std::vector<HotkeyConfig> GetDefaultHotkeysFromEmbedded() {
    ...
    try {
        ...
        Log("ERROR: Failed to parse embedded hotkeys: " + std::string(e.what()));
    }
...
std::vector<ImageConfig> GetDefaultImagesFromEmbedded() {
    ...
    try {
        ...
        Log("ERROR: Failed to parse embedded images: " + std::string(e.what()));
    }
...
CursorsConfig GetDefaultCursorsFromEmbedded() {
    ...
    try {
        ...
        if (auto t = GetTable(tbl, "cursors")) {
            CursorsConfigFromToml(*t, cursors);
        }
        ...
        Log("ERROR: Failed to parse embedded cursors: " + std::string(e.what()));
    }
...
EyeZoomConfig GetDefaultEyeZoomConfigFromEmbedded() {
    ...
    try {
        ...
        if (auto t = GetTable(tbl, "eyezoom")) {
            EyeZoomConfigFromToml(*t, eyezoom);
        }
        ...
        Log("ERROR: Failed to parse embedded eyezoom: " + std::string(e.what()));
    }
