...
void DrawOverlayBorder(float nx1, float ny1, float nx2, float ny2, float borderWidth, float borderHeight, bool isDragging, bool drawCorners = false) {
...
void CleanupGPUResources() {
    ...
    auto safeGLDelete = [](auto deleteFunc, auto handle) {
        ...
        try {
            ...
            Log("Exception during GPU resource deletion");
        }
    };
    ...
    try {
        ...
        Log("CleanupGPUResources: Exception during FBO cleanup");
    }
    ...
    try {
        ...
        Log("CleanupGPUResources: Exception during texture cleanup");
    }
    ...
    try {
        ...
        Log("CleanupGPUResources: Exception during VAO/VBO cleanup");
    }
    ...
    try {
        ...
        Log("CleanupGPUResources: Exception during shader cleanup");
    }
...
void InitializeGPUResources() {
    ...
    std::vector<MirrorConfig> mirrorsToCreate;
    {
        ...
        if (initSnap) {
            mirrorsToCreate = initSnap->mirrors;
        }
...
void CreateMirrorGPUResources(const MirrorConfig& conf) {
    ...
    if (frontComplete && backComplete && finalFrontComplete && finalBackComplete) {
        ...
        LogCategory("init", "Created double-buffered GPU resources for mirror '" + conf.name + "' (FBO: " + std::to_string(inst.fbo) + ", Back: " + std::to_string(inst.fboBack) + ", FinalFBO: " + std::to_string(inst.finalFbo) + " [" + std::to_string(inst.final_w) + "x" + std::to_string(inst.final_h) + "])");
    } else {
...
void handleEyeZoomMode(const GLState& s, float opacity, int animatedViewportX) {
    ...
    if (zoomOutputHeight < minHeight) {
        zoomOutputHeight = minHeight;
    }
    ...
    auto EnsureEyeZoomSnapshotAllocated = [&]() {
        if (s_eyeZoomSnapshotTexture == 0 || s_eyeZoomSnapshotWidth != zoomOutputWidth || s_eyeZoomSnapshotHeight != zoomOutputHeight) {
            if (s_eyeZoomSnapshotTexture != 0) {
                glDeleteTextures(1, &s_eyeZoomSnapshotTexture);
            }
            if (s_eyeZoomSnapshotFBO != 0) {
    ...
    if (opacity < 1.0f) {
        if (s_eyeZoomTempTexture == 0 || s_eyeZoomTempWidth != zoomOutputWidth || s_eyeZoomTempHeight != zoomOutputHeight) {
            if (s_eyeZoomTempTexture != 0) {
                glDeleteTextures(1, &s_eyeZoomTempTexture);
            }
            if (s_eyeZoomTempFBO != 0) {
        ...
        if (useSnapshot) {
            if (s_eyeZoomBlitFBO == 0) {
                glGenFramebuffers(1, &s_eyeZoomBlitFBO);
            }
            ...
            glBlitFramebuffer(0, 0, s_eyeZoomSnapshotWidth, s_eyeZoomSnapshotHeight, 0, 0, zoomOutputWidth, zoomOutputHeight, GL_COLOR_BUFFER_BIT, GL_NEAREST);
        } else {
            if (s_eyeZoomBlitFBO == 0) {
                glGenFramebuffers(1, &s_eyeZoomBlitFBO);
            }
            ...
            glBlitFramebuffer(0, 0, zoomOutputWidth, zoomOutputHeight, 0, 0, s_eyeZoomSnapshotWidth, s_eyeZoomSnapshotHeight, GL_COLOR_BUFFER_BIT, GL_NEAREST);
            glBindFramebuffer(GL_FRAMEBUFFER, 0);
        ...
        glUniform4f(g_solidColorShaderLocs.color, zoomConfig.centerLineColor.r, zoomConfig.centerLineColor.g, zoomConfig.centerLineColor.b, zoomConfig.centerLineColorOpacity);
        ...
        if (useSnapshot) {
            ...
            glBlitFramebuffer(0, 0, s_eyeZoomSnapshotWidth, s_eyeZoomSnapshotHeight, dstLeft, dstBottom, dstRight, dstTop, GL_COLOR_BUFFER_BIT, GL_NEAREST);
        } else {
            if (s_eyeZoomBlitFBO == 0) {
                glGenFramebuffers(1, &s_eyeZoomBlitFBO);
            }
            ...
            glBindFramebuffer(GL_DRAW_FRAMEBUFFER, s_eyeZoomSnapshotFBO);
            glBlitFramebuffer(dstLeft, dstBottom, dstRight, dstTop, 0, 0, s_eyeZoomSnapshotWidth, s_eyeZoomSnapshotHeight, GL_COLOR_BUFFER_BIT, GL_NEAREST);
            glBindFramebuffer(GL_FRAMEBUFFER, 0);
...
void RenderModeInternal(const ModeConfig* modeToRender, const GLState& s, int current_gameW, int current_gameH, bool skipAnimation, bool excludeOnlyOnMyScreen);

void RenderMode(const ModeConfig* modeToRender, const GLState& s, int current_gameW, int current_gameH, bool skipAnimation, bool excludeOnlyOnMyScreen) {
...
void RenderModeInternal(const ModeConfig* modeToRender, const GLState& s, int current_gameW, int current_gameH, bool skipAnimation, bool excludeOnlyOnMyScreen) {
    ...
    if (useOptimizedPath) {
...

        {
            ...
            if (transitioningToFullscreen && fromBorder.enabled && fromBorder.width > 0) {
                ...
                RenderGameBorder(finalX, finalY, finalW, finalH, modeToRender->border.width, modeToRender->border.radius, modeToRender->border.color, fullW, fullH);
            }
        }
    }
    ...
    if (hasMirrors) {
        ...
        if (!useFramebufferFallback && g_graphicsHookDetected.load()) {
            StartObsHookThread();
        }
        ...
        if (useFramebufferFallback) {
            ...
            for (size_t i = 0; i < fbMirrors.size(); ++i) {
                mirrorIndex[fbMirrors[i].name] = i;
            }
            std::unordered_map<std::string, size_t> groupIndex;
            groupIndex.reserve(fbGroups.size());
            for (size_t i = 0; i < fbGroups.size(); ++i) {
                groupIndex[fbGroups[i].name] = i;
            }
...

            {
                ...
                for (size_t i = 0; i < fallbackMirrors.size(); ++i) {
                    ...
                    if (needsUpdate) {
                        mirrorsNeedingUpdate.push_back(i);
                    }
                }
            }
            ...
            if (!mirrorsNeedingUpdate.empty()) {
                ...
                for (size_t idx : mirrorsNeedingUpdate) {
                    ...
                    for (const auto& r : conf.input) {
                        int capX, capY;
                        GetRelativeCoords(r.relativeTo, r.x, r.y, conf.captureWidth, conf.captureHeight, current_gameW, current_gameH, capX, capY);
                    ...
                    if (inst.forceUpdateFrames > 0) {
                        inst.forceUpdateFrames--;
                    }
                }
            ...
            if (hwnd) {
                ...
                if (mousePos.x >= s.vp[0] && mousePos.x < (s.vp[0] + s.vp[2]) && mousePos.y >= s.vp[1] &&
                    ...
                    for (const auto& imageName : modeToRender->imageIds) {
                        ...
                        GetRelativeCoordsForImageWithViewport(conf.relativeTo, conf.x, conf.y, displayW, displayH, currentGeo.finalX, currentGeo.finalY, currentGeo.finalW, currentGeo.finalH, fullW, fullH, finalScreenX_win, finalScreenY_win);
                    ...
                    if (!s_isWindowOverlayDragging) {
                        ...
                        if (cacheLock.owns_lock()) {
                            ...
                            for (const auto& overlayId : modeToRender->windowOverlayIds) {
                                ...
                                if (config) {
                                    activeOverlays.push_back(config);
                                }
                            }
    ...
    bool wantWelcomeToast = false;
    {
        ...
        auto                                         now = std::chrono::steady_clock::now();
        if (isFull && !s_prevFullscreen) {
            s_fullscreenEnterTime = now;
        }
    ...
    if (g_renderThreadRunning.load() && wantAsyncOverlayThisFrame) {
...

        {
            ...
            if (isAnimating && transitionState.gameTransition == GameTransitionType::Bounce) {
                if (transitionState.fromWidth != transitionState.targetWidth) {
                    request.letterboxExtendX = 1;
                }
                if (transitionState.fromHeight != transitionState.targetHeight) {
                    request.letterboxExtendY = 1;
                }
            }
...
void RenderDebugBordersForMirror(const MirrorConfig* conf, Color captureColor, Color outputColor, GLint originalVAO) {
    ...
    if (instIt != g_mirrorInstances.end()) {
        ...
        CalculateFinalScreenPos(conf, inst, geo.gameW, geo.gameH, geo.finalX, geo.finalY, geo.finalW, geo.finalH, fullW, fullH, finalScreenX, finalScreenY_win);
        float scaleX = conf->output.separateScale ? conf->output.scaleX : conf->output.scale;
        float scaleY = conf->output.separateScale ? conf->output.scaleY : conf->output.scale;
...
void RenderTextureGridOverlay(bool showTextureGrid, int modeWidth, int modeHeight) {
    ...
    if (!g_glInitialized.load(std::memory_order_acquire) || g_solidColorProgram == 0) {
        return;
    }
    ...
    for (const auto& tex : validTextures) {
        ...
        {
            std::lock_guard<std::mutex> lock(s_textureGridMutex);
            s_textureGridLabels.push_back({tex.id, (float)x, (float)y, TILE_SIZE, texWidth, texHeight, sizeMB, (GLenum)internalFormat, minFilter, magFilter, wrapS, wrapT});
        }
...
void RenderCachedTextureGridLabels() {
    ...
    if (!ImGui::GetCurrentContext() || s_textureGridLabels.empty()) {
        return;
    }
...
static float ApplyDualEasing(float t, float easeInPower, float easeOutPower) {
    ...
    if (easeInPower <= 1.0f && easeOutPower <= 1.0f) {
        return t;
    }
...
void StartModeTransition(const std::string& fromModeId, const std::string& toModeId, int fromWidth, int fromHeight, int fromX, int fromY, int toWidth, int toHeight, int toX, int toY, const ModeConfig& toMode) {
    ...
    LogCategory("animation", "[ANIMATION] Starting mode transition (Game:" + GameTransitionTypeToString(toMode.gameTransition) + ", Overlay:" + OverlayTransitionTypeToString(toMode.overlayTransition) + ", Bg:" + BackgroundTransitionTypeToString(toMode.backgroundTransition) + ", " + std::to_string(toMode.transitionDurationMs) + "ms): " + fromModeId + " (" + std::to_string(fromWidth) + "x" + std::to_string(fromHeight) + " at " + std::to_string(fromX) + "," + std::to_string(fromY) + ")" + " -> " + toModeId + " (" + std::to_string(toWidth) + "x" + std::to_string(toHeight) + " at " + std::to_string(toX) + "," + std::to_string(toY) + ")");
...
void UpdateModeTransition() {
        ...
        if (g_modeTransition.progress < baseRatio) {
            ...
            if (g_modeTransition.bounceCount > 0 && totalBounceDuration > 0) {
                ...
                if (currentBounce < g_modeTransition.bounceCount) {
                    ...
                    bounceOffset              = CalculateBounceOffset(bouncePhaseProgress, currentBounce, g_modeTransition.bounceCount, g_modeTransition.bounceIntensity);
                }
            }
        }
    ...
    if (allComplete) {
        LogCategory("animation", "[ANIMATION] Mode transition complete: " + g_modeTransition.toModeId + " (final stretch: " + std::to_string(g_modeTransition.toWidth) + "x" + std::to_string(g_modeTransition.toHeight) + " at " + std::to_string(g_modeTransition.toX) + "," + std::to_string(g_modeTransition.toY) + ")");
