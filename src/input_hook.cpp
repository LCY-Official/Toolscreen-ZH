...
InputHandlerResult HandleShutdownCheck(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    if (g_isShuttingDown.load() && g_originalWndProc) {
...
static LRESULT EncodeToolscreenVersionNumber() {
    ...
    return static_cast<LRESULT>(((TOOLSCREEN_VERSION_MAJOR & 0xFF) << 16) | ((TOOLSCREEN_VERSION_MINOR & 0xFF) << 8) | (TOOLSCREEN_VERSION_PATCH & 0xFF));
}
...
InputHandlerResult HandleToolscreenQueryMessages(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    if (uMsg != isInstalledMsg && uMsg != getVersionMsg) {
        return {false, 0};
    }
...
InputHandlerResult HandleWindowPosChanged(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    if (!GetMonitorRectForWindow(hWnd, monitorRect)) {
        if (!GetWindowRect(hWnd, &monitorRect)) {
            return forwardToOriginal();
        }
    }
    ...
    if (sizeChanged && !isFullscreenNow && wasFullscreen && monitorW > 0 && monitorH > 0) {
        ...
        SetWindowPos(hWnd, HWND_NOTOPMOST, windowedX, windowedY, windowedW, windowedH, SWP_NOACTIVATE | SWP_NOOWNERZORDER | SWP_FRAMECHANGED);
...
InputHandlerResult HandleGuiToggle(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    if (!isEscape && !CheckHotkeyMatch(g_config.guiHotkey, vkCode)) {
        return {false, 0};
    }
    ...
    if (isEscape && IsHotkeyBindingActive()) {
        is_closing = false;
    }
    if (isEscape && IsRebindBindingActive()) {
        is_closing = false;
    }
...
InputHandlerResult HandleBorderlessToggle(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    if (g_config.borderlessHotkey.empty()) {
        return {false, 0};
    }

    if (IsHotkeyBindingActive() || IsRebindBindingActive()) {
        return {false, 0};
    }
    ...
    if (!CheckHotkeyMatch(g_config.borderlessHotkey, vkCode)) {
        return {false, 0};
    }
...
InputHandlerResult HandleImageOverlaysToggle(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    if (g_config.imageOverlaysHotkey.empty()) {
        return {false, 0};
    }
    ...
    if (!CheckHotkeyMatch(g_config.imageOverlaysHotkey, vkCode)) {
        return {false, 0};
    }
...
InputHandlerResult HandleWindowOverlaysToggle(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    if (g_config.windowOverlaysHotkey.empty()) {
        return {false, 0};
    }
    ...
    if (!CheckHotkeyMatch(g_config.windowOverlaysHotkey, vkCode)) {
        return {false, 0};
    }
...
InputHandlerResult HandleWmSizeModeDimensions(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, const std::string& currentModeId) {
    ...
    if (targetW <= 0 || targetH <= 0 || (msgW == targetW && msgH == targetH)) {
        return {false, 0};
    }
...
InputHandlerResult HandleHotkeys(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, const std::string& currentModeId, const std::string& gameState) {
    switch (uMsg) {
        case WM_KEYDOWN:
        case WM_SYSKEYDOWN:
        case WM_KEYUP:
    ...
    if (!isHotkeyMainKey) {
        ...
        if (isKeyDown) {
            ...
            for (const auto& pendingHotkeyId : g_triggerOnReleasePending) {
                g_triggerOnReleaseInvalidated.insert(pendingHotkeyId);
            }
        }
    ...
    for (size_t hotkeyIdx = 0; hotkeyIdx < cfg.hotkeys.size(); ++hotkeyIdx) {
        ...
        if (!conditionsMet) {
            if (!(hotkey.allowExitToFullscreenRegardlessOfGameState && wouldExitToFullscreen)) {
                if (s_enableHotkeyDebug) {
                    Log("[Hotkey] SKIP: Game state conditions not met");
                }
                continue;
            }
            if (s_enableHotkeyDebug) {
            ...
            if (matched || matchedViaRebind) {
                ...
                if (hotkey.triggerOnRelease) {
                    if (isKeyDown) {
                        std::lock_guard<std::mutex> lock(g_triggerOnReleaseMutex);
                        for (const auto& pendingHotkeyId : g_triggerOnReleasePending) {
                            if (pendingHotkeyId != hotkeyId) {
                                g_triggerOnReleaseInvalidated.insert(pendingHotkeyId);
                            }
                        }
                        g_triggerOnReleasePending.insert(hotkeyId);
                        if (s_enableHotkeyDebug) {
                            Log("[Hotkey] Alt trigger-on-release hotkey pressed, added to pending: " + hotkeyId);
                        }
                        ...
                        if (wasInvalidated) {
                            if (s_enableHotkeyDebug) {
                                Log("[Hotkey] Alt trigger-on-release hotkey invalidated: " + hotkeyId);
                            }
                            if (blockKey) return {true, 0};
                            return {true, CallWindowProc(g_originalWndProc, hWnd, uMsg, wParam, lParam)};
                        }
                ...
                if (hotkey.triggerOnRelease != isKeyDown) {
                    ...
                    if (debounced) {
                        if (s_enableHotkeyDebug) {
                            Log("[Hotkey] Alt hotkey matched but debounced: " + hotkeyId);
                        }
                        if (blockKey) return {true, 0};
                        return {true, CallWindowProc(g_originalWndProc, hWnd, uMsg, wParam, lParam)};
                    }
                    ...
                    if (s_enableHotkeyDebug) {
                        Log("[Hotkey] ✓✓✓ ALT HOTKEY TRIGGERED: " + hotkeyId + " -> " + newSecMode);
                    }

                    if (!newSecMode.empty()) {
                        SwitchToMode(newSecMode, "alt hotkey");
                    }
                }
                if (blockKey) return {true, 0};
                return {true, CallWindowProc(g_originalWndProc, hWnd, uMsg, wParam, lParam)};
            ...
            if (matched || matchedViaRebind) {
                ...
                if (hotkey.triggerOnRelease) {
                    if (isKeyDown) {
                        ...
                        g_triggerOnReleasePending.insert(hotkeyId);
                        if (s_enableHotkeyDebug) {
                            Log("[Hotkey] Trigger-on-release hotkey pressed, added to pending: " + hotkeyId);
                        }
                        if (blockKey) return {true, 0};
                        return {true, CallWindowProc(g_originalWndProc, hWnd, uMsg, wParam, lParam)};
                    } else {
                ...
                if (hotkey.triggerOnRelease != isKeyDown) {
                    ...
                    if (debounced) {
                        if (s_enableHotkeyDebug) {
                            Log("[Hotkey] Main hotkey matched but debounced: " + hotkeyId);
                        }
                        if (blockKey) return {true, 0};
                        return {true, CallWindowProc(g_originalWndProc, hWnd, uMsg, wParam, lParam)};
                    }
                    ...
                    if (!targetMode.empty()) {
                        SwitchToMode(targetMode, "main hotkey");
                    }
                }
                if (blockKey) return {true, 0};
                return {true, CallWindowProc(g_originalWndProc, hWnd, uMsg, wParam, lParam)};
    ...
    for (size_t sensIdx = 0; sensIdx < cfg.sensitivityHotkeys.size(); ++sensIdx) {
        ...
        if (!conditionsMet) {
            if (s_enableHotkeyDebug) {
                Log("[Hotkey] SKIP sensitivity: Game state conditions not met");
            }
            continue;
        }
...

        {
            ...
            if (matched || matchedViaRebind) {
                ...
                if (debounced) {
                    if (s_enableHotkeyDebug) {
                        Log("[Hotkey] Sensitivity hotkey matched but debounced: " + hotkeyId);
                    }
                    if (blockKey) return {true, 0};
                    return {true, CallWindowProc(g_originalWndProc, hWnd, uMsg, wParam, lParam)};
                }
                ...
                if (sensHotkey.toggle) {
                    ...
                    if (g_tempSensitivityOverride.active && g_tempSensitivityOverride.activeSensHotkeyIndex == static_cast<int>(sensIdx)) {
                        ...
                        if (s_enableHotkeyDebug) {
                            Log("[Hotkey] ✓✓✓ SENSITIVITY HOTKEY TOGGLED OFF: " + hotkeyId);
                        }
...
static LPARAM BuildKeyboardMessageLParam(UINT scanCodeWithFlags, bool isKeyDown, bool isSystemKey, UINT repeatCount, bool previousKeyState, bool transitionState) {
    const UINT scanLow    = scanCodeWithFlags & 0xFF;
    const bool isExtended = (scanCodeWithFlags & 0xFF00) != 0;
...
static UINT ResolveOutputScanCode(DWORD outputVk, UINT configuredScanCodeWithFlags) {
    if (configuredScanCodeWithFlags == 0) {
        return GetScanCodeWithExtendedFlag(outputVk);
    }

    if ((configuredScanCodeWithFlags & 0xFF00) == 0) {
        UINT vkScan = GetScanCodeWithExtendedFlag(outputVk);
        if ((vkScan & 0xFF00) != 0 && ((vkScan & 0xFF) == (configuredScanCodeWithFlags & 0xFF))) {
            return vkScan;
        }
    }
...
InputHandlerResult HandleKeyRebinding(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    if (isMouseButton && g_showGui.load(std::memory_order_acquire)) {
        return {false, 0};
    }
    ...
    if (!rebindCfg || !rebindCfg->keyRebinds.enabled) {
        return {false, 0};
    }
...
InputHandlerResult HandleCustomKeyNoRebind(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    if (uMsg != WM_TOOLSCREEN_KEYDOWN_NO_REBIND && uMsg != WM_TOOLSCREEN_KEYUP_NO_REBIND) {
        return {false, 0};
    }
    ...
    if (g_originalWndProc) {
        return {true, CallWindowProc(g_originalWndProc, hWnd, forwardedMsg, wParam, lParam)};
    }
...
InputHandlerResult HandleCustomCharNoRebind(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    if (uMsg != WM_TOOLSCREEN_CHAR_NO_REBIND) {
        return {false, 0};
    }
    ...
    if (g_originalWndProc) {
        return {true, CallWindowProc(g_originalWndProc, hWnd, WM_CHAR, wParam, lParam)};
    }
...
InputHandlerResult HandleCharRebinding(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    if (!charRebindCfg || !charRebindCfg->keyRebinds.enabled) {
        return {false, 0};
    }
