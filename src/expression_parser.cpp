   ...
   public:
    ...
    ExprToken next() {
        ...
        if (m_pos >= m_input.size()) {
            return ExprToken(ExprTokenKind::End, "", 0);
        }
   ...
   public:
    ...
    std::string validate() {
        try {
            parseExpression();
            if (m_currentToken.kind != ExprTokenKind::End) {
                return "Unexpected token at end: " + m_currentToken.text;
            }
            return "";
...
void RecalculateExpressionDimensions() {
    ...
    for (auto& mode : g_config.modes) {
        ...
        if (mode.id == "Thin" && mode.width < 330) {
            mode.width = 330;
        }
    }
    ...
    for (auto& mode : g_config.modes) {
        if (!eyezoomMode && mode.id == "EyeZoom") {
            eyezoomMode = &mode;
        }
        if (!preemptiveMode && mode.id == "Preemptive") {
            preemptiveMode = &mode;
        }
    }
