...
std::string GetHotkeySecondaryMode(size_t hotkeyIndex) {
    ...
    if (hotkeyIndex < g_hotkeySecondaryModes.size()) {
        return g_hotkeySecondaryModes[hotkeyIndex];
    }
    return "";
}
...
void SetHotkeySecondaryMode(size_t hotkeyIndex, const std::string& mode) {
    std::lock_guard<std::mutex> lock(g_hotkeySecondaryModesMutex);
    if (hotkeyIndex >= g_hotkeySecondaryModes.size()) {
        g_hotkeySecondaryModes.resize(hotkeyIndex + 1);
    }
...
void ResetAllHotkeySecondaryModes() {
    ...
    for (size_t i = 0; i < g_config.hotkeys.size(); ++i) {
        g_hotkeySecondaryModes[i] = g_config.hotkeys[i].secondaryMode;
    }
}
...
void ResetAllHotkeySecondaryModes(const Config& config) {
    ...
    for (size_t i = 0; i < config.hotkeys.size(); ++i) {
        g_hotkeySecondaryModes[i] = config.hotkeys[i].secondaryMode;
    }
}
...
// Lock-free mode ID access (double-buffered) - input handlers read from these
// without locking
...
std::chrono::high_resolution_clock::time_point g_lastFrameEndTime =
    std::chrono::high_resolution_clock::now();
std::mutex g_fpsLimitMutex;
HANDLE     g_highResTimer = NULL;
int        g_originalWindowsMouseSpeed =
    0;  // Original Windows mouse speed to restore on exit
...
bool SubclassGameWindow(HWND hwnd) {
    ...
    if (currentSubclassed != NULL && currentSubclassed != hwnd) {
        Log("Window handle changed from " +
            std::to_string(reinterpret_cast<uintptr_t>(currentSubclassed)) +
            " to " + std::to_string(reinterpret_cast<uintptr_t>(hwnd)) +
            " (likely fullscreen toggle)");
...
template <typename T>
bool CreateHookOrDie(LPVOID pTarget, LPVOID pDetour, T** ppOriginal, const char* hookName) {
    if (pTarget == NULL) {
        std::string warnMsg = std::string("WARNING: ") + hookName +
                              " function not found (NULL pointer)";
        Log(warnMsg);
        return false;
    }
    if (MH_CreateHook(pTarget, pDetour, reinterpret_cast<void**>(ppOriginal)) !=
        MH_OK) {
...
// REQUIRES: g_configMutex and g_hotkeyMainKeysMutex must already be held by
// caller
void RebuildHotkeyMainKeys_Internal() {
    ...
    for (const auto& sensHotkey : g_config.sensitivityHotkeys) {
        addMainKey(sensHotkey.keys);
    }
...
// This version acquires both required locks - use when you don't already hold
// them
...
void ApplyWindowsMouseSpeed() {
    ...
    if (targetSpeed == 0) {
        if (g_windowsMouseSpeedApplied.load()) {
            if (SystemParametersInfo(SPI_SETMOUSESPEED, 0, reinterpret_cast<void*>(static_cast<intptr_t>(g_originalWindowsMouseSpeed)), 0)) {
                Log("Restored Windows mouse speed to: " +
                    std::to_string(g_originalWindowsMouseSpeed));
            }
    ...
    if (SystemParametersInfo(
            SPI_SETMOUSESPEED, 0,
            reinterpret_cast<void*>(static_cast<intptr_t>(targetSpeed)), 0
        )) {
        g_windowsMouseSpeedApplied.store(true);
        Log("Applied Windows mouse speed: " + std::to_string(targetSpeed));
    } else {
        Log("WARNING: Failed to set Windows mouse speed to: " +
            std::to_string(targetSpeed));
    }
}
...
void SaveOriginalKeyRepeatSettings() {
    ...
    if (SystemParametersInfo(SPI_GETFILTERKEYS, sizeof(FILTERKEYS), &g_originalFilterKeys, 0)) {
        ...
        LogCategory("init", "Saved original FILTERKEYS: flags=0x" + std::to_string(g_originalFilterKeys.dwFlags) + ", iDelayMSec=" + std::to_string(g_originalFilterKeys.iDelayMSec) + ", iRepeatMSec=" + std::to_string(g_originalFilterKeys.iRepeatMSec));
    } else {
...
void ApplyKeyRepeatSettings() {
    ...
    fk.iWaitMSec  = 0;
    fk.iDelayMSec =
        (startDelay > 0) ? startDelay : g_originalFilterKeys.iDelayMSec;
    fk.iRepeatMSec =
        (repeatDelay > 0) ? repeatDelay : g_originalFilterKeys.iRepeatMSec;
    fk.iBounceMSec = 0;

    if (SystemParametersInfo(SPI_SETFILTERKEYS, sizeof(FILTERKEYS), &fk, 0)) {
        g_filterKeysApplied.store(true);
        Log("Applied key repeat settings: startDelay=" +
            std::to_string(fk.iDelayMSec) +
...
// Additional glViewport hook chains for compatibility with driver-level
// entrypoints and
...
typedef void(WINAPI* GLBLITNAMEDFRAMEBUFFERPROC)(
...
typedef void(APIENTRY* PFNGLBLITFRAMEBUFFERPROC_HOOK)(
...
BOOL WINAPI hkClipCursor(const RECT* lpRect) {
    return ClipCursorHook_Impl(oClipCursor, lpRect);
}
...
static HCURSOR SetCursorHook_Impl(SETCURSORPROC next, HCURSOR hCursor) {
    ...
    std::string localGameState = g_gameStateBuffers[g_currentGameStateIndex.load(
        std::memory_order_acquire
    )];

    if (g_showGui.load()) {
        const CursorTextures::CursorData* cursorData =
            CursorTextures::GetSelectedCursor(localGameState, 64);
        if (cursorData && cursorData->hCursor) {
            return next(cursorData->hCursor);
        }
    }

    if (g_specialCursorHandle.load() != NULL) {
        return next(hCursor);
    }
    ...
    if (GetIconInfo(hCursor, &ii)) {
        ...
        if (maskHash == "773ff800") {
            Log("hkSetCursor: Detected special cursor (maskHash=773ff800), caching "
                "for later use");
...
HCURSOR WINAPI hkSetCursor(HCURSOR hCursor) {
    return SetCursorHook_Impl(oSetCursor, hCursor);
}
...
static bool GetLatestViewportForHook(int& outModeW, int& outModeH, bool& outStretchEnabled, int& outStretchX, int& outStretchY, int& outStretchW, int& outStretchH) {
...
static inline void ViewportHook_Impl(GLVIEWPORTPROC next, GLint x, GLint y, GLsizei width, GLsizei height) {
    ...
    const ViewportTransitionSnapshot& transitionSnap =
        g_viewportTransitionSnapshots[g_viewportTransitionSnapshotIndex.load(
            std::memory_order_acquire
        )];
    bool isTransitionActive = transitionSnap.active;

    // Lock-free read of cached mode viewport data (updated by logic_thread)
    const CachedModeViewport& cachedMode =
        g_viewportModeCache[g_viewportModeCacheIndex.load(
            std::memory_order_acquire
        )];

    // The snapshot is updated synchronously on mode switch, while cache has ~16ms
    // lag.
    ...
    if (isTransitionActive && (!widthMatches || !heightMatches)) {
        widthMatches = widthMatches || (width == transitionSnap.fromNativeWidth) ||
                       (width == transitionSnap.toNativeWidth);
        heightMatches = heightMatches ||
                        (height == transitionSnap.fromNativeHeight) ||
                        (height == transitionSnap.toNativeHeight);
    }

    if (!posValid || !widthMatches || !heightMatches) {
        Log("Returning because viewport parameters don't match mode (x=" +
            ...
            ", modeWidth=" + std::to_string(modeWidth) + ", modeHeight=" +
            std::to_string(modeHeight) + ")");
        return next(x, y, width, height);
    }
    ...
    // Check if mode transition animation is active (from snapshot - no lock
    // needed)
    ...
    // Convert Y coordinate from Windows screen space (top-left origin) to OpenGL
    // viewport space (bottom-left origin)
...
// Driver-level glViewport hook (wglGetProcAddress / GLEW-resolved function
// pointer).
...
void WINAPI hkglViewport_ThirdParty(GLint x, GLint y, GLsizei width, GLsizei height) {
    ...
    if (g_config.hookChainingNextTarget == HookChainingNextTarget::LatestHook) {
        next = g_oglViewportThirdParty
                   ? g_oglViewportThirdParty
                   : (oglViewport ? oglViewport : g_oglViewportDriver);
    }
    ViewportHook_Impl(next, x, y, width, height);
}

void WINAPI hkglBlitNamedFramebuffer(GLuint readFramebuffer, GLuint drawFramebuffer, GLint srcX0, GLint srcY0, GLint srcX1, GLint srcY1, GLint dstX0, GLint dstY0, GLint dstX1, GLint dstY1, GLbitfield mask, GLenum filter);
...
static void AttemptHookGlBlitNamedFramebufferViaGlew() {
    ...
    MH_STATUS st =
        MH_CreateHook(reinterpret_cast<void*>(pFunc), reinterpret_cast<void*>(&hkglBlitNamedFramebuffer), reinterpret_cast<void**>(&oglBlitNamedFramebuffer));
...
BOOL WINAPI hkSetCursorPos(int X, int Y) {
    return SetCursorPosHook_Impl(oSetCursorPos, X, Y);
}
...
BOOL WINAPI hkSetCursorPos_ThirdParty(int X, int Y) {
    ...
    if (g_config.hookChainingNextTarget == HookChainingNextTarget::LatestHook) {
        next =
            g_oSetCursorPosThirdParty ? g_oSetCursorPosThirdParty : oSetCursorPos;
    }
...
static void GlfwSetInputModeHook_Impl(GLFWSETINPUTMODE next, void* window, int mode, int value) {
    if (!next) return;
    if (mode != GLFW_CURSOR) {
        return next(window, mode, value);
    }

    if (value == GLFW_CURSOR_DISABLED) {
        g_capturingMousePos.store(CapturingState::DISABLED);
        // When GUI is open, don't actually disable/lock the cursor - let it move
        // freely
        if (g_showGui.load()) {
...
void hkglfwSetInputMode(void* window, int mode, int value) {
    GlfwSetInputModeHook_Impl(oglfwSetInputMode, window, mode, value);
}

void hkglfwSetInputMode_ThirdParty(void* window, int mode, int value) {
    GLFWSETINPUTMODE next = oglfwSetInputMode;
    if (g_config.hookChainingNextTarget == HookChainingNextTarget::LatestHook) {
        next = g_oglfwSetInputModeThirdParty ? g_oglfwSetInputModeThirdParty
                                             : oglfwSetInputMode;
    }
    GlfwSetInputModeHook_Impl(next, window, mode, value);
}

static UINT GetRawInputDataHook_Impl(GETRAWINPUTDATAPROC next, HRAWINPUT hRawInput, UINT uiCommand, LPVOID pData, PUINT pcbSize, UINT cbSizeHeader) {
    ...
    if (result == static_cast<UINT>(-1) || pData == nullptr ||
        uiCommand != RID_INPUT) {
        return result;
    }
    ...
    if (raw->header.dwType == RIM_TYPEMOUSE) {
        ...
        if (!sensitivityDetermined) {
            ...
            if (transitionSnap.active) {
                modeId = transitionSnap.toModeId;  // Target mode during transition
                                                   // (lock-free from snapshot)
            } else {
                // Lock-free read of current mode ID from double-buffer
                modeId = g_modeIdBuffers[g_currentModeIdIndex.load(
                    std::memory_order_acquire
                )];
            }

            // Check if the mode has a sensitivity override (use snapshot for thread
            // safety)
            auto              inputCfgSnap = GetConfigSnapshot();
            const ModeConfig* mode =
                inputCfgSnap ? GetModeFromSnapshot(*inputCfgSnap, modeId) : nullptr;
        ...
        if (!(raw->data.mouse.usFlags & MOUSE_MOVE_ABSOLUTE)) {
            ...
            const bool sensitivityChanged =
                !hasLastSensitivity ||
                (std::fabs(sensitivityX - lastSensitivityX) > 0.000001f) ||
            ...
            // Prevent stale remainder from one direction delaying opposite-direction
            // output.
            if (rawX != 0 && xAccum != 0.0f && ((rawX > 0) != (xAccum > 0.0f))) {
                xAccum = 0.0f;
            }
            if (rawY != 0 && yAccum != 0.0f && ((rawY > 0) != (yAccum > 0.0f))) {
                yAccum = 0.0f;
            }
            ...
            if (sensitivityX != 1.0f || sensitivityY != 1.0f) {
                ...
                if (roundedX > static_cast<float>(LONG_MAX))
                    roundedX = static_cast<float>(LONG_MAX);
                if (roundedX < static_cast<float>(LONG_MIN))
                    roundedX = static_cast<float>(LONG_MIN);
                if (roundedY > static_cast<float>(LONG_MAX))
                    roundedY = static_cast<float>(LONG_MAX);
                if (roundedY < static_cast<float>(LONG_MIN))
                    roundedY = static_cast<float>(LONG_MIN);
                ...
                // No scaling: avoid carrying stale fractional remainder across future
                // overrides.
                xAccum = 0.0f;
                yAccum = 0.0f;
            }
...
UINT WINAPI hkGetRawInputData_ThirdParty(HRAWINPUT hRawInput, UINT uiCommand, LPVOID pData, PUINT pcbSize, UINT cbSizeHeader) {
    ...
    if (g_config.hookChainingNextTarget == HookChainingNextTarget::LatestHook) {
        next = g_oGetRawInputDataThirdParty ? g_oGetRawInputDataThirdParty
                                            : oGetRawInputData;
    }
...
void WINAPI hkglBlitNamedFramebuffer(GLuint readFramebuffer, GLuint drawFramebuffer, GLint srcX0, GLint srcY0, GLint srcX1, GLint srcY1, GLint dstX0, GLint dstY0, GLint dstX1, GLint dstY1, GLbitfield mask, GLenum filter) {
    ...
    if (viewport.valid) {
        ...
        return oglBlitNamedFramebuffer(
            readFramebuffer, drawFramebuffer, srcX0, srcY0, srcX1, srcY1,
            viewport.stretchX, destY0_screen,
...
void AttemptAggressiveGlViewportHook() {
    ...
    // Strategy 1 (preferred): Hook via wglGetProcAddress (driver-specific
    // implementation)
        ...
        if (hOpenGL32) {
            PFN_wglGetProcAddress pwglGetProcAddress =
                reinterpret_cast<PFN_wglGetProcAddress>(
                    GetProcAddress(hOpenGL32, "wglGetProcAddress")
                );
            if (pwglGetProcAddress) {
                PROC pGlViewportWGL = pwglGetProcAddress("glViewport");
                if (pGlViewportWGL != NULL &&
                    reinterpret_cast<void*>(pGlViewportWGL) !=
                        reinterpret_cast<void*>(&hkglViewport) &&
                    reinterpret_cast<void*>(pGlViewportWGL) !=
                        reinterpret_cast<void*>(&hkglViewport_Driver) &&
                    reinterpret_cast<void*>(pGlViewportWGL) !=
                        reinterpret_cast<void*>(oglViewport)) {
                    Log("Attempting glViewport hook via wglGetProcAddress: " +
                        std::to_string(reinterpret_cast<uintptr_t>(pGlViewportWGL)));
                    GLVIEWPORTPROC pViewportFunc =
                        reinterpret_cast<GLVIEWPORTPROC>(pGlViewportWGL);
                    if (HookChain::TryCreateAndEnableHook(
                            reinterpret_cast<void*>(pViewportFunc),
                            reinterpret_cast<void*>(&hkglViewport_Driver),
                            reinterpret_cast<void**>(&g_oglViewportDriver),
                            "glViewport (wglGetProcAddress)"
                        )) {
                        g_glViewportHookedViaWGL.store(true);
                        g_glViewportDriverHookTarget.store(
                            reinterpret_cast<void*>(pViewportFunc),
                            std::memory_order_release
                        );
                        hooksCreated++;
                        Log("SUCCESS: glViewport hooked via wglGetProcAddress (driver "
                            "target)");
                    }
                }
            }
    ...
    if (g_glViewportDriverHookTarget.load(std::memory_order_acquire) == nullptr &&
        !g_glViewportHookedViaGLEW.load()) {
        GLVIEWPORTPROC pGlViewportGLEW = glViewport;
        if (pGlViewportGLEW != NULL &&
            reinterpret_cast<void*>(pGlViewportGLEW) !=
                reinterpret_cast<void*>(&hkglViewport) &&
            reinterpret_cast<void*>(pGlViewportGLEW) !=
                reinterpret_cast<void*>(&hkglViewport_Driver) &&
            reinterpret_cast<void*>(pGlViewportGLEW) !=
                reinterpret_cast<void*>(oglViewport)) {
            Log("Attempting glViewport hook via GLEW pointer: " +
                std::to_string(reinterpret_cast<uintptr_t>(pGlViewportGLEW)));
            if (HookChain::TryCreateAndEnableHook(
                    reinterpret_cast<void*>(pGlViewportGLEW),
                    reinterpret_cast<void*>(&hkglViewport_Driver),
                    reinterpret_cast<void**>(&g_oglViewportDriver),
                    "glViewport (GLEW pointer)"
                )) {
    ...
    Log("Aggressive glViewport hooking complete. Total additional hooks "
        "created: " +
        std::to_string(hooksCreated));
    Log("Total glViewport hook count: " +
        std::to_string(g_glViewportHookCount.load()));
}

GLuint CalculateGameTextureId() {
    ModeViewportInfo viewport = GetCurrentModeViewport();
    if (!viewport.valid) {
        Log("CalculateGameTextureId: Invalid viewport, cannot calculate texture "
            "ID");
        return UINT_MAX;
    }
    ...
    Log("CalculateGameTextureId: Looking for texture with dimensions " +
            ...
            if (g_gameVersion <= GameVersion(1, 16, 5)) {
                if (minFilter != GL_NEAREST || magFilter != GL_NEAREST ||
                    ...
                    Log("CalculateGameTextureId: Texture " + std::to_string(texId) +
                        " has matching dimensions but wrong parameters (minFilter=" +
                        std::to_string(minFilter) + ", magFilter=" +
                        std::to_string(magFilter) + ", wrapS=" + std::to_string(wrapS) +
                        ", wrapT=" + std::to_string(wrapT) + ")");
                    continue;
                }
            } else {
                /*
                if (minFilter != GL_NEAREST || magFilter != GL_NEAREST || wrapS !=
                GL_CLAMP || wrapT != GL_CLAMP) { Log("CalculateGameTextureId: Texture "
                + std::to_string(texId) + " has matching dimensions but wrong parameters
                (minFilter=" + std::to_string(minFilter) + ", magFilter=" +
                        std::to_string(magFilter) + ", wrapS=" + std::to_string(wrapS) +
                ", wrapT=" + std::to_string(wrapT) + ")"); continue;
                }*/
            }

            Log("CalculateGameTextureId: Found matching texture ID " +
                std::to_string(texId) + " with dimensions " + std::to_string(width) +
    ...
    Log("CalculateGameTextureId: No matching texture found in range 1-" +
        std::to_string(maxCheckRange));
    return UINT_MAX;
}
    ...
    try {
        if (!g_glewLoaded) {
            ...
            if (glewInit() == GLEW_OK) {
                ...
                // Initialize shared OpenGL contexts for all worker threads (render,
                // mirror) This must be done BEFORE any thread starts to ensure all
                // contexts are in the same share group
                HGLRC currentContext = wglGetCurrentContext();
                if (currentContext) {
                    if (InitializeSharedContexts(currentContext, hDc)) {
                        LogCategory("init",
                                    "[RENDER] Shared contexts initialized - GPU texture "
                                    "sharing enabled for all threads");
                    } else {
                        Log("[RENDER] Shared context initialization failed - starting "
                            "worker threads in fallback mode");
                    }

                    // ALWAYS start worker threads. They will automatically use the
                    // pre-shared contexts if available,
        ...
        {
            static std::chrono::steady_clock::time_point s_lastViewportCompatCheck =
                std::chrono::steady_clock::now();
...

        {
            HGLRC currentContext = wglGetCurrentContext();
            HGLRC lastContext =
                g_lastSeenGameGLContext.load(std::memory_order_acquire);
            const uint64_t nowMs = GetTickCount64();

            // Some environments (mods/overlays/drivers) can temporarily switch WGL
            // contexts or alternate between multiple contexts. Restarting shared
            // contexts/threads immediately in that case can cause severe lag and make
            // mirrors/overlays appear to "stop" (threads never stabilize). Debounce
            // restarts.
            ...
            const bool contextChanged =
                (currentContext && lastContext && currentContext != lastContext);
            if (contextChanged) {
                ...
                const bool stableLongEnough =
                    (s_pendingSinceMs != 0) &&
                    ((nowMs - s_pendingSinceMs) >= kContextStableMs);
                const bool restartAllowed =
                    (s_lastRestartMs == 0) ||
                    ((nowMs - s_lastRestartMs) >= kMinRestartIntervalMs);
                if (stableLongEnough && restartAllowed) {
                    Log("[RENDER] Detected stable WGL context change - restarting shared "
                        "contexts/threads");
                    ...
                    if (InitializeSharedContexts(currentContext, hDc)) {
                        ...
                        Log("[RENDER] Failed to reinitialize shared contexts after context "
                            "change - restarting threads in fallback mode");
                    }
            ...
            // Thread health watchdog (rate-limited): if a worker thread
            // crashed/exited, try restarting it. This helps recover from transient
            // driver/ImGui/font/etc crashes that would otherwise leave mirrors black.
            {
                static uint64_t    s_lastHealthRestartAttemptMs = 0;
                constexpr uint64_t kHealthRestartIntervalMs     = 2000;
                if (currentContext && (s_lastHealthRestartAttemptMs == 0 ||
                                       (nowMs - s_lastHealthRestartAttemptMs) >=
                                           kHealthRestartIntervalMs)) {
                    s_lastHealthRestartAttemptMs = nowMs;

                    // Only restart capture thread when something actually consumes
                    // captures.
                    const bool needCaptureForMirrors =
                        (g_activeMirrorCaptureCount.load(std::memory_order_acquire) > 0);
                    const bool needCaptureForEyeZoom =
                        g_showEyeZoom.load(std::memory_order_relaxed) ||
                        g_isTransitioningFromEyeZoom.load(std::memory_order_relaxed);
                    const bool needCaptureForObsOrVc =
                        g_graphicsHookDetected.load(std::memory_order_acquire) ||
                        IsVirtualCameraActive();
                    const bool needCapture = needCaptureForMirrors ||
                                             needCaptureForEyeZoom ||
                                             needCaptureForObsOrVc;

                    if (!g_renderThreadRunning.load(std::memory_order_acquire)) {
                        StartRenderThread(currentContext);
                    }
                    if (needCapture &&
                        !g_mirrorCaptureRunning.load(std::memory_order_acquire)) {
                        StartMirrorCaptureThread(currentContext);
                    }
                }
            }
        ...
        // Start logic thread if not already running (handles OBS detection, hotkey
        // resets, etc.)
        if (!g_logicThreadRunning.load() && g_configLoaded.load()) {
            StartLogicThread();
        }

        // Early exit if config hasn't been loaded yet (prevents race conditions
        // during startup)
        ...
        if (hwnd != g_minecraftHwnd.load()) {
            g_minecraftHwnd.store(hwnd);
        }

        // This copy is expensive: it blits the full game texture + inserts fences +
        // flushes.
        {
            const bool needCaptureForMirrors =
                (g_activeMirrorCaptureCount.load(std::memory_order_acquire) > 0);
            ...
            const bool needCapture = needCaptureForMirrors || needCaptureForEyeZoom ||
                                     needCaptureForObsOrVc;
            if (needCapture) {
                static auto s_lastMirrorOnlyCaptureSubmit =
                ...
                GLuint gameTexture =
                    g_cachedGameTextureId.load(std::memory_order_acquire);
                if (gameTexture != UINT_MAX) {
                    ModeViewportInfo viewport = GetCurrentModeViewport();
                    if (viewport.valid) {
                        if (needCaptureForMirrors && !needCaptureForEyeZoom &&
                            !needCaptureForObsOrVc) {
                            const int maxMirrorFps =
                                g_activeMirrorCaptureMaxFps.load(std::memory_order_acquire);
                            if (maxMirrorFps > 0) {
                                const auto   now = std::chrono::steady_clock::now();
                                const double intervalMsD =
                                    1000.0 / static_cast<double>((std::max)(1, maxMirrorFps));
                                ...
                                const bool dimsChanged =
                                    (viewport.width != s_lastMirrorOnlyW) ||
                                    (viewport.height != s_lastMirrorOnlyH);

                                if (!dimsChanged &&
                                    s_lastMirrorOnlyCaptureSubmit.time_since_epoch().count() !=
                                        0) {
                        ...
                        // Sync screen/game geometry for capture thread to compute render
                        // cache.
                        ...
                        // SubmitFrameCapture already inserts its own fences and flushes
                        // after them; avoid an extra glFlush here (it can reduce FPS by
                        // forcing more driver work per frame).
                        if (allowCaptureThisFrame) {
        ...
        // Mark safe capture window - capture thread can now safely read the game
        // texture
        g_safeToCapture.store(true, std::memory_order_release);

        bool shouldCheckSubclass =
            (g_gameVersion < GameVersion(1, 13, 0)) || (g_originalWndProc == NULL);
...

        {
            ...
            if (showTextureGrid && g_glInitialized.load(std::memory_order_acquire) &&
        ...
        // Note: Hotkey secondary mode reset on world exit is now handled by the
        // logic thread
        ...
        if (!isFull) {
            ...
            if (isPre113) {
                ...
                if (viewport.valid && viewport.stretchWidth > 0 &&
                    viewport.stretchHeight > 0) {
        ...
        std::string lastFrameModeIdCopy =
            g_lastFrameModeIdBuffers[g_lastFrameModeIdIndex.load(
                std::memory_order_acquire
            )];
        ...
        if (IsModeTransitionActive()) {
            ...
            g_isTransitioningMode = true;
            Log("Mode transition detected (no animation): " + lastFrameModeIdCopy +
                " -> " + desiredModeId);
        ...
        std::string localGameState =
            g_gameStateBuffers[g_currentGameStateIndex.load(
                std::memory_order_acquire
            )];
        ...
        if (showProfiler) {
            Profiler::GetInstance().MarkAsRenderThread();
        }
        ...
        bool showEyeZoomOnScreen =
            isEyeZoom || (isTransitioningFromEyeZoom && !hideAnimOnScreenEyeZoom);
        ...
        // Release ensures all preceding EyeZoom stores are visible when the reader
        // acquires this value
        ...
        const bool needsDualRendering =
            g_graphicsHookDetected.load(std::memory_order_acquire) ||
            IsVirtualCameraActive();

        {
            const bool modeSizesFullscreen =
                (modeToRenderCopy.width == fullW && modeToRenderCopy.height == fullH);
            const bool stretchIsFullscreen =
                (!modeToRenderCopy.stretch.enabled) ||
                (modeToRenderCopy.stretch.width == fullW &&
                 modeToRenderCopy.stretch.height == fullH &&
                 modeToRenderCopy.stretch.x == 0 && modeToRenderCopy.stretch.y == 0);
            const bool borderVisible =
                modeToRenderCopy.border.enabled && modeToRenderCopy.border.width > 0;

            const bool anyModeOverlaysConfigured =
                (!modeToRenderCopy.mirrorIds.empty() ||
                 !modeToRenderCopy.mirrorGroupIds.empty() ||
                 (g_imageOverlaysVisible.load(std::memory_order_acquire) &&
                  !modeToRenderCopy.imageIds.empty()) ||
                 (g_windowOverlaysVisible.load(std::memory_order_acquire) &&
                  !modeToRenderCopy.windowOverlayIds.empty()));

            const bool anyImGuiOrDebugOverlay =
                shouldRenderGui || showPerformanceOverlay || showProfiler ||
                showEyeZoomOnScreen || frameCfg.debug.showTextureGrid;

            const bool anyOtherCustomOutput =
                frameCfg.debug.fakeCursor ||
                g_screenshotRequested.load(std::memory_order_relaxed);

            const bool canSkipCustomRender =
                g_glInitialized.load(std::memory_order_acquire) && isFull &&
                !needsDualRendering && !IsModeTransitionActive() &&
                modeSizesFullscreen && stretchIsFullscreen && !borderVisible &&
                !anyModeOverlaysConfigured && !anyImGuiOrDebugOverlay &&
                !anyOtherCustomOutput;
            ...
            if (canSkipCustomRender) {
                ...
                if (targetFPS > 0 && g_highResTimer) {
                    ...
                    if (timeToWaitUs > 0) {
                        ...
                        g_lastFrameEndTime = g_lastFrameEndTime +
                ...
                if (frameCfg.debug.delayRenderingUntilFinished) {
                    glFinish();
                }
                if (frameCfg.debug.delayRenderingUntilBlitted) {
                ...
                std::chrono::duration<double, std::milli> swapDuration =
                ...
                {
                    int nextIndex =
                        1 - g_lastFrameModeIdIndex.load(std::memory_order_relaxed);
        ...
        if (!g_glInitialized.load(std::memory_order_acquire)) {
            ...
            if (!g_glInitialized.load(std::memory_order_acquire)) {
                Log("FATAL: GPU resource initialization failed. Aborting custom render "
                    "for this frame.");
                g_safeToCapture.store(false, std::memory_order_release);
                return next(hDc);
            }
        ...
        // Note: Game state reset (wall/title/waiting) is now handled by
        // logic_thread
        ...
        bool hideAnimOnScreen =
            frameCfg.hideAnimationsInGame && IsModeTransitionActive();

        {
            PROFILE_SCOPE_CAT("Normal Mode Handling", "Rendering");

            if (needsDualRendering) {
                // Submit animated frame to render thread for OBS capture using helper
                // function
                {
                    PROFILE_SCOPE_CAT("Submit OBS Frame", "OBS");

                    // Build lightweight context struct (no lock-free reads needed here -
                    ...
                    submission.context.relativeStretching =
                        modeToRenderCopy.relativeStretching;
                    ...
                    submission.context.isTransitioningFromEyeZoom =
                        isTransitioningFromEyeZoom;
                    submission.context.eyeZoomAnimatedViewportX =
                        eyeZoomAnimatedViewportX;
                    submission.context.eyeZoomSnapshotTexture =
                        GetEyeZoomSnapshotTexture();
                    ...
                    submission.gameTextureFence =
                        glFenceSync(GL_SYNC_GPU_COMMANDS_COMPLETE, 0);
                    glFlush();

                    // Submit lightweight context - render thread will call
                    // BuildObsFrameRequest
        ...
        // All ImGui rendering is handled by render thread (via FrameRenderRequest
        // ImGui state fields) Screenshot handling stays on main thread since it
        // needs direct backbuffer access
        ...
        // Render fake cursor overlay if enabled (MUST be after RestoreGLState)
        {
            ...
            if (fakeCursorEnabled) {
                ...
                if (IsCursorVisible()) {
                    RenderFakeCursor(hwnd, windowWidth, windowHeight);
                }
            }
        }
        ...
        // We're the only writer on this thread, so no lock needed - just atomic
        // swap
        {
            int nextIndex =
                1 - g_lastFrameModeIdIndex.load(std::memory_order_relaxed);
            g_lastFrameModeIdBuffers[nextIndex] = desiredModeId;
            g_lastFrameModeIdIndex.store(nextIndex, std::memory_order_release);
            g_lastFrameModeId = desiredModeId;  // Keep legacy variable in sync (no
                                                // lock needed - single writer)
        }
        ...
        if (targetFPS > 0 && g_highResTimer) {
            ...
            auto targetTime =
                g_lastFrameEndTime +
                std::chrono::microseconds(static_cast<long long>(targetFrameTimeUs));
            auto now = std::chrono::high_resolution_clock::now();

            if (now < targetTime) {
                auto timeToWaitUs =
                    std::chrono::duration_cast<std::chrono::microseconds>(targetTime - now)
                        .count();
        ...
        // Optionally wait for the async overlay blit fence to complete before
        // SwapBuffers
        if (frameCfg.debug.delayRenderingUntilBlitted) {
            WaitForOverlayBlitFence();
        }
        ...
        // Update last frame mode ID for next frame's viewport calculations
        // (lock-free)
        {
            int nextIndex =
                1 - g_lastFrameModeIdIndex.load(std::memory_order_relaxed);
...
BOOL WINAPI hkwglSwapBuffers(HDC hDc) {
    return SwapBuffersHook_Impl(owglSwapBuffers, hDc);
}

BOOL WINAPI hkwglSwapBuffers_ThirdParty(HDC hDc) {
    WGLSWAPBUFFERS next = owglSwapBuffers;
    if (g_config.hookChainingNextTarget == HookChainingNextTarget::LatestHook) {
        next = g_owglSwapBuffersThirdParty ? g_owglSwapBuffersThirdParty
                                           : owglSwapBuffers;
    }
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved) {
    if (ul_reason_for_call == DLL_PROCESS_ATTACH) {
        ...
        GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT, (LPCSTR)DllMain, &g_hModule);
        ...
        g_highResTimer = CreateWaitableTimerExW(
            NULL, NULL, CREATE_WAITABLE_TIMER_HIGH_RESOLUTION, TIMER_ALL_ACCESS
        );
        if (g_highResTimer) {
            LogCategory("init",
                        "High-resolution waitable timer created successfully for FPS "
                        "limiting.");
        } else {
            Log("Warning: Failed to create high-resolution waitable timer. FPS "
                "limiting may be less precise.");
        }
        ...
        if (!g_toolscreenPath.empty()) {
            ...
            if (GetFileAttributesW(latestLogPath.c_str()) !=
                ...
                if (hFile != INVALID_HANDLE_VALUE) {
                    ...
                    if (GetFileTime(hFile, NULL, NULL, &lastWriteTime)) {
                        ...
                        std::wstring archivedLogPath =
                            logsDir + L"\\" + timestamp + L".log";

                        CloseHandle(hFile);

                        if (GetFileAttributesW(archivedLogPath.c_str()) !=
                            INVALID_FILE_ATTRIBUTES) {
                            for (int counter = 1; counter < 100; counter++) {
                                std::wstring altPath = logsDir + L"\\" + timestamp + L"_" +
                                                       std::to_wstring(counter) + L".log";
                                if (GetFileAttributesW(altPath.c_str()) ==
                                    INVALID_FILE_ATTRIBUTES) {
                        ...
                        if (!MoveFileW(latestLogPath.c_str(), archivedLogPath.c_str())) {
                            Log("WARNING: Could not rename old log to " +
                                WideToUtf8(archivedLogPath) +
        ...
        LogVersionInfo();
        if (g_toolscreenPath.empty()) {
            Log("FATAL: Could not get toolscreen directory.");
        }
        ...
        if (g_gameVersion.valid) {
            ...
            std::ostringstream oss;
            oss << "Game version " << g_gameVersion.major << "."
                << g_gameVersion.minor << "." << g_gameVersion.patch;
            if (inRange) {
        ...
        if (GetCurrentDirectoryW(MAX_PATH, dir) > 0) {
            ...
            bool  stateOutputAvailable = (stateFileAttrs != INVALID_FILE_ATTRIBUTES) &&
                                        !(stateFileAttrs & FILE_ATTRIBUTE_DIRECTORY);
            g_isStateOutputAvailable.store(stateOutputAvailable, std::memory_order_release);
            if (!stateOutputAvailable) {
                LogCategory(
                    "init",
                    "WARNING: wpstateout.txt not found. Game-state hotkey restrictions "
                    "will not apply until State Output is installed."
                );
            }
        } else {
        ...
        // initialization (locale facets, errno, etc.). CreateThread skips CRT init
        // which
...
#define HOOK(mod, name) \
    CreateHookOrDie(GetProcAddress(mod, #name), &hk##name, &o##name, #name)
        ...
        if (hGlfw) {
            ...
            LogCategory(
                "init",
                "WARNING: glfw.dll not loaded; skipping glfwSetInputMode hook"
            );
        }
#undef HOOK

        LPVOID pGlBlitNamedFramebuffer =
            GetProcAddress(hOpenGL32, "glBlitNamedFramebuffer");
        if (pGlBlitNamedFramebuffer != NULL) {
            ...
            LogCategory("init",
                        "WARNING: glBlitNamedFramebuffer not found in opengl32.dll - "
                        "will attempt to hook via GLEW after context init");
        }
        ...
        // This thread periodically detects those detours (prolog or IAT) and chains
        // behind them.
        g_stopHookCompat.store(false, std::memory_order_release);
        g_hookCompatThread = std::thread([]() {
            std::this_thread::sleep_for(std::chrono::milliseconds(500));
            while (!g_stopHookCompat.load(std::memory_order_acquire) &&
                   !g_isShuttingDown.load(std::memory_order_acquire)) {
                HookChain::RefreshAllThirdPartyHookChains();

                for (int i = 0; i < 20; i++) {
                    if (g_stopHookCompat.load(std::memory_order_acquire) ||
                        g_isShuttingDown.load(std::memory_order_acquire))
                        break;
        ...
        if (g_imageMonitorThread.joinable()) {
            g_imageMonitorThread.join();
        }

        // Stop hook compatibility monitor thread
        g_stopHookCompat.store(true, std::memory_order_release);
        if (g_hookCompatThread.joinable()) {
            g_hookCompatThread.join();
        }
