if (ImGui::BeginTabItem("Inputs")) {
  ...
  if (ImGui::BeginTabBar("InputsSubTabs")) {
    if (ImGui::BeginTabItem("Mouse")) {
      ...
      if (ImGui::SliderFloat("##mouseSensitivity", &g_config.mouseSensitivity,
                             0.001f, 10.0f, "%.3fx")) {
        g_configIsDirty = true;
      }
      ImGui::SameLine();
      HelpMarker(
          "Multiplies mouse movement for raw input events (mouselook).\n"
      ...
      ImGui::SameLine();
      HelpMarker(
          "Temporarily overrides Windows mouse speed setting while game is "
          "running.\n"
          "0 = Disabled (use system setting)\n"
          "1-20 = Override Windows mouse speed (10 = default Windows speed)\n"
          "Affects cursor movement in game menus. Original setting is restored "
          "on exit.");

      if (g_gameVersion < GameVersion(1, 13, 0)) {
        if (ImGui::Checkbox("Let Cursor Escape Window",
                            &g_config.allowCursorEscape)) {
          g_configIsDirty = true;
        }
        ImGui::SameLine();
        HelpMarker(
            "For pre 1.13, prevents the cursor being locked to the game window "
            "when in fullscreen");
      }
      ...
      if (ImGui::Checkbox("Enable Custom Cursors", &g_config.cursors.enabled)) {
        g_configIsDirty = true;
        // Schedule cursor reload (will happen outside GUI rendering to avoid
        // deadlock)
        g_cursorsNeedReload = true;
      }
      ImGui::SameLine();
      HelpMarker(
          "When enabled, the mouse cursor will change based on the current "
          "game state.");
      ...
      if (g_config.cursors.enabled) {
        ...
        for (int i = 0; i < 3; ++i) {
          ...
          if (ImGui::BeginCombo("##cursor", currentCursorName)) {
            for (const auto& option : availableCursors) {
              ...
              if (ImGui::Selectable(option.name.c_str(), is_selected)) {
                ...
                g_configIsDirty = true;
                // Schedule cursor reload (will happen outside GUI rendering to
                // avoid deadlock)
                g_cursorsNeedReload = true;

                std::wstring cursorPath;
                UINT loadType = IMAGE_CURSOR;
                CursorTextures::GetCursorPathByName(option.key, cursorPath,
                                                    loadType);

                const CursorTextures::CursorData* cursorData =
                    CursorTextures::LoadOrFindCursor(cursorPath, loadType,
                                                     cursorConfig.cursorSize);
              ...
              if (ImGui::IsItemHovered()) {
                ImGui::BeginTooltip();
                ImGui::TextColored(ImVec4(1.0f, 1.0f, 0.6f, 1.0f), "%s",
                                   option.name.c_str());
                ImGui::Separator();
          ...
          if (ImGui::SliderInt("##cursorSize", &sliderValue, 8, 144, "%d px",
            ...
            if (newSize != cursorConfig.cursorSize) {
              ...
              std::wstring cursorPath;
              UINT loadType = IMAGE_CURSOR;
              CursorTextures::GetCursorPathByName(cursorConfig.cursorName,
                                                  cursorPath, loadType);
        ...
        if (ImGui::Button("Reset to Defaults##cursors")) {
          ImGui::OpenPopup("Reset Cursors to Defaults?");
        }

        if (ImGui::BeginPopupModal("Reset Cursors to Defaults?", NULL,
    ...
    if (ImGui::BeginTabItem("Keyboard")) {
      ...
      if (ImGui::SliderInt("##keyRepeatStartDelay", &startDelayValue, 0, 500,
                           startDelayValue == 0 ? "Default" : "%d ms")) {
      ...
      if (ImGui::SliderInt("##keyRepeatDelay", &repeatDelayValue, 0, 500,
                           repeatDelayValue == 0 ? "Default" : "%d ms")) {
      ...
      ImGui::SameLine();
      HelpMarker(
          "Time between repeated key presses while held.\n"
          "0 = Use Windows default, 1-500ms = custom delay.\n"
          "Only applied while the game window is focused.");
      ...
      ImGui::TextWrapped(
          "Intercept keyboard inputs and remap them before they reach the "
          "game.");
      ImGui::Spacing();

      if (ImGui::Checkbox("Enable Key Rebinding",
                          &g_config.keyRebinds.enabled)) {
        g_configIsDirty = true;
      ...
      ImGui::SameLine();
      HelpMarker(
          "When enabled, configured key rebinds will intercept keyboard input "
          "and send the remapped key to the game instead.");
      ...
      if (g_config.keyRebinds.enabled) {
        ...
        auto tryParseUnicodeInput = [&](const std::string& in,
                                        uint32_t& outCp) -> bool {
          ...
          if (startsWithI("U+"))
            hex = s.substr(2);
          else if (startsWithI("\\\\u") || startsWithI("\\\\U"))
            hex = s.substr(2);
          ...
          if (!w.empty()) {
            uint32_t cp = 0;
            if (w.size() >= 2 && w[0] >= 0xD800 && w[0] <= 0xDBFF &&
                w[1] >= 0xDC00 && w[1] <= 0xDFFF) {
              cp = 0x10000u + (((uint32_t)w[0] - 0xD800u) << 10) +
                   ((uint32_t)w[1] - 0xDC00u);
            } else {
              cp = (uint32_t)w[0];
            }
        ...
        auto syncUnicodeEditBuffers = [&]() {
          if (s_rebindUnicodeTextEdit.size() !=
              g_config.keyRebinds.rebinds.size()) {
        ...
        if (ImGui::Button("Open Keyboard Layout")) {
          s_keyboardLayoutOpen = true;
        }
        ImGui::SameLine();
        HelpMarker(
            "Shows a visual keyboard. Keys with a configured rebind are "
            "outlined.\n"
            "Each key shows the output scan code (large text) and the game "
            "keybind hotkey below it.\n"
        ...
        if (s_keyboardLayoutOpen) {
          ...
          const ImVec2 center =
              vp ? ImVec2(vp->WorkPos.x + vp->WorkSize.x * 0.5f,
        ...
        auto scanCodeToDisplayName = [&](DWORD scan,
                                         DWORD fallbackVk) -> std::string {
          if (scan == 0) {
            if (fallbackVk == VK_LBUTTON || fallbackVk == VK_RBUTTON ||
                fallbackVk == VK_MBUTTON || fallbackVk == VK_XBUTTON1 ||
        ...
        if (s_keyboardLayoutOpen) {
          ...
          if (keyboardLayoutWindowVisible) {
            ...
            const bool escapePressedThisFrame =
                ImGui::IsKeyPressed(ImGuiKey_Escape);
            bool layoutEscapeConsumedThisFrame = false;

            const bool anyPopupOpenNow =
                ImGui::IsPopupOpen(nullptr, ImGuiPopupFlags_AnyPopup);
            const bool contextPopupOpenNow =
                ImGui::IsPopupOpen("Rebind Config##layout") ||
                ImGui::IsPopupOpen("Triggers Custom##layout") ||
                ImGui::IsPopupOpen("Custom Unicode##layout");
            bool blockLayoutEscapeThisFrame = false;
            if (escapePressedThisFrame &&
                (anyPopupOpenNow || contextPopupOpenNow ||
                 s_layoutContextPopupWasOpenLastFrame)) {
...

            {
              ...
              if (ImGui::SliderFloat("##keyboardLayoutScalePct", &scalePct,
                                     60.0f, 300.0f, "%.0f%%",
                                     ImGuiSliderFlags_AlwaysClamp)) {
            ...
            // Make the keyboard region scrollable (both axes) to fit on smaller
            // windows.
            ...
            auto Spacer = [](float wUnits) -> KeyCell {
              return KeyCell{0, nullptr, wUnits};
            };
            auto Key = [](DWORD vk, float wUnits = 1.0f,
                          const char* overrideLabel = nullptr) -> KeyCell {
            ...
            const std::vector<std::vector<KeyCell>> rows = {
                {Key(VK_ESCAPE),
                 ...
                 Key(VK_F12),
                 Spacer(0.5f),
                 Key(VK_SNAPSHOT, 1.25f, "PRTSC"),
                 Key(VK_SCROLL, 1.25f, "SCRLK"),
                 Key(VK_PAUSE, 1.25f, "PAUSE")},
                ...
                {Key(VK_OEM_3, 1.0f, "`"),
                 ...
                 Key('9'),
                 Key('0'),
                 Key(VK_OEM_MINUS, 1.0f, "-"),
                 Key(VK_OEM_PLUS, 1.0f, "="),
                 Key(VK_BACK, 2.0f, "BACK"),
                 Spacer(0.5f),
                 Key(VK_INSERT, 1.25f, "INS"),
                 Key(VK_HOME, 1.25f, "HOME"),
                 Key(VK_PRIOR, 1.25f, "PGUP"),
                 Spacer(0.5f),
                 Key(VK_NUMLOCK, 1.25f, "NUMLK"),
                 Key(VK_DIVIDE, 1.25f, "/"),
                 Key(VK_MULTIPLY, 1.25f, "*"),
                 Key(VK_SUBTRACT, 1.25f, "-")},
                ...
                {Key(VK_TAB, 1.5f, "TAB"),
                 ...
                 Key(VK_OEM_5, 1.5f, "\\"),
                 Spacer(0.5f),
                 Key(VK_DELETE, 1.25f, "DEL"),
                 Key(VK_END, 1.25f, "END"),
                 Key(VK_NEXT, 1.25f, "PGDN"),
                 Spacer(0.5f),
                 Key(VK_NUMPAD7, 1.25f, "NUM7"),
                 Key(VK_NUMPAD8, 1.25f, "NUM8"),
                 Key(VK_NUMPAD9, 1.25f, "NUM9"),
                 Key(VK_ADD, 1.25f, "+")},

                {Key(VK_CAPITAL, 1.75f, "CAPS"),
                 Key('A'),
                 Key('S'),
                 Key('D'),
                 Key('F'),
                 ...
                 Key(VK_OEM_1, 1.0f, ";"),
                 Key(VK_OEM_7, 1.0f, "'"),
                 Key(VK_RETURN, 2.25f, "ENTER"),
                 Spacer(0.5f),
                 Spacer(1.25f),
                 Spacer(1.25f),
                 Spacer(1.25f),
                 Spacer(0.5f),
                 Key(VK_NUMPAD4, 1.25f, "NUM4"),
                 Key(VK_NUMPAD5, 1.25f, "NUM5"),
                 Key(VK_NUMPAD6, 1.25f, "NUM6"),
                 Spacer(1.25f)},

                {Key(VK_LSHIFT, 2.25f, "LSHIFT"),
                 Key('Z'),
                 Key('X'),
                 Key('C'),
                 Key('V'),
                 ...
                 Key('N'),
                 Key('M'),
                 Key(VK_OEM_COMMA, 1.0f, ","),
                 Key(VK_OEM_PERIOD, 1.0f, "."),
                 Key(VK_OEM_2, 1.0f, "/"),
                 Key(VK_RSHIFT, 2.75f, "RSHIFT"),
                 Spacer(0.5f),
                 Spacer(1.25f),
                 Key(VK_UP, 1.25f, "UP"),
                 Spacer(1.25f),
                 Spacer(0.5f),
                 Key(VK_NUMPAD1, 1.25f, "NUM1"),
                 Key(VK_NUMPAD2, 1.25f, "NUM2"),
                 Key(VK_NUMPAD3, 1.25f, "NUM3"),
                 Key(VK_RETURN, 1.25f, "ENTER")},

                {Key(VK_LCONTROL, 1.25f, "LCTRL"), Key(VK_LWIN, 1.25f, "LWIN"),
                 Key(VK_LMENU, 1.25f, "LALT"), Key(VK_SPACE, 6.25f, "SPACE"),
                 Key(VK_RMENU, 1.25f, "RALT"), Key(VK_RWIN, 1.25f, "RWIN"),
                 Key(VK_APPS, 1.25f, "APPS"), Key(VK_RCONTROL, 1.25f, "RCTRL"),
                 Spacer(0.5f), Key(VK_LEFT, 1.25f, "LEFT"),
                 Key(VK_DOWN, 1.25f, "DOWN"), Key(VK_RIGHT, 1.25f, "RIGHT"),
                 Spacer(0.5f), Key(VK_NUMPAD0, 2.5f, "NUM0"),
                 Key(VK_DECIMAL, 1.25f, "NUM."), Spacer(1.25f)},
            };
            ...
            const float keyboardScale =
                s_keyboardLayoutScale * kKeyboardScaleMult;

            const float keyH = roundPx(ImGui::GetFrameHeight() * kKeyHeightMul *
                                       keyboardScale);
            float unit = roundPx(keyH * kKeyUnitMul);
            unit = (float)(((int)(unit + 2.0f) / 4) * 4);
            if (unit < 20.0f) unit = 20.0f;
            float gap = roundPx(ImGui::GetStyle().ItemInnerSpacing.x *
                                keyboardScale * kKeyGapMul);
...

            {
              ...
              const ImVec2 plateMin = ImVec2(layoutStartScreen.x - platePad,
                                             layoutStartScreen.y - platePad);
              const ImVec2 plateMax =
                  ImVec2(layoutStartScreen.x + keyboardMaxRowW + platePad,
                         layoutStartScreen.y + keyboardTotalH - gap + platePad);

              dl->AddRectFilled(ImVec2(plateMin.x + 5, plateMin.y + 6),
                                ImVec2(plateMax.x + 5, plateMax.y + 6),
              ...
              const ImVec2 plateMid =
                  ImVec2(plateMax.x,
                         plateMin.y + (plateMax.y - plateMin.y) * plateSplit);
              dl->AddRectFilled(plateMin, plateMid, plateTop, plateRound,
                                ImDrawFlags_RoundCornersTop);
              dl->AddRect(plateMin, plateMax, IM_COL32(10, 10, 12, 255),
                          plateRound);
              dl->AddLine(ImVec2(plateMin.x + 6, plateMin.y + 6),
                          ImVec2(plateMax.x - 6, plateMin.y + 6),
            ...
            auto drawKeyCell = [&](DWORD vk, const char* label,
              ...
              auto themeForVk = [&](DWORD v) -> KeyTheme {
                const bool isMouse =
                    (v == VK_LBUTTON || v == VK_RBUTTON || v == VK_MBUTTON ||
                     v == VK_XBUTTON1 || v == VK_XBUTTON2);
                const bool isFn =
                    (v == VK_ESCAPE || (v >= VK_F1 && v <= VK_F24) ||
                     v == VK_SNAPSHOT || v == VK_SCROLL || v == VK_PAUSE);
                const bool isMod =
                    (v == VK_SHIFT || v == VK_LSHIFT || v == VK_RSHIFT ||
                     v == VK_CONTROL || v == VK_LCONTROL || v == VK_RCONTROL ||
                     v == VK_MENU || v == VK_LMENU || v == VK_RMENU ||
                     v == VK_LWIN || v == VK_RWIN || v == VK_TAB ||
                     v == VK_CAPITAL || v == VK_BACK || v == VK_RETURN ||
                     v == VK_SPACE || v == VK_APPS);
                const bool isNav =
                    (v == VK_INSERT || v == VK_DELETE || v == VK_HOME ||
                     v == VK_END || v == VK_PRIOR || v == VK_NEXT ||
                     v == VK_UP || v == VK_DOWN || v == VK_LEFT ||
                     v == VK_RIGHT);
                const bool isNum = (v >= VK_NUMPAD0 && v <= VK_NUMPAD9) ||
                                   v == VK_ADD || v == VK_SUBTRACT ||
                                   v == VK_MULTIPLY || v == VK_DIVIDE ||
                                   v == VK_DECIMAL || v == VK_NUMLOCK;

                KeyTheme t{IM_COL32(80, 86, 98, 255), IM_COL32(45, 49, 60, 255),
              ...
              const ImVec2 pressOff =
                  active ? ImVec2(0.0f, 1.2f * keyboardScale) : ImVec2(0, 0);

              dl->AddRectFilled(ImVec2(pMin.x + shadow, pMin.y + shadow),
                                ImVec2(pMax.x + shadow, pMax.y + shadow),
                                IM_COL32(0, 0, 0, 90), rounding);

              const ImVec2 kMin =
                  ImVec2(pMin.x + pressOff.x, pMin.y + pressOff.y);
              const ImVec2 kMax =
                  ImVec2(pMax.x + pressOff.x, pMax.y + pressOff.y);

              dl->AddRectFilled(kMin, kMax, theme.bottom, rounding);
              const float split = 0.70f;
              const ImVec2 kMid =
                  ImVec2(kMax.x, kMin.y + (kMax.y - kMin.y) * split);
              dl->AddRectFilled(kMin, kMid, theme.top, rounding,
                                ImDrawFlags_RoundCornersTop);

              dl->AddLine(ImVec2(kMin.x + 2, kMin.y + 1),
                          ImVec2(kMax.x - 2, kMin.y + 1),
                          IM_COL32(255, 255, 255, 18), 1.0f);

              dl->AddRect(kMin, kMax, theme.border, rounding, 0, 1.0f);

              if (rb && rb->fromKey != 0 && rb->toKey != 0) {
                const ImU32 outline = rb->enabled ? IM_COL32(0, 220, 110, 255)
                                                  : IM_COL32(255, 170, 0, 255);
                dl->AddRect(kMin, kMax, outline, rounding, 0, 3.0f);
                const ImU32 tint = rb->enabled ? IM_COL32(0, 190, 95, 30)
                                               : IM_COL32(255, 165, 0, 35);
              ...
              const bool hasConfigured =
                  (rb && rb->fromKey != 0 && rb->toKey != 0);
              const bool isNoOp = [&]() -> bool {
                if (!hasConfigured) return false;
                if (rb->toKey != vk) return false;
                if (rb->customOutputVK != 0 && rb->customOutputVK != rb->toKey)
                  return false;
              ...
              DWORD outScan = (rb && showRebindInfo && rb->useCustomOutput &&
                               rb->customOutputScanCode != 0)
                                  ? rb->customOutputScanCode
                                  : getScanCodeWithExtendedFlag(triggerVK);
              if (outScan != 0 && (outScan & 0xFF00) == 0) {
                DWORD derived = getScanCodeWithExtendedFlag(triggerVK);
                if ((derived & 0xFF00) != 0 &&
                    ((derived & 0xFF) == (outScan & 0xFF))) {
                  outScan = derived;
                }
              }
              ...
              const std::string primaryText =
                  showRebindInfo
                      ? ((rb->useCustomOutput && rb->customOutputUnicode != 0)
                             ? codepointToDisplay(
                                   (uint32_t)rb->customOutputUnicode)
                             : normalizeMouseButtonLabel(
                                   VkToString((rb->useCustomOutput &&
                                               rb->customOutputVK != 0)
                                                  ? rb->customOutputVK
                                                  : vk)))
                      : std::string(label);
              const std::string secondaryText =
                  showRebindInfo
                      ? normalizeMouseButtonLabel(
                            scanCodeToDisplayName(outScan, triggerVK))
                      : std::string();

              ImFont* fLabel = g_keyboardLayoutPrimaryFont
                                   ? g_keyboardLayoutPrimaryFont
                                   : ImGui::GetFont();
              auto snapPxText = [](float v) -> float {
              ...
              ImVec2 labelSz = fLabel->CalcTextSizeA(labelFontSize, FLT_MAX,
                                                     0.0f, primaryText.c_str());
              ...
              if (textAvailW > 8.0f) {
                ...
                labelFontSize =
                    snapFontSize(fLabel->LegacySize * layoutTextBoost * scaleW);
                labelSz = fLabel->CalcTextSizeA(labelFontSize, FLT_MAX, 0.0f,
                                                primaryText.c_str());
              }
              ...
              if (!showRebindInfo) {
                ...
                dl->AddText(fLabel, labelFontSize, ImVec2(x, y), theme.text,
                            primaryText.c_str());
              } else {
                ImFont* f = g_keyboardLayoutPrimaryFont
                                ? g_keyboardLayoutPrimaryFont
                                : ImGui::GetFont();
                ImFont* fSecondary = g_keyboardLayoutSecondaryFont
                                         ? g_keyboardLayoutSecondaryFont
                                         : f;

                float primaryFs = labelFontSize;
                ImVec2 primarySz = f->CalcTextSizeA(primaryFs, FLT_MAX, 0.0f,
                                                    primaryText.c_str());

                float secondaryFs = fSecondary->LegacySize * layoutTextBoost;
                ImVec2 secondarySz = fSecondary->CalcTextSizeA(
                    secondaryFs, FLT_MAX, 0.0f, secondaryText.c_str());
                if (textAvailW > 8.0f) {
                  float secScaleW = textAvailW / (secondarySz.x + 0.001f);
                  if (secScaleW < 0.55f) secScaleW = 0.55f;
                  if (secScaleW > 1.00f) secScaleW = 1.00f;
                  secondaryFs = snapFontSize(fSecondary->LegacySize *
                                             layoutTextBoost * secScaleW);
                  secondarySz = fSecondary->CalcTextSizeA(
                      secondaryFs, FLT_MAX, 0.0f, secondaryText.c_str());
                }
                ...
                if (textAvailH > 0.0f && totalH > textAvailH) {
                  ...
                  if (fit < 1.0f) {
                    ...
                    primarySz = f->CalcTextSizeA(primaryFs, FLT_MAX, 0.0f,
                                                 primaryText.c_str());
                    secondarySz = fSecondary->CalcTextSizeA(
                        secondaryFs, FLT_MAX, 0.0f, secondaryText.c_str());
                ...
                float startY =
                    snapPxText(kMin.y + padY + (textAvailH - totalH) * 0.5f);
                ...
                dl->AddText(f, primaryFs, ImVec2(x1, startY), theme.text,
                            primaryText.c_str());
                ...
                const ImU32 infoCol = (rb && !rb->enabled)
                                          ? IM_COL32(255, 220, 170, 235)
                                          : IM_COL32(245, 245, 245, 235);
                dl->AddText(fSecondary, secondaryFs, ImVec2(x2, y2), infoCol,
                            secondaryText.c_str());
              }
            ...
            for (size_t rowIdx = 0; rowIdx < rows.size(); ++rowIdx) {
              ...
              for (size_t colIdx = 0; colIdx < rows[rowIdx].size(); ++colIdx) {
                ...
                std::string keyName = kc.labelOverride
                                          ? std::string(kc.labelOverride)
                                          : VkToString(kc.vk);
                ...
                if (ImGui::IsItemHovered()) {
                  ...
                  if (rb && rb->fromKey != 0 && rb->toKey != 0) {
                    ...
                    DWORD triggerScanTip =
                        getScanCodeWithExtendedFlag(triggerVkTip);
                    DWORD outputVkTip =
                        (rb->useCustomOutput && rb->customOutputVK != 0)
                            ? rb->customOutputVK
                            : triggerVkTip;
                    const std::string typesTip =
                        (rb->useCustomOutput && rb->customOutputUnicode != 0)
                            ? codepointToDisplay(
                                  (uint32_t)rb->customOutputUnicode)
                            : normalizeMouseButtonLabel(
                                  VkToString(outputVkTip));
                    const std::string triggersTip = normalizeMouseButtonLabel(
                        scanCodeToDisplayName(triggerScanTip, triggerVkTip));
                    ImGui::Text("Types: %s", typesTip.c_str());
                    ImGui::Text("Triggers: %s", triggersTip.c_str());
                  } else {
                    ImGui::Text("%s (%u)", VkToString(kc.vk).c_str(),
                                (unsigned)kc.vk);
            ...
            auto drawTallKey = [&](DWORD vk, const char* label,
            ...
            if (haveNumpadPlusAnchor) {
              drawTallKey(VK_ADD, "+", numpadPlusAnchorMin,
                          numpadPlusAnchorMax);
            }
            if (haveNumpadEnterAnchor) {
              drawTallKey(VK_RETURN, "ENTER", numpadEnterAnchorMin,
                          numpadEnterAnchorMax);
            }

            const float mousePanelX =
                layoutStart.x + keyboardMaxRowW + unit * 0.9f;
...

            {
              ...
              ImGui::SetCursorPos(
                  ImVec2(mousePanelX, mousePanelY + mouseHeaderH));
              ...
              dl->AddRectFilled(bodyMin, bodyMax, IM_COL32(24, 26, 33, 255),
                                bodyR);
              dl->AddRect(bodyMin, bodyMax, IM_COL32(10, 10, 12, 255), bodyR, 0,
                          1.5f);
              ...
              auto drawMouseSegment = [&](DWORD vk, const char* label,
                                          const ImVec2& segMin,
                                          const ImVec2& segMax, float segR,
                                          ImDrawFlags segFlags) {
                const bool hovered = ImGui::IsItemHovered();
                const bool active = ImGui::IsItemActive();
                ...
                KeyTheme theme{
                    IM_COL32(88, 90, 108, 255), IM_COL32(44, 46, 60, 255),
                    IM_COL32(18, 18, 20, 255), IM_COL32(245, 245, 245, 255)};
                theme.bottom = adjust(theme.bottom, 10);
                if (hovered) {
                  theme.top = adjust(theme.top, 12);
                  theme.bottom = adjust(theme.bottom, 10);
                ...
                const KeyRebind* rb = findRebindForKey(vk);
                if (rb && rb->fromKey != 0 && rb->toKey != 0) {
                  const ImU32 outline = rb->enabled
                ...
                const ImVec2 tpos =
                    ImVec2(segMin.x + (segMax.x - segMin.x - tsz.x) * 0.5f,
                           segMin.y + (segMax.y - segMin.y - tsz.y) * 0.5f);
                ...
                if (ImGui::IsItemHovered()) {
                  ...
                  if (rb && rb->fromKey != 0 && rb->toKey != 0) {
                    ...
                    ImGui::Text("Input: %s (%u)", VkToString(vk).c_str(),
                                (unsigned)vk);
                    ImGui::TextUnformatted("Right-click to configure rebinds.");
                  }
                  ImGui::EndTooltip();
              ...
              const ImVec2 wheelMin =
                  ImVec2(midX - wheelW * 0.5f, innerMin.y + topH * 0.18f);
              const ImVec2 wheelMax =
                  ImVec2(midX + wheelW * 0.5f, wheelMin.y + wheelH);
              ...
              const ImVec2 rightMin =
                  ImVec2(wheelMax.x + wheelSideGap, innerMin.y);
              ...
              const float sideX0 =
                  innerMin.x + (innerMax.x - innerMin.x) * 0.07f;
              const float sideY0 =
                  innerMin.y + topH + (innerMax.y - innerMin.y - topH) * 0.26f;
              ...
              const ImVec2 side2Max =
                  ImVec2(sideX0 + sideW, sideY0 + sideH + sideGap + sideH);

              dl->AddLine(ImVec2(midX, innerMin.y + 2),
                          ImVec2(midX, splitY - 2), IM_COL32(10, 10, 12, 255),
                          1.0f);
              dl->AddLine(ImVec2(innerMin.x + 2, splitY),
                          ImVec2(innerMax.x - 2, splitY),
                          IM_COL32(10, 10, 12, 255), 1.0f);

              ImGui::SetCursorScreenPos(leftMin);
              ImGui::InvisibleButton(
                  "##mouse_left",
                  ImVec2(leftMax.x - leftMin.x, leftMax.y - leftMin.y),
                  ImGuiButtonFlags_MouseButtonLeft |
                      ImGuiButtonFlags_MouseButtonRight);
              drawMouseSegment(VK_LBUTTON, "MB1", leftMin, leftMax,
                               bodyR * 0.75f, ImDrawFlags_RoundCornersTopLeft);

              ImGui::SetCursorScreenPos(rightMin);
              ImGui::InvisibleButton(
                  "##mouse_right",
                  ImVec2(rightMax.x - rightMin.x, rightMax.y - rightMin.y),
                  ImGuiButtonFlags_MouseButtonLeft |
                      ImGuiButtonFlags_MouseButtonRight);
              drawMouseSegment(VK_RBUTTON, "MB2", rightMin, rightMax,
                               bodyR * 0.75f, ImDrawFlags_RoundCornersTopRight);

              ImGui::SetCursorScreenPos(wheelMin);
              ImGui::InvisibleButton(
                  "##mouse_mid",
                  ImVec2(wheelMax.x - wheelMin.x, wheelMax.y - wheelMin.y),
                  ImGuiButtonFlags_MouseButtonLeft |
                      ImGuiButtonFlags_MouseButtonRight);
              drawMouseSegment(VK_MBUTTON, "MB3", wheelMin, wheelMax,
                               6.0f * keyboardScale,
              ...
              ImGui::InvisibleButton(
                  "##mouse_x1",
                  ImVec2(side1Max.x - side1Min.x, side1Max.y - side1Min.y),
                  ImGuiButtonFlags_MouseButtonLeft |
                      ImGuiButtonFlags_MouseButtonRight);
              drawMouseSegment(VK_XBUTTON2, "MB5", side1Min, side1Max,
                               6.0f * keyboardScale,
                               ImDrawFlags_RoundCornersAll);
              ...
              ImGui::InvisibleButton(
                  "##mouse_x2",
                  ImVec2(side2Max.x - side2Min.x, side2Max.y - side2Min.y),
                  ImGuiButtonFlags_MouseButtonLeft |
                      ImGuiButtonFlags_MouseButtonRight);
              drawMouseSegment(VK_XBUTTON1, "MB4", side2Min, side2Max,
                               6.0f * keyboardScale,
                               ImDrawFlags_RoundCornersAll);
            }
            ...
            if (ImGui::BeginPopup("Rebind Config##layout")) {
              // Also block global ESC-to-close-GUI while editing inside this
              // popup.
              MarkRebindBindingActive();

              const bool nestedLayoutPopupOpen =
                  ImGui::IsPopupOpen("Triggers Custom##layout") ||
                  ImGui::IsPopupOpen("Custom Unicode##layout");
              if (ImGui::IsKeyPressed(ImGuiKey_Escape) &&
                  !nestedLayoutPopupOpen) {
                s_layoutEscapeRequiresRelease = true;
                layoutEscapeConsumedThisFrame = true;
              ...
              auto isNoOpRebindForKey = [&](const KeyRebind& r,
                                            DWORD originalVk) -> bool {
                if (r.fromKey != originalVk) return false;
                if (r.toKey != originalVk) return false;
                if (r.customOutputVK != 0 && r.customOutputVK != r.toKey)
                  return false;
              ...
              auto eraseRebindIndex = [&](int eraseIdx) {
                if (eraseIdx < 0 ||
                    eraseIdx >= (int)g_config.keyRebinds.rebinds.size())
                  return;
                g_config.keyRebinds.rebinds.erase(
                    g_config.keyRebinds.rebinds.begin() + eraseIdx);
                g_configIsDirty = true;
                std::lock_guard<std::mutex> hotkeyLock(g_hotkeyMainKeysMutex);
                RebuildHotkeyMainKeys_Internal();
              ...
              auto findBestRebindIndexForKey = [&](DWORD fromVk) -> int {
                ...
                for (int ri = 0; ri < (int)g_config.keyRebinds.rebinds.size();
                     ++ri) {
              ...
              if (idx < 0 || idx >= (int)g_config.keyRebinds.rebinds.size() ||
                  g_config.keyRebinds.rebinds[idx].fromKey !=
                      s_layoutContextVk) {
              ...
              auto typesValueFor = [&](const KeyRebind* rb,
                                       DWORD originalVk) -> std::string {
                if (!rb) return VkToString(originalVk);
                if (rb->useCustomOutput && rb->customOutputUnicode != 0)
                  return codepointToDisplay((uint32_t)rb->customOutputUnicode);
                DWORD textVk = (rb->useCustomOutput && rb->customOutputVK != 0)
                                   ? rb->customOutputVK
                                   : originalVk;
              ...
              auto triggersValueFor = [&](const KeyRebind* rb,
                                          DWORD originalVk) -> std::string {
                DWORD triggerVk = rb ? rb->toKey : originalVk;
                if (triggerVk == 0) triggerVk = originalVk;
                DWORD displayScan =
                    (rb && rb->useCustomOutput && rb->customOutputScanCode != 0)
                        ? rb->customOutputScanCode
                        : getScanCodeWithExtendedFlag(triggerVk);
                return scanCodeToDisplayName(displayScan, triggerVk);
              };

              static std::vector<std::pair<DWORD, std::string>>
                  s_knownScanCodes;
              ...
              if (!s_knownScanCodesBuilt) {
                ...
                std::sort(s_knownScanCodes.begin(), s_knownScanCodes.end(),
                          [](const auto& a, const auto& b) {
                            if (a.first == b.first) return a.second < b.second;
                            return a.first < b.first;
                          });
              ...
              if (s_layoutBindTarget != LayoutBindTarget::None &&
                  s_layoutBindIndex >= 0 &&
                ...
                if (ConsumeBindingInputEventSince(s_layoutBindLastSeq,
                                                  capturedVk, capturedLParam,
                                                  capturedIsMouse)) {
                  if (capturedVk == VK_ESCAPE) {
                    auto& r = g_config.keyRebinds.rebinds[s_layoutBindIndex];
                    int maybeErase = isNoOpRebindForKey(r, r.fromKey)
                                         ? s_layoutBindIndex
                                         : -1;
                    s_layoutBindTarget = LayoutBindTarget::None;
                    s_layoutBindIndex = -1;
                    if (maybeErase != -1) {
                      ...
                      if (s_layoutContextPreferredIndex == maybeErase)
                        s_layoutContextPreferredIndex = -1;
                    }
                    ...
                    if (s_layoutBindTarget == LayoutBindTarget::TypesVk) {
                      ...
                      if (s_layoutBindIndex >= 0 &&
                          s_layoutBindIndex <
                              (int)s_rebindUnicodeTextEdit.size()) {
                        s_rebindUnicodeTextEdit[s_layoutBindIndex].clear();
                      }

                      if (r.customOutputVK == r.fromKey) {
                        r.customOutputVK = 0;
                        if (r.customOutputScanCode == 0 &&
                            r.customOutputUnicode == 0)
                          r.useCustomOutput = false;
                      }
                      g_configIsDirty = true;
                    } else if (s_layoutBindTarget ==
                               LayoutBindTarget::TriggersVk) {
                      r.toKey = capturedVk;
                      g_configIsDirty = true;
                    }
              ...
              if (ImGui::BeginPopupModal("Custom Unicode##layout", NULL,
                                         ImGuiWindowFlags_AlwaysAutoResize |
                ...
                ImGui::TextUnformatted(
                    "Enter a Unicode character or codepoint:");
                ImGui::TextDisabled("Examples: ø   U+00F8   0x00F8");
                ImGui::Separator();
                ImGui::SetNextItemWidth(260.0f);
                ImGui::InputTextWithHint("##unicode", "ø or U+00F8",
                                         &s_layoutUnicodeEditText);
                ImGui::Spacing();

                const bool canApply = true;
                if (ImGui::Button("Apply", ImVec2(120, 0)) && canApply) {
                  if (s_layoutUnicodeEditIndex >= 0 &&
                      s_layoutUnicodeEditIndex <
                          (int)g_config.keyRebinds.rebinds.size()) {
                    auto& r =
                        g_config.keyRebinds.rebinds[s_layoutUnicodeEditIndex];
                    if (s_layoutUnicodeEditText.empty()) {
                      r.customOutputUnicode = 0;
                      if (r.customOutputVK == 0 && r.customOutputScanCode == 0)
                        r.useCustomOutput = false;
                      g_configIsDirty = true;
                    } else {
                      uint32_t cp = 0;
              ...
              KeyRebind* rbPtr =
                  (idx >= 0 && idx < (int)g_config.keyRebinds.rebinds.size())
                      ? &g_config.keyRebinds.rebinds[idx]
                      : nullptr;
              const std::string typesValue =
                  typesValueFor(rbPtr, s_layoutContextVk);
              const std::string triggersValue =
                  triggersValueFor(rbPtr, s_layoutContextVk);

              // Wider so key names like "RSHIFT" / "NUMLOCK" fit without
              // truncation.
              ...
              ImGui::SameLine();
              {
                std::string label =
                    (s_layoutBindTarget == LayoutBindTarget::TypesVk)
                        ...
                        : typesValue;
                if (ImGui::Button((label + "##types").c_str(),
                                  ImVec2(vBtnW, 0))) {
              ...
              if (ImGui::Button("Custom##types_custom", ImVec2(112, 0))) {
                ...
                if (idx >= 0) {
                  ...
                  s_layoutUnicodeEditText =
                      (r.customOutputUnicode != 0)
                          ? formatCodepointUPlus(
                                (uint32_t)r.customOutputUnicode)
                          : std::string();
                  MarkRebindBindingActive();
                }
              }
              ...
              ImGui::SameLine();
              {
                std::string label =
                    (s_layoutBindTarget == LayoutBindTarget::TriggersVk)
                        ? std::string("[Press key...]")
                        : triggersValue;
                if (ImGui::Button((label + "##triggers").c_str(),
                                  ImVec2(vBtnW, 0))) {
                  idx = createRebindForKeyIfMissing(s_layoutContextVk);
                  s_layoutContextPreferredIndex = idx;
                  if (idx >= 0) {
              ...
              {
                idx = (idx >= 0) ? idx
                                 : findBestRebindIndexForKey(s_layoutContextVk);
                KeyRebind* r =
                    (idx >= 0 && idx < (int)g_config.keyRebinds.rebinds.size())
                        ? &g_config.keyRebinds.rebinds[idx]
                        : nullptr;

                if (ImGui::Button("Custom##triggers_scan_custom",
                                  ImVec2(82, 0))) {
                ...
                if (ImGui::BeginPopup("Triggers Custom##layout")) {
                  ...
                  DWORD curScan =
                      (r && r->useCustomOutput && r->customOutputScanCode != 0)
                          ? r->customOutputScanCode
                          : getScanCodeWithExtendedFlag(curTriggerVk);
                  std::string preview =
                      scanCodeToDisplayName(curScan, curTriggerVk);

                  ImGui::Text("Current: %s", preview.c_str());
                  ImGui::Separator();

                  bool isDefault = !(r && r->useCustomOutput &&
                  ...
                  if (ImGui::Selectable("Default", isDefault)) {
                    ...
                    if (idx >= 0) {
                      ...
                      if (rr.customOutputVK == 0 && rr.customOutputUnicode == 0)
                        rr.useCustomOutput = false;
                      g_configIsDirty = true;
                    }
                  }
                  ImGui::Separator();

                  ImGui::BeginChild("##triggers_custom_list",
                                    ImVec2(360.0f, 230.0f), true);
                  for (const auto& it : s_knownScanCodes) {
                    ...
                    const std::string name =
                        scanCodeToDisplayName(scan, vkFromScan);
                    const std::string itemLabel =
                        name + "  (" + formatScanHex(scan) + ")##scan_" +
                        std::to_string((unsigned)scan);

                    const bool selected = (r && r->useCustomOutput &&
                                           r->customOutputScanCode == scan);
              ...
              if (ImGui::Button("Reset##layout_reset", ImVec2(170, 0))) {
                if (idx >= 0 && idx < (int)g_config.keyRebinds.rebinds.size()) {
                  ...
                  if (isNoOpRebindForKey(r, r.fromKey)) {
                    ...
                    std::lock_guard<std::mutex> hotkeyLock(
                        g_hotkeyMainKeysMutex);
                    RebuildHotkeyMainKeys_Internal();
                  }
                }
              }
            ...
            const float totalH = (keyboardTotalH > mouseDiagramTotalH)
                                     ? keyboardTotalH
                                     : mouseDiagramTotalH;
            ImGui::SetCursorPos(
                ImVec2(layoutStart.x, layoutStart.y + totalH + gap));

            {
              ...
              for (const auto& r : g_config.keyRebinds.rebinds) {
                ...
                if (r.useCustomOutput && r.customOutputUnicode != 0) {
                  typesStr =
                      codepointToDisplay((uint32_t)r.customOutputUnicode);
                } else {
                  DWORD textVk = (r.useCustomOutput && r.customOutputVK != 0)
                                     ? r.customOutputVK
                                     : r.fromKey;
                ...
                DWORD displayScan =
                    (r.useCustomOutput && r.customOutputScanCode != 0)
                        ? r.customOutputScanCode
                        : getScanCodeWithExtendedFlag(triggerVk);
                std::string triggersStr =
                    scanCodeToDisplayName(displayScan, triggerVk);
                ImGui::Text("%s -> %s & %s", fromStr.c_str(), typesStr.c_str(),
                            triggersStr.c_str());
                anyShown = true;
              }
              if (!anyShown) {
            ...
            const bool anyRebindBindUiActiveAfter =
                ...
                ImGui::IsPopupOpen("Triggers Custom##layout") ||
                ImGui::IsPopupOpen("Custom Unicode##layout");
            if (!blockLayoutEscapeThisFrame && !layoutEscapeConsumedThisFrame &&
                escapePressedThisFrame && !anyRebindBindUiActiveAfter) {
        ...
        if (is_rebind_from_binding) {
          MarkRebindBindingActive();
        }
        if (is_rebind_from_binding) {
          ImGui::OpenPopup("Bind From Key");
        }

        if (ImGui::BeginPopupModal("Bind From Key", NULL,
                                   ImGuiWindowFlags_AlwaysAutoResize |
                                       ImGuiWindowFlags_NoTitleBar)) {
          ...
          if (ImGui::IsWindowAppearing()) {
            s_lastBindingInputSeqInputs1 = GetLatestBindingInputSequence();
          }

          DWORD capturedVk = 0;
          LPARAM capturedLParam = 0;
          bool capturedIsMouse = false;
          if (ConsumeBindingInputEventSince(s_lastBindingInputSeqInputs1,
                                            capturedVk, capturedLParam,
                                            capturedIsMouse)) {
            if (capturedVk == VK_ESCAPE) {
              s_rebindFromKeyToBind = -1;
              ...
              if (capturedVk != VK_LWIN && capturedVk != VK_RWIN) {
                if (s_rebindFromKeyToBind != -1 &&
                    s_rebindFromKeyToBind <
                        (int)g_config.keyRebinds.rebinds.size()) {
                  g_config.keyRebinds.rebinds[s_rebindFromKeyToBind].fromKey =
                      capturedVk;
                  g_configIsDirty = true;
                  std::lock_guard<std::mutex> hotkeyLock(g_hotkeyMainKeysMutex);
                  RebuildHotkeyMainKeys_Internal();
        ...
        if (is_vk_binding) {
          ImGui::OpenPopup("Bind Output VK");
        }

        if (ImGui::BeginPopupModal("Bind Output VK", NULL,
                                   ImGuiWindowFlags_AlwaysAutoResize |
                                       ImGuiWindowFlags_NoTitleBar)) {
          ...
          if (ImGui::IsWindowAppearing()) {
            s_lastBindingInputSeqInputs2 = GetLatestBindingInputSequence();
          }

          DWORD capturedVk = 0;
          LPARAM capturedLParam = 0;
          bool capturedIsMouse = false;
          if (ConsumeBindingInputEventSince(s_lastBindingInputSeqInputs2,
                                            capturedVk, capturedLParam,
                                            capturedIsMouse)) {
            if (capturedVk == VK_ESCAPE) {
              s_rebindOutputVKToBind = -1;
              ...
              if (capturedVk != VK_LWIN && capturedVk != VK_RWIN) {
                if (s_rebindOutputVKToBind >= 0 &&
                    s_rebindOutputVKToBind <
                        (int)g_config.keyRebinds.rebinds.size()) {
                  auto& rebind =
                      g_config.keyRebinds.rebinds[s_rebindOutputVKToBind];
        ...
        if (is_text_vk_binding) {
          MarkRebindBindingActive();
        }
        if (is_text_vk_binding) {
          ImGui::OpenPopup("Bind Text Override VK");
        }
        ...
        if (ImGui::BeginPopupModal("Bind Text Override VK", NULL,
          ...
          if (ImGui::IsWindowAppearing()) {
            s_lastBindingInputSeqInputs2b = GetLatestBindingInputSequence();
          }

          DWORD capturedVk = 0;
          LPARAM capturedLParam = 0;
          bool capturedIsMouse = false;
          if (ConsumeBindingInputEventSince(s_lastBindingInputSeqInputs2b,
                                            capturedVk, capturedLParam,
                                            capturedIsMouse)) {
            if (capturedVk == VK_ESCAPE) {
              s_rebindTextOverrideVKToBind = -1;
              ...
              if (capturedVk != VK_LWIN && capturedVk != VK_RWIN) {
                if (s_rebindTextOverrideVKToBind >= 0 &&
                    s_rebindTextOverrideVKToBind <
                        (int)g_config.keyRebinds.rebinds.size()) {
                  auto& rebind =
                      g_config.keyRebinds.rebinds[s_rebindTextOverrideVKToBind];
        ...
        if (is_scan_binding) {
          ImGui::OpenPopup("Bind Output Scan");
        }

        if (ImGui::BeginPopupModal("Bind Output Scan", NULL,
                                   ImGuiWindowFlags_AlwaysAutoResize |
                                       ImGuiWindowFlags_NoTitleBar)) {
          ...
          if (ImGui::IsWindowAppearing()) {
            s_lastBindingInputSeqInputs3 = GetLatestBindingInputSequence();
          }

          DWORD capturedVk = 0;
          LPARAM capturedLParam = 0;
          bool capturedIsMouse = false;
          if (ConsumeBindingInputEventSince(s_lastBindingInputSeqInputs3,
                                            capturedVk, capturedLParam,
                                            capturedIsMouse)) {
            if (capturedVk == VK_ESCAPE) {
              s_rebindOutputScanToBind = -1;
              ...
              if (capturedVk != VK_LWIN && capturedVk != VK_RWIN) {
                if (s_rebindOutputScanToBind >= 0 &&
                    s_rebindOutputScanToBind <
                        (int)g_config.keyRebinds.rebinds.size()) {
                  auto& rebind =
                      g_config.keyRebinds.rebinds[s_rebindOutputScanToBind];

                  if (capturedVk == VK_LBUTTON || capturedVk == VK_RBUTTON ||
                      capturedVk == VK_MBUTTON || capturedVk == VK_XBUTTON1 ||
                      capturedVk == VK_XBUTTON2) {
                    rebind.toKey = capturedVk;
                    rebind.customOutputScanCode = 0;
                  } else {
                    UINT scanCode =
                        static_cast<UINT>((capturedLParam >> 16) & 0xFF);
                    if ((capturedLParam & (1LL << 24)) != 0) {
                      scanCode |= 0xE000;
                    }
                    ...
                    Log("[Rebind][GameKeybind] capturedVk=" +
                        std::to_string(capturedVk) + " capturedLParam=" +
                        std::to_string(static_cast<long long>(capturedLParam)) +
                        " storedScan=" + std::to_string(scanCode) +
                        " ext=" + std::string((scanCode & 0xFF00) ? "1" : "0"));
                  }
        ...
        ImGui::Spacing();
        ImGui::TextDisabled(
            "Configure key rebinds in the Keyboard Layout window (right-click "
            "keys). ");
      }
