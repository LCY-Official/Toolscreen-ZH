if (ImGui::BeginTabItem("General")) {
    ...
    auto RemoveModeAndHotkey = [&](const std::string& modeId) {
        ...
        g_config.hotkeys.erase(std::remove_if(g_config.hotkeys.begin(), g_config.hotkeys.end(), [&](const HotkeyConfig& h) { return EqualsIgnoreCase(h.secondaryMode, modeId); }), g_config.hotkeys.end());
    ...
    auto RenderModeTableRow = [&](const std::string& modeId, const char* label, const char* hotkeyLabel, int defaultWidth,
        ...
        if (modeConfig) {
            ...
            if (Spinner("##w", &modeConfig->width, 10, 1, maxWidth, 64, 3)) {
                if (!modeConfig->widthExpr.empty()) {
                    modeConfig->widthExpr.clear();
                }
        ...
        if (modeConfig) {
            ...
            if (Spinner("##h", &modeConfig->height, 10, 1, maxHeight, 64, 3)) {
                if (!modeConfig->heightExpr.empty()) {
                    modeConfig->heightExpr.clear();
                }
    ...
    if (ImGui::BeginTable("ModeTable", 5, ImGuiTableFlags_SizingFixedFit)) {
        ...
        RenderModeTableRow("Fullscreen", "Fullscreen", "fullscreen_hotkey", monitorWidth, monitorHeight, monitorWidth, monitorHeight, EyeZoomInlineKind::None, true);
...

    {
        auto FindNinjabrainBotImage = [&]() -> ImageConfig* {
            for (auto& img : g_config.images) {
                if (EqualsIgnoreCase(img.name, "Ninjabrain Bot")) {
                    return &img;
                }
            }
            return nullptr;
        ...
        auto AddNinjabrainToMode = [&](const std::string& modeId) {
            ModeConfig* mode = GetModeConfig(modeId);
            if (mode && !ModeHasNinjabrain(modeId)) {
                mode->imageIds.push_back("Ninjabrain Bot");
            }
        };
        ...
        auto RemoveNinjabrainFromMode = [&](const std::string& modeId) {
            ModeConfig* mode = GetModeConfig(modeId);
            if (mode) {
                mode->imageIds.erase(std::remove_if(mode->imageIds.begin(), mode->imageIds.end(), [](const std::string& id) { return EqualsIgnoreCase(id, "Ninjabrain Bot"); }), mode->imageIds.end());
            }
        };
        ...
        if (ImGui::Checkbox("Ninjabrainbot Overlay", &ninjabrainEnabled)) {
            if (ninjabrainEnabled) {
                if (!FindNinjabrainBotImage()) {
                    CreateNinjabrainBotImage();
                }
    ...
    auto RenderMirrorAssignments = [&](const std::string& modeId, const char* label) {
        ...
        if (ImGui::TreeNode(label)) {
            ...
            if (ImGui::BeginCombo("##AddMirrorOrGroup", "[Add Mirror/Group]")) {
                ...
                if (!g_config.mirrors.empty() && !g_config.mirrorGroups.empty()) {
                    ImGui::Separator();
                }
