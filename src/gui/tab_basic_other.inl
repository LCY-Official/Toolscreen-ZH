if (ImGui::BeginTabItem(trc("tabs.other"))) {
...

    {
        ...
        ImGui::Text("%s", trc("label.auto_borderless"));
        ImGui::SameLine();
    ...
    if (ImGui::SliderInt("##FpsLimit", &fpsLimitValue, 30, 1001, fpsLimitValue == 1001 ? trc("label.unlimited") : ftrc("label.formatted_fps_limit", fpsLimitValue))) {
        g_config.fpsLimit = (fpsLimitValue == 1001) ? 0 : fpsLimitValue;
        g_configIsDirty = true;
    }
    ImGui::SameLine();
    HelpMarker(trc("tooltip.fps_limit.basic"));

    if (ImGui::Checkbox(trc("label.hide_animations_in_game"), &g_config.hideAnimationsInGame)) {
        g_configIsDirty = true;
    }
    ImGui::SameLine();
