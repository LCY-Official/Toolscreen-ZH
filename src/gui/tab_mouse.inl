if (ImGui::BeginTabItem(trc("tabs.mouse"))) {
    ...
    ImGui::SeparatorText(trc("tabs.mouse.mouse_settings"));
    ...
    ImGui::Spacing();
    ImGui::SeparatorText(trc("tabs.mouse.cursor_configuration"));

    if (ImGui::Checkbox(trc("tabs.mouse.enable_custom_cursors"), &g_config.cursors.enabled)) {
        g_configIsDirty = true;
    ...
    if (g_config.cursors.enabled) {
        ImGui::Text(trc("tabs.mouse.configure_cursors_for_different_game_states"));
        ImGui::Spacing();
        ...
        if (!cursorListInitialized) {
            ...
            for (const auto& cursorName : cursorNames) {
                ...
                if (cursorName.find("Cross") != std::string::npos) {
                    description = trc("tabs.mouse.crosshair_cursor");
                } else if (cursorName.find("Arrow") != std::string::npos) {
                    description = trc("tabs.mouse.arrow_pointer_cursor");
                } else {
                    description = trc("tabs.mouse.custom_cursor");
                }
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            ImGui::Text(trc("tabs.mouse.cursor"));
            ImGui::SameLine();
        ...
        if (ImGui::Button((tr("button.reset_to_defaults") + "##cursors")).c_str()) {
            ImGui::OpenPopup(trc("tabs.mouse.reset_cursors_to_defaults"));
        }
