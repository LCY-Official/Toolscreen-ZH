if (ImGui::BeginTabItem(trc("tabs.images"))) {
    ...
    for (size_t i = 0; i < g_config.images.size(); ++i) {
        ...
        if (ImGui::BeginPopupModal(img_popup_id.c_str(), NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
            ImGui::Text("%s",ftrc("images.delete_images_confirm", img.name));
            ImGui::Separator();
        ...
        if (node_open) {
            ...
            if (hasDuplicate) {
                ImGui::SameLine();
                ImGui::TextColored(ImVec4(1.0f, 0.3f, 0.3f, 1.0f), "%s", trc("images.name_duplicate"));
            }
            ...
            if (ImGui::Button(("+ " + tr("images.add_color_key")).c_str())) {
    ...
    ImGui::Separator();
    if (ImGui::Button(trc("images.add_new_image"))) {
        ImageConfig newImg;
    ...
    ImGui::SameLine();
    if (ImGui::Button((tr("images.reset_to_defaults")+"##images").c_str())) {
        ImGui::OpenPopup((tr("images.reset_to_defaults") + "?").c_str());
    }

    if (ImGui::BeginPopupModal((tr("images.reset_to_defaults") + "?").c_str(), NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
