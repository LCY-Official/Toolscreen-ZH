...
#define TOOLSCREEN_VERSION_STRING    \
  TOSTRING(TOOLSCREEN_VERSION_MAJOR) \
  "." TOSTRING(TOOLSCREEN_VERSION_MINOR) "." TOSTRING(TOOLSCREEN_VERSION_PATCH)
...
struct GameVersion {
  ...
  GameVersion() = default;
  GameVersion(int maj, int min, int pat)
      : major(maj), minor(min), patch(pat), valid(true) {}
...
bool IsVersionInRange(const GameVersion& version, const GameVersion& minVer,
                      const GameVersion& maxVer);
